...
for (const loc of ["zh-Hans", "zh-Hant"]) {
  for (const key of files[loc].keys()) {
    if (!files.en.has(key))
      report.missingPages.push({ key, missingIn: "en", orphanFrom: loc });
  }
}
...
for (const s of report.stale)
  byPage[s.page] = Math.max(byPage[s.page] ?? 0, s.daysBehind);
for (const [page, days] of Object.entries(byPage)
  .sort((a, b) => b[1] - a[1])
  .slice(0, 15)) {
