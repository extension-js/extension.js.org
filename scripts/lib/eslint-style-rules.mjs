    ...
    return {
      Program() {
        ...
        context.report({
          ...
          fix: (fixer) =>
            fixer.insertTextAfterRange([0, lines[0].length], "\n"),
        });
      },
    };
  },
    ...
    return {
      Program(node) {
        ...
        const header = leading
          ...
          .filter(
            (comment) => !(allowBanner && BANNER_ART.test(comment.value)),
          );
...
const paddingLineBetweenStatements = {
  ...
  create(context) {
    ...
    return basePadding.create(
      Object.create(context, { report: { value: report } }),
    );
  },
};
