...
- Six more browsers by name: Brave, Opera, Vivaldi, Yandex, Waterfox, and LibreWolf, alongside the `chromium-based` and `gecko-based` targets for custom binaries. Forks inherit the `chrome:` and `firefox:` manifest keys of their engine family, so a fork target does not need its own prefix (since 4.1.19 the family prefixes are `chromium:` and `gecko:`, and `chrome:` and `firefox:` name one browser each, see [Browser-specific manifest fields](/docs/features/browser-specific-fields)).
