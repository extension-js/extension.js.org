...
浏览器扩展使用与 Web 相同的语言来构建。JavaScript 和 TypeScript 驱动逻辑，HTML 定义扩展页面，CSS（或 Less、Sass、CSS Modules、Tailwind）为它们设置样式。Extension.js 通过 Rspack 并使用具备扩展感知能力的默认值把这一切连接起来，你不需要为每个浏览器配置打包器。各文件扩展名归属哪里，见 [浏览器扩展中的 JavaScript 与 TypeScript 文件](/zh-Hans/docs/concepts/javascript-typescript-browser-extension-files)。
...
Extension.js 通过 [按浏览器的 manifest 字段](/zh-Hans/docs/features/browser-specific-fields) 和按目标的构建输出来处理这些差异。完整流水线见 [跨浏览器兼容性](/zh-Hans/docs/features/cross-browser-compatibility)。
...
- 试用 [`extension create`](/zh-Hans/docs/commands/create) 在几秒内脚手架生成一个扩展。
- 了解 [Manifest V3](/zh-Hans/docs/concepts/manifest-v3) 以及浏览器间的 API 差异。
- 查看 [跨浏览器兼容性](/zh-Hans/docs/features/cross-browser-compatibility) 了解构建流水线。
- 选择一个 [浏览器目标](/zh-Hans/docs/browsers/browsers-available) 进行开发。
