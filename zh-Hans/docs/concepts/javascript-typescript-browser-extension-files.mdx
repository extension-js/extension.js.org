...
如果你想了解的是作为 Chrome 或 Firefox 软件附加组件意义上的"扩展"，请从 [什么是浏览器扩展？](/zh-Hans/docs/concepts/what-is-a-browser-extension) 开始。
...
在 Manifest V3 中，当 `manifest.json` 在 `background` 块里包含 `"type": "module"` 时，背景 `service_worker` 会作为模块运行。详情见 [Manifest V3 排错](/zh-Hans/docs/concepts/manifest-v3)。
...
你不需要从零写一个 `tsconfig.json`。Extension.js 自带合理的默认值。`chrome.*`、`browser.*`、`import.meta.env` 以及公共环境变量键的类型来自 Extension.js 自带的 ambient 类型。从 Extension.js 4.1.19 起，这些类型所需的 `@types/chrome` 和 `@types/webextension-polyfill` 会随 `extension` 一起安装。参见 [chrome 与 browser API 的类型](/zh-Hans/docs/languages-and-frameworks/extension-api-types) 和 [TypeScript](/zh-Hans/docs/languages-and-frameworks/typescript)。
...
React 既可以在扩展页面（popup、options、side panel、new-tab）中工作，也可以在注入到网页中的 content script 中工作。设置与 shadow DOM 模式见 [React](/zh-Hans/docs/languages-and-frameworks/react)。
...
- 试用 [`extension create`](/zh-Hans/docs/commands/create) 来脚手架生成一个 TypeScript 或 React 扩展。
...
- 使用 [`extension build`](/zh-Hans/docs/commands/build) 构建。
