...
- 通过 [ready.json 与事件流](/zh-Hans/docs/workflows/playwright-e2e) 解析会话状态，而不是解析终端输出。
- 在 [浏览器配置文件](/zh-Hans/docs/browsers/browser-profile) 中了解受管理配置文件的行为。
- 在 [读懂 CLI 输出](/zh-Hans/docs/concepts/reading-cli-output) 中了解会话在终端一侧的样子。
