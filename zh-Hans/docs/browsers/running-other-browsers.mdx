...
description: "通过向 Extension.js 显式提供二进制路径，使用你已安装的 Chrome、Brave、Vivaldi、Waterfox 或 Firefox，而不是托管下载的浏览器。"
keywords:
  [
    "extension.js",
    "firefox",
...
如果该浏览器未安装，Extension.js 会带上安装指引退出。具名分支会继承其家族的 manifest 键，因此带 `chromium:`/`firefox:` 前缀的字段会被正确解析（参见 [按浏览器划分的 manifest 字段](/zh-Hans/docs/features/browser-specific-fields)）。
...
| **Waterfox**                  | Gecko-based 浏览器    | `--browser=waterfox` 或 `--gecko-binary`   | [waterfox.com](https://www.waterfox.com)                  |
| **LibreWolf**                 | Gecko-based 浏览器    | `--browser=librewolf` 或 `--gecko-binary`  | [librewolf.net](https://librewolf.net)                    |
| **Zen**                       | Gecko-based 浏览器    | `--browser=zen` 或 `--gecko-binary`        | [zen-browser.app](https://zen-browser.app)                |
| **Floorp**                    | Gecko-based 浏览器    | `--browser=floorp` 或 `--gecko-binary`     | [floorp.app](https://floorp.app)                          |
...
- `librewolf` 需要在每台机器上开启一次远程调试。LibreWolf 每次启动都会把 `devtools.debugger.remote-enabled` 重置为 `false`，覆盖 Extension.js 创建的配置文件，因此启动前会检查 `~/.librewolf/librewolf.overrides.cfg`（Windows 上为 `%USERPROFILE%\.librewolf\librewolf.overrides.cfg`），缺少时会停止并给出需要加入的两行：
...
把那个 `dist/<browser>` 文件夹加载进你已经在运行的浏览器。在 Chromium 系浏览器中，开启开发者模式后在 `chrome://extensions` 选择 "Load unpacked"。加载的扩展会随保存持续更新。关于用 `--wait` 做就绪同步，见 [`dev`](/zh-Hans/docs/commands/dev)。
...
排除 `--force-dark-mode` 会丢弃整个外观包，包括 Gecko 的 preferences。排除条目也按开关名匹配，所以 `--enable-features` 会移除 `--enable-features=WebUIDarkMode`。默认 flag 清单和完整排除规则见[浏览器 flag](/zh-Hans/docs/browsers/browser-flags)。
