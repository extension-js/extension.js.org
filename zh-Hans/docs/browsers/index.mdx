...
你也可以并行运行两个 dev 会话，每个浏览器一个终端。会话状态（ready 契约、控制通道、日志、profile）都按浏览器分键，所以 Chrome 会话与 Firefox 会话永远不会冲突。磁盘上的布局参见 [会话产物](/docs/concepts/session-artifacts)。
...
| 在 WSL 下开发             | [WSL 支持](/docs/browsers/wsl)                           |
