---
title: "Chrome、Firefox 与 Edge 的浏览器启动 flag"
description: "在 Extension.js 中使用 firefox://flags、chrome://flags 与浏览器启动 flag，在扩展开发期间控制 Chrome、Firefox 与 Edge 的运行时行为。"
keywords:
  [
    "firefox flags",
    "firefox://flags",
    "browser://flags",
    "chrome://flags",
    "extension.js",
    "chrome",
    "firefox",
    "edge",
    "browser",
    "devtools",
    "docs",
    "browsers",
    "allow control",
    "no sandbox",
    "webgl",
    "gpu",
    "webgpu",
  ]
---

为调试、自动化与运行时实验控制浏览器的启动行为。

<Note>
  **在找 `chrome://flags` 或 `edge://flags`？** 把它输入地址栏即可，这是浏览器自带的页面。**Firefox 没有
  `firefox://flags`**，请改用 `about:config`。

如果你切换 flag 是为了**测试或构建浏览器扩展**，Extension.js 会在 Chrome、Edge 与 Firefox
上按项目自动应用它们，你不需要每次运行都手动传入 flag。[30 秒创建你的第一个扩展 →](/zh-Hans/docs/getting-started)

</Note>

无需修改扩展源代码，即可调整浏览器的启动行为。Extension.js 会合并你在 `extension.config.*` 中配置的浏览器 flag，并在 `dev`、`preview` 与 `start` 流程中应用它们。

## Firefox 有 `firefox://flags` 吗？

Firefox 并不像 Chromium 系浏览器使用 `chrome://flags` 或 `edge://flags` 那样使用 `firefox://flags`。Firefox 通过 `about:config`（preferences）暴露运行时开关，并接受命令行传入的启动 flag。

如果你搜索过 `firefox://flags`、`firefox //flags`、`browser://flags`、`mozilla://flags` 或 `about flags firefox`，你大概率是想在扩展开发期间修改浏览器行为。在 Extension.js 中，这件事分两处来做：

- 用 [`browserFlags`](/zh-Hans/docs/browsers/browser-flags) 配置 Extension.js 传给浏览器二进制的启动 flag。
- 用 [Firefox preferences](/zh-Hans/docs/browsers/browser-preferences) 配置原本会写在 `about:config` 中的、可复现的 Gecko 运行时行为。

这两种方式在同一个 Extension.js 项目里都同时适用于 Chrome、Edge 与 Firefox 的扩展开发。

## 其他浏览器中的 flag

Extension.js 对任何基于 Chromium 的浏览器都以相同方式启动与配置。

- **`brave://flags`、`opera://flags`、`vivaldi://flags`、`yandex://flags`**：它们都基于
  Chromium，因此行为与 `chrome://flags` 完全一致，Extension.js 会按项目管理它们。
- **`internet://flags` 与 `browser://flags`**：这些不是真实存在的浏览器 scheme。你多半是在找
  `chrome://flags`（Chromium）或 `about:config`（Firefox）。
- **`edge://flags`**：Edge 就是 Chromium，因此在 Extension.js 中它的表现与 Chrome 完全一样。

在构建的扩展需要特定的启动 flag？在 `extension.config.*` 里设置一次，Extension.js
每次运行都会应用。参见下文的 [`browserFlags`](/zh-Hans/docs/browsers/browser-flags)。

## GPU 与 WebGL flag

无头会话、容器或虚拟机通常没有 GPU。此时 WebGL 上下文无法创建，需要它的 canvas 代码会抛错。通过 `browserFlags` 传入 Chromium 开关：

```js
export default {
  browser: {
    chrome: {
      browserFlags: [
        "--ignore-gpu-blocklist",
        "--use-gl=angle",
        "--use-angle=swiftshader",
      ],
    },
  },
};
```

`--ignore-gpu-blocklist` 让 Chromium 使用它的黑名单本来会拒绝的 GPU。`--use-gl=angle` 配合 `--use-angle=swiftshader` 用软件渲染 WebGL，这是持续集成中的正确选择。`--enable-unsafe-webgpu` 在浏览器仍然限制 WebGPU 的地方打开它。Firefox 没有等价的开关，请改为通过浏览器偏好设置 `webgl.force-enabled`。

## 模板示例

### `new-browser-flags`

![new-browser-flags template screenshot](/images/examples/newtab-browser-flags/screenshot.png)

通过一个配置了启动行为的 new-tab 扩展，直观了解浏览器 flag 的作用。

<CodeGroup>

```bash npm
npx extension@latest create my-extension --template=newtab-browser-flags
```

```bash pnpm
pnpx extension@latest create my-extension --template=newtab-browser-flags
```

```bash yarn
yarn dlx extension@latest create my-extension --template=newtab-browser-flags
```

```bash bun
bunx extension@latest create my-extension --template=newtab-browser-flags
```

```bash deno
deno run -A npm:extension@latest create my-extension --template=newtab-browser-flags
```

</CodeGroup>

仓库：[extension-js/examples/newtab-browser-flags](https://github.com/extension-js/examples/tree/main/examples/newtab-browser-flags)

## 工作原理

在 `extension.config.*` 中配置 flag：

- `browser.<target>.browserFlags`
- `commands.dev|start|preview.browserFlags`
- 可选的 `excludeBrowserFlags`，用于移除默认 flag 或用户自定义的 flag（具体行为取决于目标是 Chromium 还是 Firefox）。

覆盖顺序：浏览器默认值 → 命令默认值 → CLI 选中的命令上下文。

## flag 能力

| 配置键                                 | 作用                                              |
| -------------------------------------- | ------------------------------------------------- |
| `browser.<target>.browserFlags`        | 为指定的浏览器目标设置默认启动 flag。             |
| `commands.dev.browserFlags`            | 为 `dev` 运行添加或覆盖 flag。                    |
| `commands.start.browserFlags`          | 为 `start` 运行添加或覆盖 flag。                  |
| `commands.preview.browserFlags`        | 为 `preview` 运行添加或覆盖 flag。                |
| `browser.<target>.excludeBrowserFlags` | 为某个目标移除匹配的默认 flag 或用户自定义 flag。 |
| `commands.<name>.excludeBrowserFlags`  | 在特定命令上下文中移除 flag。                     |

### 配置示例

```js
export default {
  browser: {
    chrome: {
      browserFlags: ["--disable-web-security", "--auto-open-devtools-for-tabs"],
      excludeBrowserFlags: ["--mute-audio"],
    },
    firefox: {
      browserFlags: ["--devtools", "--new-instance"],
      excludeBrowserFlags: ["--devtools"],
    },
  },
};
```

## Chromium 与 Firefox 行为对比

- **Chromium 家族（`chrome`、`edge`、`chromium`、`chromium-based`）**
  - 从一组内部默认 flag 出发，然后追加你的 `browserFlags`。
  - `excludeBrowserFlags` 会移除匹配的默认 flag 与用户 flag。
  - Extension.js 自己管理 `--load-extension=...`，并会从用户传入的 flag 中将其过滤掉。

- **Firefox/Gecko 家族（`firefox`、`gecko-based` / `firefox-based`）**
  - 使用用户传入的 `browserFlags`（没有像 Chromium 那样的大量默认 flag）。
  - `excludeBrowserFlags` 会移除匹配的用户 flag，规则与 Chromium 相同。

两个家族共用同一条排除规则：当某个 flag 与排除值完全相等，或者排除值指向的开关后面接着 `=` 或 `,` 时，该 flag 会被移除。所以 `--disable-features` 也会移除 `--disable-features=Translate`，而 `--dev` 永远不会移除 `--devtools`。

### Chromium 默认 flag

启动 Chromium 系浏览器时，Extension.js 会自动应用这些 flag。如果不需要其中某些，可以用 `excludeBrowserFlags` 移除。

| flag                                                       | 用途                                                        |
| ---------------------------------------------------------- | ----------------------------------------------------------- |
| `--no-first-run`                                           | 关闭首次运行体验                                            |
| `--disable-client-side-phishing-detection`                 | 关闭钓鱼检测                                                |
| `--disable-sync`                                           | 关闭同步以避免账号提示                                      |
| `--disable-component-extensions-with-background-pages`     | 关闭不受 `--disable-extensions` 影响的内置扩展              |
| `--disable-default-apps`                                   | 关闭默认应用的安装                                          |
| `--disable-features=InterestFeedContentSuggestions`        | 关闭新标签页（NTP）上的 Discover feed                       |
| `--disable-features=Translate`                             | 关闭 Chrome 翻译                                            |
| `--hide-scrollbars`                                        | 在截图中隐藏滚动条                                          |
| `--mute-audio`                                             | 静音所有音频                                                |
| `--no-default-browser-check`                               | 关闭默认浏览器检查提示                                      |
| `--ash-no-nudges`                                          | 避免用户教育引导                                            |
| `--disable-search-engine-choice-screen`                    | 关闭搜索引擎选择界面                                        |
| `--disable-features=MediaRoute`                            | 关闭 Chrome Media Router 后台网络                           |
| `--use-mock-keychain`                                      | 在 Mac 上使用 mock keychain，避免阻塞性对话框               |
| `--disable-background-networking`                          | 关闭后台网络服务                                            |
| `--disable-breakpad`                                       | 关闭崩溃转储收集                                            |
| `--disable-component-update`                               | 关闭组件更新                                                |
| `--disable-domain-reliability`                             | 关闭域名可靠性监控                                          |
| `--no-pings`                                               | 关闭超链接审计 ping                                         |
| `--enable-features=SidePanelUpdates`                       | 确保侧边栏可见                                              |
| `--disable-features=DisableLoadExtensionCommandLineSwitch` | 允许命令行使用 `--load-extension`                           |
| `--disable-features=ExtensionDisableUnsupportedDeveloper`  | 让未打包扩展在 `runtime.reload()` 之后继续存活（Chromium 152+） |
| `--enable-unsafe-extension-debugging`                      | 允许 Chrome DevTools Protocol (CDP) 管理扩展（Chrome 126+） |
| `--silent-debugger-extension-api`                          | 抑制 "X is debugging this browser" 信息条                   |

### 始终生效的会话 flag

除了上面的默认表，每次 Chromium 启动还会带上一组固定的会话稳定性 flag。它们可以避免后台计时器、被遮挡的窗口以及渲染进程节流干扰自动化与 AI 驱动的会话。`excludeBrowserFlags` 无法移除它们：

```text
--disable-background-timer-throttling
--disable-renderer-backgrounding
--disable-backgrounding-occluded-windows
--disable-features=TranslateUI
--disable-hang-monitor
--disable-prompt-on-repost
--memory-pressure-off
--max_old_space_size=4096
--disable-dev-shm-usage
```

只有在 `dev` 模式下，Extension.js 才会用三个 flag 接上调试通道：`--remote-debugging-port=<derived port>`、`--remote-debugging-address=127.0.0.1` 与 `--remote-debugging-pipe`。生产模式的启动（`start`、`preview`）会跳过这三个。

### feature 开关的合并

Chromium 只认重复开关中最后一次出现的那个。因此 Extension.js 会把每一处 `--enable-features` 与 `--disable-features` 各自合并成一个用逗号连接的开关。默认值、配置与环境变量的值都会参与合并，重复的 feature 名会被去掉。你可以放心地在 `browserFlags` 里传入 `--disable-features=Foo`，而不会抹掉默认值。

### 环境变量开关

- `EXTENSION_BROWSER_FLAGS` 会把以空白分隔的 flag 追加到每一次启动。它在配置 flag 之后、也在 `excludeBrowserFlags` 过滤之后才应用，因此 `excludeBrowserFlags` 无法移除来自这个变量的 flag。
- `EXTENSION_HEADLESS=1`（或 `true`）会强制使用 `--headless=new`，除非配置或环境 flag 中已经传入了某种 `--headless` 变体。可以把它当作自动化会话的抢焦点防护。

<Note>
  在 Docker、持续集成（CI）或容器化环境中，Extension.js 还会自动应用
  `--no-sandbox` 与 `--disable-setuid-sandbox`。在 Linux 上，它通过以下任一信号判断处于容器中：
  `CI=true`、存在 `/.dockerenv` 文件、存在 `/run/.containerenv` 文件、
  `REMOTE_CONTAINERS=true`、`CODESPACES=true`，或设置了 `container` 变量。
</Note>

## 支持的目标与参考资料

| 浏览器         | 用法                                     | 更多信息                                                                               |
| -------------- | ---------------------------------------- | -------------------------------------------------------------------------------------- |
| Chrome         | `extension dev --browser=chrome`         | [Chrome Flags](https://peter.sh/experiments/chromium-command-line-switches/)           |
| Edge           | `extension dev --browser=edge`           | [Edge Flags](https://docs.microsoft.com/en-us/deployedge/microsoft-edge-policies)      |
| Firefox        | `extension dev --browser=firefox`        | [Firefox Flags](https://developer.mozilla.org/en-US/docs/Mozilla/Command_Line_Options) |
| Chromium-based | `extension dev --browser=chromium-based` | [Chromium Flags](https://peter.sh/experiments/chromium-command-line-switches/)         |
| Gecko-based    | `extension dev --browser=gecko-based`    | 基于 Firefox 的浏览器使用与 Firefox 相同的 flag。                                      |

针对基于引擎的目标，需要时可使用二进制 flag：

- `--chromium-binary=...`
- `--gecko-binary=...`

## 最佳实践

- **只加必要的 flag**：尽量精简 flag 列表，避免不稳定或不可移植的配置。
- **优先使用 `excludeBrowserFlags`，而不是整体替换默认值**：只移除与你的工作流冲突的部分。
- **不要手动传入 `--load-extension`**：Extension.js 会在内部管理扩展加载相关的 flag。
- **按浏览器家族分别验证**：在 Chromium 上可用的 flag，在 Firefox 上可能无效或被忽略。

## 下一步

- 进一步了解 [浏览器 preferences](/zh-Hans/docs/browsers/browser-preferences)。
- 进一步了解 [浏览器 profile](/zh-Hans/docs/browsers/browser-profile)。
- 在 [可用浏览器](/zh-Hans/docs/browsers/browsers-available) 中选择目标。
- 通过 [`extension.config.*`](/zh-Hans/docs/features/extension-configuration) 串联 flag 配置。
- 第一次接触扩展工具链？先从 [浏览器扩展框架](/zh-Hans/docs/compare) 概览开始。
