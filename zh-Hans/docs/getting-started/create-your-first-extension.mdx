...
<Frame>
  <iframe
    ...
    src="https://www.youtube-nocookie.com/embed/4Em5t3HEEhw?rel=0"
...
deno run -A npm:extension@4.1.16 create github-search
```
...
- `package.json` 标记项目根目录。[特殊文件夹](/zh-Hans/docs/features/special-folders)(`pages/`、`scripts/`、`public/`)和 `dist/` 输出都从那个目录解析，永远不从 `src/` 解析。
...
<Note>
  **首次运行。** `extension dev` 默认以 `chromium` 为目标，在一个版本固定的 Chrome for Testing
  和隔离配置文件里运行你的扩展，从不触碰你日常上网用的浏览器。当本机还没有这个浏览器时，
  首次运行会询问是否下载一次，下载完成后直接继续进入开发会话。

  回答 `n` 就不会下载任何东西。你可以随时运行 `npx extension install chromium`，
  用 `--browser=edge` 或 `--browser=brave` 使用已安装的浏览器，用 `--chromium-binary <path>`
  固定任意二进制，或用 `--no-browser` 只启动开发服务器。非交互式 shell，包括 CI，不会被询问：
  它会打印安装命令并停止。参见
  [为什么 Extension.js 会下载浏览器](/zh-Hans/docs/commands/install#why-extensionjs-downloads-a-browser)
  与 [跳过托管下载](/zh-Hans/docs/commands/install#skip-the-managed-download)。
</Note>
...
- 使用 [模板](/zh-Hans/docs/getting-started/templates) 创建另一个扩展。
- 用 [Playwright 端到端测试](/zh-Hans/docs/workflows/playwright-e2e) 添加自动化检查。
- 随着扩展的成长，回顾 [故障排查](/zh-Hans/docs/workflows/troubleshooting)、[安全清单](/zh-Hans/docs/workflows/security-checklist) 与 [性能手册](/zh-Hans/docs/workflows/performance-playbook)。
