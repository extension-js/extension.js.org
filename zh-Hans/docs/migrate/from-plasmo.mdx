...
description: "分步迁移指南:把基于 Plasmo 的浏览器扩展项目逐步迁移到 Extension.js。将文件约定映射到 manifest.json、转换环境变量、复用 React 组件,并保留现有的内容脚本、后台 service worker、弹窗与选项页面 UI 代码。"
keywords:
  [
    "plasmo migration",
...
在做决定之前，请自行核对这些信号。以下数据来自 2026 年 9 月 14 日的 npm 和 GitHub。

| 信号                | 值                          |
| ------------------- | --------------------------- |
| 最近一次 npm 发布   | 0.90.5，发布于 2025 年 5 月 17 日 |
| `main` 分支最近提交 | 2025 年 5 月 17 日          |
| 未关闭的 issue      | 371                         |
| 最近一个月下载量    | 约 230 万                   |

Plasmo 仍然可以安装和构建。下载量居高不下，是因为现有项目仍然锁定它。它已经十六个月没有发布新版本。如果你的扩展今天能正常工作，你不需要迁移。如果你要开始一个新扩展，或者需要 Node.js 22、最新的 Manifest V3 变化，或一个仍在更新的打包器，本指南大约需要十分钟。
...
如果你使用过 Plasmo CSUI（通过 `getRootContainer` 与 anchor 实现的 content script UI），可以改用常规 DOM 代码挂载你的组件：创建一个容器元素，把它附加到你原先 anchor 的位置，再渲染进去。完整模式（包含使用 shadow DOM 隔离样式）参见 [Content scripts](/zh-Hans/docs/implementation-guide/content-scripts)。

## 第 4 步：环境变量与消息

- 把 `.env` 文件和代码中的 `PLASMO_PUBLIC_*` 重命名为 `EXTENSION_PUBLIC_*`。参见 [环境变量](/zh-Hans/docs/features/environment-variables)。
- `@plasmohq/storage` 可以原样继续使用。
- `@plasmohq/messaging` 依赖 Plasmo 的 `background/messages/*` 构建约定。请用标准的 `chrome.runtime.onMessage` 监听器替换 background 脚本中的处理器。参见 [消息](/zh-Hans/docs/implementation-guide/messaging)。
...
检查 popup、options、content script 与 background 的行为，然后用 `--browser=firefox` 在 Firefox 上做同样的事。调用 `chrome.*` 的代码在 Firefox 上无需改动就能运行，因为 Firefox 原生支持 `chrome.*` 命名空间。`--polyfill` 选项处理的是相反的方向：它在 Chromium 目标上提供 `browser.*`。`extension dev` 默认应用它，`extension build` 除非你传入该选项，否则保持关闭。

## 常见坑

- **`assets/` 中的图标：** Plasmo 会基于 `assets/icon.png` 自动生成各种尺寸的图标。Extension.js 只使用 manifest 中 `"icons"` 声明的内容，自己导出需要的尺寸即可。参见 [图标](/zh-Hans/docs/implementation-guide/icons)。
- **`~` 导入别名：** Plasmo 把 `~` 别名指向项目根。请在 `tsconfig.json` 的 paths 中映射它，或者改用相对导入。参见 [路径解析](/zh-Hans/docs/features/path-resolution)。
- **Storage hooks：** `@plasmohq/storage/hook` 的 `useStorage` 可以保持不变，因为它只依赖 React 和 `chrome.storage`。

## 另请参阅

- [从 WXT 迁移](/zh-Hans/docs/migrate/from-wxt)
