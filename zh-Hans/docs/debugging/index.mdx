...
| `extension open action`                    | [触发工具栏按钮](/zh-Hans/docs/debugging/trigger-actions-and-commands)——会打开它的 popup，或重放 `onClicked` | `--allow-control` |
| `extension open command --name <cmd>`      | [重放某个键盘快捷键命令](/zh-Hans/docs/debugging/trigger-actions-and-commands)                               | `--allow-control` |
| `extension eval "<expr>" --context <c>`    | 在指定上下文中求值一个表达式并返回结果                                                               | `--allow-eval`    |

`extension logs` 与 `extension inspect` 各自有独立的参考页：[logs](/zh-Hans/docs/commands/logs) 与 [inspect](/zh-Hans/docs/commands/inspect)。
...
`extension logs` 把每个上下文合并进同一条终端时间线(见 [logs](/zh-Hans/docs/commands/logs))。当你想看某个上下文在浏览器自己的控制台里的输出时，每个上下文都在不同的门后面：
...
同样这些操作通过 [`@extension.dev/mcp`](/zh-Hans/docs/integrations/chrome-devtools-mcp) 以 MCP 工具的形式暴露出来（`extension_logs`、`extension_eval`、`extension_storage`、`extension_reload`、`extension_open`、`extension_list_extensions`）。门控完全一致——助手可以自由地观察，但只有在你为本次会话启用后才能执行操作。

## 下一步

- [触发动作和键盘命令](/zh-Hans/docs/debugging/trigger-actions-and-commands)——无需点击即可测试处理器，无头并可在 CI 中运行。
- [Manifest 拒绝](/zh-Hans/docs/debugging/manifest-refusals)——为什么 Chromium 会在会话还来不及连接之前就拒绝一个扩展。
- [同时运行两个 MCP 服务器](/zh-Hans/docs/integrations/chrome-devtools-mcp)——Extension.js 控制能力与 Chrome DevTools MCP 并存。
- [CI 模板](/zh-Hans/docs/workflows/ci-templates)——将这些能力接入 pull request 的检查。
