...
它能做到这一点，是因为 Extension.js 在构建阶段就捕获了你的 `chrome.action.onClicked` 与 `chrome.commands.onCommand` 监听器（开发运行时会在你的 background 代码之前加载），并按需重放它们。重放运行在 background 上下文中，也就是你的监听器所在的那个 service worker 或 background 页面。不需要窗口，不需要鼠标，也不会出现偶发失败。
...
<Note>
  需要以 `--allow-control` 启动一个开发会话——参见 [调试概览](/zh-Hans/docs/debugging)。
</Note>
...
可以考虑[把它和 Extension.js 一起使用](/zh-Hans/docs/integrations/chrome-devtools-mcp)：让它负责真实手势的浏览器驱动，而 Extension.js 负责其余所有场景下构建感知、跨浏览器、无头的重放。
...
第一行在后台启动会话。第二行是就绪门控：`--wait`
会等待该会话的[就绪契约](/zh-Hans/docs/contracts/ready-json)然后退出，它自己从不启动任何东西。
把两者放在同一个 `run:` 步骤里，让后台服务器和门控共用一个 shell。
...
完整的流水线请参见 [CI 模板](/zh-Hans/docs/workflows/ci-templates#测试无-ui-的处理器)。
...
- [调试概览](/zh-Hans/docs/debugging)——完整的控制面板。
- [CI 模板](/zh-Hans/docs/workflows/ci-templates)——将其接入 pull request 的检查。
- [Chrome DevTools MCP](/zh-Hans/docs/integrations/chrome-devtools-mcp)：保真度与可移植性并列对比。
