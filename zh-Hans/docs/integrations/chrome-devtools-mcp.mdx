...
[chrome-devtools-mcp](https://github.com/ChromeDevTools/chrome-devtools-mcp) 让 AI agent 能够直接控制一个真实运行的 Chrome 实例，近期版本还加入了扩展相关的工具（`install`、`list`、`reload`、`trigger`、`uninstall`，以及 service-worker 日志）。Extension.js 的 MCP 服务器（`@extension.dev/mcp`）覆盖同样的扩展能力，但具备构建感知，并支持跨浏览器。它有一个专门的页面：[Extension.js MCP 服务器](/zh-Hans/docs/integrations/extension-mcp)。
...
| -------------------------- | ---------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `install_extension`        | `extension_dev`、`extension_start`       | 加载与真实构建 + 热重载绑定，不是加载一个静态文件夹                                                                                                          |
...
参见 [重载与 HMR](/zh-Hans/docs/features/reload-and-hmr)。
...
- 把文档也接入你的助手：[通过 MCP 与 llms.txt 提供 AI 访问](/zh-Hans/docs/ai-access)。
- 把检查接入流水线：[CI 模板](/zh-Hans/docs/workflows/ci-templates)。
- 让同一个扩展运行在所有地方：[跨浏览器兼容性](/zh-Hans/docs/features/cross-browser-compatibility)。
