...
从 4.1.20 起，Safari 构建产出的是一个非持久的 background 页面。Extension.js 会把 `background.service_worker` 条目翻译成 `background: {scripts: [...]}`，和它为 Firefox 做的翻译一样。
...
从 4.1.20 起的 Safari 构建还会对 Safari 缺少的扩展 API 发出警告，这是在你开始调试之前发现问题最省事的办法。详见 [构建 Safari 扩展](/zh-Hans/docs/browsers/safari)。
