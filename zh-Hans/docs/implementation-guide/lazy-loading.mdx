...
- 只有 background 才会做的重型工作（解析、编码、推理）放到 [offscreen 文档](/zh-Hans/docs/implementation-guide/offscreen-documents)里，按需创建、用完即关。worker 保持成一个轻量的消息路由器。
...
对应的 manifest 块见 [web_accessible_resources](/zh-Hans/docs/implementation-guide/web-accessible-resources)。如果不配置它，import 会以网络错误失败，因为宿主页面无法 fetch 那些没有显式暴露的扩展文件。

## 哪些东西干脆不要放进 worker

- 任何跟 DOM 相关的（`DOMParser`、canvas、音频）：worker 根本没有 DOM，请用 offscreen 文档。
- 几兆大的 wasm：在 offscreen 文档或者一个[额外页面](/zh-Hans/docs/features/special-folders)里实例化，并把结果缓存到 `chrome.storage`，这样重启的代价就很低。
- 一次性工具（导入、迁移、导出）：放到 `pages/` 下的一个独立页面里，免得它出现在每条启动路径上。

## 另见

- [Background scripts](/zh-Hans/docs/implementation-guide/background)
- [Offscreen 文档](/zh-Hans/docs/implementation-guide/offscreen-documents)
- [web_accessible_resources](/zh-Hans/docs/implementation-guide/web-accessible-resources)
- [性能实战手册](/zh-Hans/docs/workflows/performance-playbook)
