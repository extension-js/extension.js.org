...
- 你的扩展需要通过 `browser_specific_settings.gecko.id` 声明一个显式 ID。使用 `firefox:` [manifest 前缀](/zh-Hans/docs/features/multi-platform-builds)，让这个键只进入 Firefox 构建。
...
在 Chromium 上有一个配置文件细节需要注意。默认情况下，`extension dev` 通过 `--user-data-dir` 用一个全新的[受管理配置文件](/zh-Hans/docs/browsers/browser-profile)启动浏览器。Chromium 在当前活动的用户数据目录内部解析按用户的 host manifest，所以你为日常使用的 Chrome 安装的 manifest，对那个受管理配置文件是不可见的。从下面两种配置中选一种：
...
- 在 [Messaging](/zh-Hans/docs/implementation-guide/messaging) 中回顾扩展内部的通信通道。
- 在[权限与 host 权限](/zh-Hans/docs/implementation-guide/permissions-and-host-permissions)中理解权限提示。
- 在[浏览器配置文件](/zh-Hans/docs/browsers/browser-profile)中了解开发配置文件的工作方式。
