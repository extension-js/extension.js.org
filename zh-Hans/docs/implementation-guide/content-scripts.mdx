...
当一个 `content_scripts.js` 数组列出多个既没有 `import` 也没有 `export` 的普通 JavaScript 文件时，Extension.js 会把它们拼接成一个 bundle。把每个文件当作独立模块打包会破坏它们。浏览器会按顺序把这些经典文件注入到同一个共享作用域中，拼接正是为了保持这一点：

- 跨文件的隐式全局变量仍然有效。第一个文件中的顶层 `var` 或函数对下一个文件依然可见。
- 每个源文件都会注册为构建依赖，因此保存这一组中的任意文件都会触发重新构建。
- 输出的 source map 指向你真实的文件与行号，因此错误可以追溯回原始代码。
- 拼接组中的原始 `.ts` 文件会在拼接之前用内置的 SWC 剥离类型。

使用 `import` 或 `export` 的入口保持常规的模块打包方式。
...
MAIN world 脚本无法调用扩展 API，因此 Extension.js 会为每个 MAIN 组配一个可以调用的辅助脚本。请预期构建后的 manifest 在这里与你的源文件不同：

- dist 中的 manifest 比源文件包含更多的 `content_scripts` 入口。Extension.js 会在每个 MAIN 组之前插入一条 isolated world 桥接入口。
- 桥接入口的资源索引排在你自己的入口之后。你的 `content_scripts/content-N` 产物保留生成它们的源入口的索引。
- 桥接通过 `runtime.getURL` 解析扩展 URL，并把 MAIN world 的脚本 chunk 注入页面。
- 桥接在处理消息前会校验 `event.source` 是否为同一个 window。
- 它只会注入扩展协议的 URL（`chrome-extension://`、`moz-extension://`）或相对路径，绝不会注入任意的远程 URL。
...
使用 [浏览器特定前缀](/zh-Hans/docs/features/browser-specific-fields) 仅为 Chromium 声明 MAIN world，并为 Firefox 提供 isolated 回退：
...
- 用 [消息通信](/zh-Hans/docs/implementation-guide/messaging) 设计跨上下文通信。
- 在 [权限与主机权限](/zh-Hans/docs/implementation-guide/permissions-and-host-permissions) 中评估访问范围。
- 了解 [web-accessible 资源](/zh-Hans/docs/implementation-guide/web-accessible-resources)。
- 继续阅读 [开发中的 locales](/zh-Hans/docs/implementation-guide/locales)。
