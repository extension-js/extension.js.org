...
**manifest `content_scripts`。** 在 `manifest.json` 中的静态声明。浏览器会把列出的 `js` 与 `css` 文件注入到每个匹配 `matches` 的页面，时机由 `run_at` 决定。Extension.js 会编译每个条目并为 HMR 包装它，参见 [Content script](/zh-Hans/docs/implementation-guide/content-scripts)。
...
Chromium 与 Firefox 都按原样读取这个块，所以它不需要浏览器前缀。只有在 manifest content script 上使用 `world: "MAIN"` 时才用前缀，因为 Firefox 会忽略该字段。把它声明为 `chromium:world` 并保留一个 isolated world 回退，写法见[浏览器特定字段](/zh-Hans/docs/features/browser-specific-fields)。
...
在 Safari 上，仅启用扩展并不够。在你授予网站访问权之前，没有任何 content script 会运行，运行时注入的脚本也一样。启用与授权步骤见 [Safari](/zh-Hans/docs/browsers/safari)。
...
完整的文件夹契约见[特殊文件夹](/zh-Hans/docs/features/special-folders)。

在 `extension dev` 期间，你用 `executeScript` 从 `scripts/` 注入的脚本会在编辑时被重放，因此注入的代码会像声明式 `content_scripts` 一样实时更新。参见[重新加载与 HMR](/zh-Hans/docs/features/reload-and-hmr)。
...
- [权限与 host 权限](/zh-Hans/docs/implementation-guide/permissions-and-host-permissions)
- [特殊文件夹](/zh-Hans/docs/features/special-folders)
- [Web 可访问资源](/zh-Hans/docs/implementation-guide/web-accessible-resources)
