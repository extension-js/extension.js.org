...
- `<base href>` 标签会作用于链接，然后从页面中移除：`<a href>`、`<area href>` 与 `<form action>` 的值会基于它解析并写入输出，而脚本、样式与图片仍然留在扩展内部。
...
| Page action popup | `page_action.default_popup`                                 | Firefox 地址栏弹窗，作为工具栏弹窗旁的独立页面输出 |
...
- 用 [存储](/zh-Hans/docs/implementation-guide/storage) 持久化设置与状态。
- 用 [消息通信](/zh-Hans/docs/implementation-guide/messaging) 协调 UI 页面。
- 进一步了解 [特殊文件夹](/zh-Hans/docs/features/special-folders)。
- 继续阅读 [开发中的 CSS](/zh-Hans/docs/implementation-guide/css)。
