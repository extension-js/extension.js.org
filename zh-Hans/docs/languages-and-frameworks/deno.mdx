...
Extension.js 把 Deno 当作一等公民的运行时，并且每次改动都有一条 CI 流水线在 Deno 上运行 `extension build` 和 `extension dev`。用 Deno 生成脚手架，项目拿到的是 `deno.jsonc` 而不是 `package.json`。在混合项目里，`deno.jsonc` 会与 `package.json` 并存，承载 Deno 原生的配置。无论哪种方式，`extension dev` 和 `extension build` 的行为都是一样的。

## 哪些 Deno 版本可用

Extension.js 需要 Deno 2.5 或更新版本。CLI 在启动时读取 `process.versions.deno`，按 Deno 自己的版本来判断，而不是按 Deno 模拟出来的 Node 版本，所以消息里给出的版本号就是你能去处理的那一个。

| Deno 版本      | 状态                                                                     |
| -------------- | ------------------------------------------------------------------------ |
| 2.5 及更新版本 | 支持，2.8.0 除外                                                         |
| 2.8.0          | 拒绝。该版本无法在打包器内部解析 `node:querystring`，Deno 在 2.8.1 修复了 |
| 早于 2.5       | 拒绝，因为下限是 2.5。作为背景，这些版本模拟的 Node 版本也低于 22.12     |
...
TypeScript 的检测规则与别处完全相同。参见 [TypeScript](/zh-Hans/docs/languages-and-frameworks/typescript)：由 SWC 编译源码，`typescript` 包只在做 `tsc --noEmit` 时才需要。

## 受管理的浏览器需要 Node.js

`extension install <browser>` 通过 `npx`（或启动 CLI 的那个包管理器）下载受管理的浏览器。`deno` 没有这样的运行器，所以在 Deno 上这一条命令需要 `PATH` 里有 Node.js。没有它，命令会在下载步骤失败。`dev`、`build`、`preview` 和 `start` 只靠 Deno 就能运行，系统里已经安装的浏览器也不需要下载。参见 [`install`](/zh-Hans/docs/commands/install)。
