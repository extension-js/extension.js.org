...
发布出去的包声明了 `"engines": {"node": ">=22.12"}`。CLI 在启动时检查运行时：Node 版本低于这个下限会停下来，而只要运行时是 Bun，无论它报告什么版本都会停下来。

这道拒绝针对的是运行时，而不是某个版本号。Bun 是通过 `process.versions.bun` 识别的，这个值由运行时自己设置，所以进入 Bun 的每一条路径都被覆盖：`bunx` 和 `bun run` 上的 `--bun` 标志，以及 `bunfig.toml` 里的 `run.bun` 默认值。强行使用 Bun 运行时会触发这道守卫：
