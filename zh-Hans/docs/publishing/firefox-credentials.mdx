...
附加组件 ID（AMO 称它为 GUID）在每一次提交中标识你的附加组件。它同时也是要填进
`browser_specific_settings.gecko.id` 的那个值。Extension.js 在这个键存在时会读取它，但从不替你写入。
...
只有当扩展确实不传输任何数据时才使用 `["none"]`。这份声明、[STORE.md](/zh-Hans/docs/workflows/store-metadata)
中的隐私章节以及代码三者必须一致。完整规则见
[Firefox 商店就绪](/zh-Hans/docs/features/multi-platform-builds#firefox-store-readiness-data-collection-permissions)。
