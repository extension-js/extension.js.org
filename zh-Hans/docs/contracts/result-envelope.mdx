...
在 `--output json` 下，每个会终止的命令都会在 stdout 上恰好回一个 schema-1 信封。长时间运行的命令则以 [生命周期帧](/zh-Hans/docs/contracts/lifecycle-stream) 的形式流式输出同样的结构。
...
| 字段      | 类型   | 含义                                                                                 |
| --------- | ------ | ------------------------------------------------------------------------------------ |
| `code`    | string | 来自 [错误码表](/zh-Hans/docs/contracts/error-codes) 的稳定 `E_*` 标识符。请匹配这个字段。   |
...
- 用 [错误码表](/zh-Hans/docs/contracts/error-codes) 对失败情况做分支处理。
- 用 [生命周期流](/zh-Hans/docs/contracts/lifecycle-stream) 跟踪长时间运行的会话。
