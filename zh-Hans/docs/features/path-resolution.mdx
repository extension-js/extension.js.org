...
两趟处理都锚定在项目根目录，也就是包含你的 `package.json` 的目录。这就是为什么 `pages/`、`scripts/` 与 `public/` 的输入只从那个目录解析，永远不从 `src/` 解析，即便扩展是由 `src/manifest.json` 定义的。见[特殊文件夹](/zh-Hans/docs/features/special-folders#特殊文件夹放在哪里)中的根目录规则。
