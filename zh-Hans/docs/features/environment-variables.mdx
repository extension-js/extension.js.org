...
在 4.1.19 之前，打包器会把 `import.meta.url` 替换为源文件的 `file://` 路径。这个路径会把构建机器上的文件夹名称带进发布的 bundle。从 4.1.19 起，Extension.js 改为在运行时解析裸的 `import.meta.url`：

| 代码运行的位置                                   | `import.meta.url` 的值                     |
| ------------------------------------------------ | ------------------------------------------ |
| 扩展页面与 worker(service worker、popup、options) | 该页面或 worker 的 URL                     |
