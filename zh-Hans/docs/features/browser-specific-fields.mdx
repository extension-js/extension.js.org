...
Safari 没有实现的键与权限会从 Safari 构建里自动移除，并且从 4.1.20 起，构建会为每个被移除的键打印一行，说明是哪个键、为什么被移除。`content_scripts[].world` 会被保留，因为从 Safari 18 起 Safari 就支持它。详见 [构建 Safari 扩展](/zh-Hans/docs/browsers/safari)。
