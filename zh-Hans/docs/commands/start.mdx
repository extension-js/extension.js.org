...
`start` 没有 Safari 路径。传入 `--browser safari`（或 `webkit-based`）会以 `E_COMMAND_UNSUPPORTED_FOR_TARGET` 退出。Safari 目标请用 [`dev`](/zh-Hans/docs/commands/dev) 或 [`build`](/zh-Hans/docs/commands/build)。
