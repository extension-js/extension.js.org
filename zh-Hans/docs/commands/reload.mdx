...
[`dev`](/zh-Hans/docs/commands/dev) 会话在文件变化时本来就会自动重载。`reload` 面向的是自动重载看不到的场景：你手工改动的状态、卡死的 service worker，或者需要在两次运行之间拿到干净上下文的测试。
...
当活动标签页不是你想要的那个时，用 [`inspect --list-tabs`](/zh-Hans/docs/commands/inspect#the-discovery-loop) 找到数字形式的标签页 id。
...
成功时退出码是 `0`，任何失败都是 `1`。机器消费方应当读取 `--output json` 输出的信封（参见 [结果信封](/zh-Hans/docs/contracts/result-envelope)）。

## 下一步

- 在 [重载与 HMR](/zh-Hans/docs/features/reload-and-hmr) 了解自动重载已经覆盖了哪些情况。
- 用 [`logs`](/zh-Hans/docs/commands/logs) 或 [`inspect`](/zh-Hans/docs/commands/inspect) 确认重载后的上下文是干净的。
- 用 [`doctor`](/zh-Hans/docs/commands/doctor) 诊断拒绝了该调用的会话。
- 在 [调试](/zh-Hans/docs/debugging) 中了解更完整的调试流程。
