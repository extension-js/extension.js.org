...
从 4.1.20 起，Safari 会话的读取方式完全相同。`extension logs --browser safari` 会打印 `dev --browser safari` 会话中 background 与 content 的日志行，加上 `--follow` 就是实时流。详见 [构建 Safari 扩展](/zh-Hans/docs/browsers/safari)。
