...
`logs` 读取 [`dev`](/zh-Hans/docs/commands/dev) 会话从你的扩展里收集到的日志记录：background、content script、popup、options 以及其余上下文。一条命令就能按顺序合并展示全部内容，不用打开任何一个 DevTools 窗口。
...
| `--browser <browser>`             | 读取哪个会话（`chrome`、`chromium`、`edge`、`firefox`、`safari`）。                                      | `chromium`                   |
...
Safari 会话的读取方式完全相同。`extension logs --browser safari` 会打印 `dev --browser safari` 会话中 background 与 content 的日志行，加上 `--follow` 就是实时流。详见 [构建 Safari 扩展](/zh-Hans/docs/browsers/safari)。
...
如果该文件不存在，`logs` 会提示你先启动 `extension dev`，并以退出码 `1` 退出。机器可读格式还会输出一个错误码为 `E_LOGS_NOT_FOUND` 的失败信封（参见 [结果信封](/zh-Hans/docs/contracts/result-envelope)）。
...
- 用 [`doctor`](/zh-Hans/docs/commands/doctor) 诊断 `logs --follow` 连不上的会话。
- 用 [`inspect`](/zh-Hans/docs/commands/inspect) 一次调用就同时拿到 DOM 快照和最近的控制台输出。
- 在 [调试](/zh-Hans/docs/debugging) 中了解更完整的调试工作流。
- 在 [结果信封](/zh-Hans/docs/contracts/result-envelope) 中查看机器可读的失败格式。
