...
JSON 输出是一个 schema-1 信封（见 [结果信封](/zh-Hans/docs/contracts/result-envelope)），它的 `value` 带有五个字段：
...
3. 用动作类命令来驱动它：[`inspect`](/zh-Hans/docs/commands/inspect)、[`eval`](/zh-Hans/docs/commands/eval)、[`storage`](/zh-Hans/docs/commands/storage)、[`reload`](/zh-Hans/docs/commands/reload)。

## 下一步

- 在 [结果信封](/zh-Hans/docs/contracts/result-envelope) 中了解每个支持 json 的命令都会输出的那个信封。
- 用 [`dev`](/zh-Hans/docs/commands/dev) 开启这次握手所准备的会话。
