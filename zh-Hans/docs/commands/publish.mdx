...
- `publish` 从不编译。想让链接指向新鲜的输出时，请先运行 [`build`](/zh-Hans/docs/commands/build)。
...
- 用 [`build`](/zh-Hans/docs/commands/build) 产出要分享的产物。
- 先用 [`preview`](/zh-Hans/docs/commands/preview) 在本地验证这些产物。
