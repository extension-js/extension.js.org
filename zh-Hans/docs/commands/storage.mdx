...
成功时退出码为 `0`，任何失败都是 `1`。机器消费方应当读取 `--output json` 输出的信封（参见 [结果信封](/zh-Hans/docs/contracts/result-envelope)）。

## 下一步

- 用 [`eval`](/zh-Hans/docs/commands/eval) 在同一个会话中执行任意表达式。
- 播下状态之后，用 [`reload`](/zh-Hans/docs/commands/reload) 重启 background worker。
- 用 [`doctor`](/zh-Hans/docs/commands/doctor) 诊断拒绝该调用的会话。
