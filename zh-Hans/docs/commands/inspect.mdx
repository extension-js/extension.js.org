...
- agent 需要标签页 id，以便精确地把 [`eval`](/zh-Hans/docs/commands/eval) 或 `inspect` 调用指向某个目标。
...
超过 `--max-bytes` 的 HTML 返回时会被截断。pretty 模式会在 stderr 上打印一条截断提示，`--output json` 会在信封中设置 `truncated: true`（参见 [结果信封](/zh-Hans/docs/contracts/result-envelope)）。
...
当同一个会话拒绝所有 verb，而你想知道第一个断掉的环节是哪个时，运行 [`doctor`](/zh-Hans/docs/commands/doctor)。

## 下一步

- 用 [`eval`](/zh-Hans/docs/commands/eval) 在你刚刚查看过的页面里执行一段表达式。
- 用 [`logs`](/zh-Hans/docs/commands/logs) 持续流式查看同一个会话的日志。
