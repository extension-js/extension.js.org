...
`preview` 没有 Safari 路径。传入 `--browser safari`（或 `webkit-based`）会以 `E_COMMAND_UNSUPPORTED_FOR_TARGET` 退出。Safari 是受支持的浏览器，但这个命令无法启动它。Safari 目标请使用 [`dev`](/zh-Hans/docs/commands/dev) 或 [`build`](/zh-Hans/docs/commands/build)。
