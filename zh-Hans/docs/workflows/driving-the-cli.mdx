...
本页是写给亲自操作 CLI 的 AI agent 和自动化框架的操作手册。如果你想要的是一个能根据文档回答问题的助手，请看[通过 MCP 与 llms.txt 提供 AI 访问](/zh-Hans/docs/ai-access)。

整个循环分四步：握手、启动、过关、动作。每一步读的都是[机器契约](/zh-Hans/docs/contracts/index)，而不是终端里的文字。
...
想逐帧跟踪整个会话，就在环境中设置 `EXTENSION_OUTPUT=ndjson`，并读取[生命周期流](/zh-Hans/docs/contracts/lifecycle-stream)。
...
然后轮询 `dist/extension-js/chromium/ready.json`，直到 `runtime` 等于 `"attached"` 之后，再执行任何动作动词。关于让你避开过期契约的新鲜度与 pid 存活规则，见 [ready.json](/zh-Hans/docs/contracts/ready-json)。

## 第 4 步：执行动作并读取信封

每个动作动词都接受 `--output json`，并回一个 schema-1 的[结果信封](/zh-Hans/docs/contracts/result-envelope)：
...
请依据 `ok` 和 `error.code` 分支，绝不要依据 `error.message`。[错误码表](/zh-Hans/docs/contracts/error-codes)在各个版本之间保持稳定。
...
- 把契约细节放在手边：[ready.json](/zh-Hans/docs/contracts/ready-json) 与[结果信封](/zh-Hans/docs/contracts/result-envelope)。
- 用 [CI 模板](/zh-Hans/docs/workflows/ci-templates)把同一套循环接进 CI。
