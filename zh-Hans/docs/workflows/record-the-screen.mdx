...
popup 在失去焦点时会关闭，这会中断它的录制。请改为把录制器放在一个专用页面上。把 `recorder.html` 与 `recorder.js` 放进 `pages/` [特殊文件夹](/zh-Hans/docs/features/special-folders)，Extension.js 会把它们作为入口编译。
...
如果你要的是静态图片而不是录像，`chrome.tabs.captureVisibleTab` 会把活动标签页的可见部分作为 data URL 返回。它需要 `activeTab` 权限或该页面的主机权限。

```js
const dataUrl = await chrome.tabs.captureVisibleTab(undefined, { format: "png" })
```

整页截图不是一次调用就能完成。从 content script 分步滚动页面，逐步截取，再在 canvas 上拼接图片。整页截图工具就是这样工作的。
...
- 使用[浏览器特定的 manifest 字段](/zh-Hans/docs/features/browser-specific-fields)，让 `tabCapture` 权限不进入 Firefox 构建。
...
- 在[管理下载](/zh-Hans/docs/workflows/manage-downloads)中管理保存的文件。
- 回顾[特殊文件夹](/zh-Hans/docs/features/special-folders)中 `pages/` 入口的契约。
- 用[安全检查清单](/zh-Hans/docs/workflows/security-checklist)审查捕获权限。
