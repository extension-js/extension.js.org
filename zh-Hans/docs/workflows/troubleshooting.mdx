...
| 修改未生效       | 更新类型(热模块替换 vs 需要重启) | 查阅[开发更新行为](/zh-Hans/docs/workflows/dev-update-behavior)，需要时重启 |
...
完整矩阵请参见[开发更新行为](/zh-Hans/docs/workflows/dev-update-behavior)。
...
- 如果在持续集成 (CI) 容器中浏览器连接缓慢或不稳定，可通过[浏览器传输调优变量](/zh-Hans/docs/features/environment-variables#浏览器传输调优变量) 提高连接超时。
...
把文件移到项目根下的其他文件夹(例如 `bin/`、`tools/` 或 `ci-scripts/`)。详情请参见[特殊文件夹](/zh-Hans/docs/features/special-folders#scripts-中不允许放-nodejs-脚本)。
...
- 运行 `npx extension@latest dev` 使用最新 CLI，或者提升 `package.json` 中 `extension` 依赖的版本。

如果你的 content script 导入了 CSS 文件，或者 manifest 条目声明了 `css`，那么这个文件是真实存在的，必须出现在 `dist/<browser>/content_scripts/` 中。这种情况下文件缺失是构建问题，而不是这个幻影请求。
...
- 查阅[安全检查清单](/zh-Hans/docs/workflows/security-checklist)。
- 查阅[性能手册](/zh-Hans/docs/workflows/performance-playbook)。
