...
常规安装的 Chrome 在 Manifest V3 中无法使用 `webRequestBlocking`。如果你的目标是拦截或改写流量，请改为声明规则。见[构建一个广告拦截器](/zh-Hans/docs/workflows/build-an-ad-blocker)。
...
共享的 API 面见[跨浏览器兼容性](/zh-Hans/docs/features/cross-browser-compatibility)。
...
- 在[构建一个广告拦截器](/zh-Hans/docs/workflows/build-an-ad-blocker)中以声明式拦截流量。
- 在[安全检查清单](/zh-Hans/docs/workflows/security-checklist)中回顾 host 权限卫生。
- 回顾 [Manifest V3 概念](/zh-Hans/docs/concepts/manifest-v3)了解 service worker 的生命周期。
