  ...
  <section className="flex flex-col gap-8 max-w-3xl mx-auto w-full">
    ...
    <AccordionGroup>
      ...
      <Accordion title="Extension.js 支持 Safari 吗？">
        ...
        Extension.js 会用 Xcode 的 `safari-web-extension-converter` 转换你的扩展，
        构建并签名一个 app，然后打开它。你只需要在 Safari 设置中启用一次扩展，之后
        `extension logs` 会输出 background 与 content 的日志行，每次保存都会重载。
        它需要完整的 Xcode，仅限 macOS，而且 `preview` 与 `start` 没有 Safari 路径。详见
