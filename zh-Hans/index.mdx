...
<div className="mx-auto flex w-full max-w-6xl flex-col gap-24 px-6 py-16 sm:py-24">
  ...
  <section className="flex flex-col items-center text-center gap-8">
    ...
    <p className="text-sm text-gray-500 dark:text-gray-400 max-w-xl">
      Extension.js 是给开发者构建扩展用的工具，不是扩展商店，想安装扩展请前往
      <a
    ...
    <Note>
      **首次运行。** `extension dev` 启动的是受管理的 Chrome for Testing 二进制，
      所以第一次运行会要求你先用 `npx extension install chromium` 下载一次。
      想使用已安装的浏览器，请传 `--browser=edge`、`--browser=brave` 或
      `--chromium-binary <path>`，参见 [跳过托管下载](/docs/commands/install#skip-the-managed-download)。
    </Note>
...
{/* ─────────── FAQ ─────────── */}
  ...
  <section className="flex flex-col gap-8 max-w-3xl mx-auto w-full">
    ...
    <AccordionGroup>
      ...
      <Accordion title="如何把现有项目迁移到 Extension.js？">
        ...
        以及更新 `package.json` 的脚本。构建默认项请见
        [扩展配置](/docs/features/extension-configuration)，用一条命令运行你的项目请见
        [立即上手](/docs/getting-started/immediately)。
      </Accordion>
    </AccordionGroup>
  </section>
