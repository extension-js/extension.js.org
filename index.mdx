...
<div className="mx-auto flex w-full max-w-6xl flex-col gap-24 px-6 py-16 sm:py-24">
  ...
  <section className="flex flex-col items-center text-center gap-8">
    ...
    <p className="text-sm text-gray-500 dark:text-gray-400 max-w-xl">
      Extension.js is for developers who build extensions, not a store, so to
      install one visit the{" "}
      <a
    ...
    <Note>
      **First run.** `extension dev` launches a managed Chrome for Testing
      binary, so the first run asks you to fetch it once with
      `npx extension install chromium`. To use a browser that is already
      installed, pass `--browser=edge`, `--browser=brave`, or
      `--chromium-binary <path>`, see
      [skip the managed download](/docs/commands/install#skip-the-managed-download).
    </Note>
