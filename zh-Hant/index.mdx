  ...
  <section className="flex flex-col gap-8 max-w-3xl mx-auto w-full">
    ...
    <AccordionGroup>
      ...
      <Accordion title="Extension.js 支援 Safari 嗎？">
        支援。`--browser=safari` 可用於 macOS 上的 `build` 與 `dev`：Extension.js 會用 Xcode 的 `safari-web-extension-converter` 轉換你的擴充功能，建置並簽章一個 app，然後開啟它。你只需要在 Safari 設定中啟用一次擴充功能，之後 `extension logs` 會串流 background 與 content 的記錄列，而且每次儲存都會重新載入。它需要完整的 Xcode、僅限 macOS，而且 `preview` 與 `start` 沒有 Safari 路徑。請見 [建置 Safari 擴充功能](/zh-Hant/docs/browsers/safari)。
      </Accordion>
