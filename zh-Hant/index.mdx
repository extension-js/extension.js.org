  ...
  <section className="flex flex-col gap-8 max-w-3xl mx-auto w-full">
    ...
    <AccordionGroup>
      ...
      <Accordion title="Extension.js 支援 Safari 嗎？">
        支援，屬於 alpha 目標。`--browser=safari` 可用於 macOS 上的 `build` 與 `dev`：Extension.js 會用 Xcode 的 `safari-web-extension-converter` 轉換你的擴充功能並建置出 Safari app。它需要完整的 Xcode、僅限 macOS，且尚未支援即時重載，因此請視為實驗性功能——Chrome、Edge 與 Firefox 才是穩定目標。安裝步驟與目前限制請見 [建置 Safari 擴充功能](/docs/browsers/safari)。
      </Accordion>
