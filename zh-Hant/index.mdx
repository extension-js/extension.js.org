...
<div className="mx-auto flex w-full max-w-6xl flex-col gap-24 px-6 py-16 sm:py-24">
  ...
  <section className="flex flex-col items-center text-center gap-8">
    ...
    <p className="text-sm text-gray-500 dark:text-gray-400 max-w-xl">
      Extension.js 是給開發者打造擴充功能用的工具，不是擴充功能商店，想安裝擴充功能請前往
      <a
    ...
    <Note>
      **第一次執行。** `extension dev` 啟動的是受管的 Chrome for Testing 執行檔，
      所以第一次執行會要求你先用 `npx extension install chromium` 下載一次。
      想使用已安裝的瀏覽器，請傳 `--browser=edge`、`--browser=brave` 或
      `--chromium-binary <path>`，請見 [略過受管下載](/docs/commands/install#skip-the-managed-download)。
    </Note>
  ...
  <section className="flex flex-col gap-8 max-w-3xl mx-auto w-full">
    ...
    <AccordionGroup>
      ...
      <Accordion title="Extension.js 支援 Safari 嗎？">
        支援。`--browser=safari` 可用於 macOS 上的 `build` 與 `dev`：Extension.js 會用 Xcode 的 `safari-web-extension-converter` 轉換你的擴充功能並建置出 Safari app。它需要完整的 Xcode、僅限 macOS，且尚未支援即時重載。安裝步驟與目前限制請見 [建置 Safari 擴充功能](/docs/browsers/safari)。
      </Accordion>
      ...
      <Accordion title="如何把現有專案遷移到 Extension.js？">
        大多數 React、Vue 與 Svelte 的原始檔可以直接搬過來，不必重寫。工作集中在三個地方：替換打包工具設定、把產生的 manifest 還原為手寫且支援 [瀏覽器前綴鍵](/docs/features/browser-specific-fields) 的 `manifest.json`，以及更新 `package.json` 的 scripts。建置預設值請見 [擴充功能設定](/docs/features/extension-configuration)，用一條命令執行你的專案請見 [立刻開始](/docs/getting-started/immediately)。
      </Accordion>
    </AccordionGroup>
  </section>
