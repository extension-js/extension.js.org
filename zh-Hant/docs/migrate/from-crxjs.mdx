...
<Note>
  ...
  嗎?在遷移前,請先參考[專門的修正頁](/docs/migrate/crxjs-content-script-filename-undefined)的解法。
</Note>
...
- **帶 hash 的資產路徑:** [`dist/<browser>`](/docs/features/path-resolution) 下的正式建置使用乾淨的檔名。`extension dev` 會為 content script 附加一段建置 hash,例如 `content_scripts/content-0.813e908a.js`,用來繞過擴充功能的資源快取。參見 [重新載入與 HMR](/docs/features/reload-and-hmr)。如果你的程式碼硬寫了 Vite 風格的 hash 檔名,請改用 manifest 相對路徑。
