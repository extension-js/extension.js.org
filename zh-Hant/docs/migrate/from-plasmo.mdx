...
在做決定之前，請自行核對這些訊號。以下資料來自 2026 年 9 月 14 日的 npm 和 GitHub。

| 訊號                | 值                          |
| ------------------- | --------------------------- |
| 最近一次 npm 發布   | 0.90.5，發布於 2025 年 5 月 17 日 |
| `main` 分支最近提交 | 2025 年 5 月 17 日          |
| 未關閉的 issue      | 371                         |
| 最近一個月下載量    | 約 230 萬                   |

Plasmo 仍然可以安裝和建置。下載量居高不下，是因為既有專案仍然鎖定它。它已經十六個月沒有發布新版本。如果你的擴充功能今天能正常運作，你不需要遷移。如果你要開始一個新的擴充功能，或者需要 Node.js 22、最新的 Manifest V3 變更，或一個仍在更新的打包器，本指南大約需要十分鐘。
...
如果你用過 Plasmo 的 CSUI（具備 `getRootContainer` 與 anchor 的 content script UI），請改用一般的 DOM 程式碼掛載元件：建立一個容器元素，將它附加到你原本錨定的位置，然後渲染到其中。完整模式請見 [Content scripts](/zh-Hant/docs/implementation-guide/content-scripts)，包括以 shadow DOM 隔離樣式的方式。

## 步驟 4：環境變數與訊息傳遞

- 將 `.env` 檔案與程式碼中的 `PLASMO_PUBLIC_*` 改名為 `EXTENSION_PUBLIC_*`。參見[環境變數](/zh-Hant/docs/features/environment-variables)。
- `@plasmohq/storage` 可以原封不動繼續使用。
- `@plasmohq/messaging` 依賴 Plasmo 的 `background/messages/*` 建置慣例。請以標準的 `chrome.runtime.onMessage` 監聽器在 background script 中取代處理函式。參見[訊息傳遞](/zh-Hant/docs/implementation-guide/messaging)。
...
檢查 popup、options、content scripts 與 background 的行為，再以 `--browser=firefox` 在 Firefox 上跑相同的驗證。呼叫 `chrome.*` 的程式碼在 Firefox 上不用修改就能執行，因為 Firefox 原生支援 `chrome.*` 命名空間。`--polyfill` 處理的是相反的方向：它在 Chromium 目標上提供 `browser.*`。`extension dev` 預設會套用它，`extension build` 則除非你加上這個 flag，否則保持關閉。

## 常見陷阱

- **`assets/` 中的圖示：** Plasmo 會從 `assets/icon.png` 自動產生各種尺寸的圖示。Extension.js 使用 manifest 中 `"icons"` 所宣告的內容，請自行匯出所需的尺寸。參見 [Icons](/zh-Hant/docs/implementation-guide/icons)。
- **`~` import 別名：** Plasmo 將 `~` 別名指向專案根目錄。請在 `tsconfig.json` 的 paths 中設定它，或改用相對路徑 import。參見 [Path resolution](/zh-Hant/docs/features/path-resolution)。
- **Storage hooks：** `@plasmohq/storage/hook` 的 `useStorage` 可原封不動使用，因為它只依賴 React 與 `chrome.storage`。

## 延伸閱讀

- [從 WXT 遷移](/zh-Hant/docs/migrate/from-wxt)
