...
- 透過 [ready.json 與事件串流](/zh-Hant/docs/workflows/playwright-e2e) 解析工作階段狀態，而不是解析終端機輸出。
- 在 [瀏覽器設定檔](/zh-Hant/docs/browsers/browser-profile) 中了解受管理設定檔的行為。
- 在 [讀懂 CLI 輸出](/zh-Hant/docs/concepts/reading-cli-output) 中了解工作階段在終端機這一側的樣子。
