---
title: "Chrome、Firefox 與 Edge 的瀏覽器啟動旗標"
description: "在 Extension.js 中使用 firefox://flags、chrome://flags 與瀏覽器啟動旗標。在擴充功能開發期間控制 Chrome、Firefox 與 Edge 的執行階段行為。"
keywords:
  [
    "firefox flags",
    "firefox://flags",
    "browser://flags",
    "chrome://flags",
    "extension.js",
    "chrome",
    "firefox",
    "edge",
    "browser",
    "devtools",
    "docs",
    "browsers",
    "allow control",
    "no sandbox",
    "webgl",
    "gpu",
    "webgpu",
  ]
---

控制瀏覽器啟動行為,用於除錯、自動化與執行階段的實驗。

<Note>
  **在找 `chrome://flags` 或 `edge://flags`?** 直接在網址列輸入即可,那是瀏覽器內建的頁面。**Firefox 沒有
  `firefox://flags`**,請改用 `about:config`。

如果你切換旗標是為了**測試或建置瀏覽器擴充功能**,Extension.js 會在 Chrome、Edge 與 Firefox
上以專案為單位自動套用,你不必每次執行都手動傳入旗標。[30 秒建立你的第一個擴充功能 →](/zh-Hant/docs/getting-started)

</Note>

在不修改擴充功能原始碼的情況下調整瀏覽器啟動行為。Extension.js 會合併你在 `extension.config.*` 中設定的瀏覽器旗標,並在 `dev`、`preview` 與 `start` 流程中套用。

## Firefox 有 `firefox://flags` 嗎?

Firefox 不像 Chromium 系列瀏覽器使用 `chrome://flags` 或 `edge://flags` 那樣使用 `firefox://flags`。Firefox 透過 `about:config`(偏好設定)公開執行階段的開關,並接受從命令列傳入的啟動旗標。

如果你搜尋過 `firefox://flags`、`firefox //flags`、`browser://flags`、`mozilla://flags` 或 `about flags firefox`,你很可能是想在擴充功能開發期間改變瀏覽器行為。在 Extension.js 中,你會從兩個地方著手:

- 使用 [`browserFlags`](/zh-Hant/docs/browsers/browser-flags) 來傳遞 Extension.js 啟動瀏覽器執行檔時所用的啟動旗標。
- 使用 [Firefox 偏好設定](/zh-Hant/docs/browsers/browser-preferences) 來設定原本會放在 `about:config` 中、可重複套用的 Gecko 執行階段行為。

從同一份 Extension.js 專案,這兩種方式都能用於 Chrome、Edge 與 Firefox 的擴充功能開發。

## 其他瀏覽器的旗標

Extension.js 會以同樣的方式啟動並設定任何以 Chromium 為基礎的瀏覽器。

- **`brave://flags`、`opera://flags`、`vivaldi://flags`、`yandex://flags`**:它們都以
  Chromium 為基礎,行為與 `chrome://flags` 完全相同,Extension.js 會以專案為單位管理它們。
- **`internet://flags` 與 `browser://flags`**:這些不是真實存在的瀏覽器 scheme。你要找的多半是
  `chrome://flags`(Chromium)或 `about:config`(Firefox)。
- **`edge://flags`**:Edge 就是 Chromium,所以在 Extension.js 中的表現和 Chrome 一模一樣。

正在建置的擴充功能需要特定的啟動旗標?在 `extension.config.*` 裡設定一次,Extension.js
每次執行都會套用。請參閱下方的 [`browserFlags`](/zh-Hant/docs/browsers/browser-flags)。

## GPU 與 WebGL 旗標

無頭工作階段、容器或虛擬機通常沒有 GPU。此時 WebGL 內容無法建立，需要它的 canvas 程式碼會拋出錯誤。透過 `browserFlags` 傳入 Chromium 開關：

```js
export default {
  browser: {
    chrome: {
      browserFlags: [
        "--ignore-gpu-blocklist",
        "--use-gl=angle",
        "--use-angle=swiftshader",
      ],
    },
  },
};
```

`--ignore-gpu-blocklist` 讓 Chromium 使用它的封鎖清單原本會拒絕的 GPU。`--use-gl=angle` 搭配 `--use-angle=swiftshader` 以軟體渲染 WebGL，這是持續整合中的正確選擇。`--enable-unsafe-webgpu` 在瀏覽器仍然限制 WebGPU 的地方開啟它。Firefox 沒有等價的開關，請改為透過瀏覽器偏好設定 `webgl.force-enabled`。

## 範本範例

### `new-browser-flags`

![new-browser-flags template screenshot](/images/examples/newtab-browser-flags/screenshot.png)

透過一個會設定啟動行為的 new-tab 擴充功能,實際看看瀏覽器旗標的運作方式。

<CodeGroup>

```bash npm
npx extension@latest create my-extension --template=newtab-browser-flags
```

```bash pnpm
pnpx extension@latest create my-extension --template=newtab-browser-flags
```

```bash yarn
yarn dlx extension@latest create my-extension --template=newtab-browser-flags
```

```bash bun
bunx extension@latest create my-extension --template=newtab-browser-flags
```

```bash deno
deno run -A npm:extension@latest create my-extension --template=newtab-browser-flags
```

</CodeGroup>

儲存庫:[extension-js/examples/newtab-browser-flags](https://github.com/extension-js/examples/tree/main/examples/newtab-browser-flags)

## 運作方式

在 `extension.config.*` 中設定旗標:

- `browser.<target>.browserFlags`
- `commands.dev|start|preview.browserFlags`
- 可選的 `excludeBrowserFlags`,可移除預設旗標或使用者旗標(行為會因目標是 Chromium 或 Firefox 而不同)。

覆寫順序:瀏覽器預設 → 指令預設 → CLI 所選的指令情境。

## 旗標功能

| 設定鍵                                 | 用途                                   |
| -------------------------------------- | -------------------------------------- |
| `browser.<target>.browserFlags`        | 為特定瀏覽器目標設定預設啟動旗標。     |
| `commands.dev.browserFlags`            | 為 `dev` 執行新增或覆寫旗標。          |
| `commands.start.browserFlags`          | 為 `start` 執行新增或覆寫旗標。        |
| `commands.preview.browserFlags`        | 為 `preview` 執行新增或覆寫旗標。      |
| `browser.<target>.excludeBrowserFlags` | 為某個目標移除符合的預設或使用者旗標。 |
| `commands.<name>.excludeBrowserFlags`  | 在特定指令情境中移除旗標。             |

### 設定範例

```js
export default {
  browser: {
    chrome: {
      browserFlags: ["--disable-web-security", "--auto-open-devtools-for-tabs"],
      excludeBrowserFlags: ["--mute-audio"],
    },
    firefox: {
      browserFlags: ["--devtools", "--new-instance"],
      excludeBrowserFlags: ["--devtools"],
    },
  },
};
```

## Chromium 與 Firefox 的行為差異

- **Chromium 系列(`chrome`、`edge`、`chromium`、`chromium-based`)**
  - 從內建的預設旗標集開始,接著附加你的 `browserFlags`。
  - `excludeBrowserFlags` 會移除符合的預設旗標與使用者旗標。
  - Extension.js 自行管理 `--load-extension=...`,並會從使用者提供的旗標中過濾掉它。

- **Firefox/Gecko 系列(`firefox`、`gecko-based` / `firefox-based`)**
  - 使用使用者提供的 `browserFlags`(沒有 Chromium 那種龐大的預設旗標組)。
  - `excludeBrowserFlags` 會移除符合的使用者旗標,規則與 Chromium 相同。

兩個系列共用同一條排除規則:當某個旗標與排除值完全相等,或排除值指的那個開關後面接著 `=` 或 `,` 時,該旗標就會被移除。所以 `--disable-features` 也會移除 `--disable-features=Translate`,而 `--dev` 永遠不會移除 `--devtools`。

### Chromium 的預設旗標

Extension.js 在啟動 Chromium 系列瀏覽器時會自動套用以下旗標。若有不需要的,使用 `excludeBrowserFlags` 移除即可。

| 旗標                                                       | 用途                                                                  |
| ---------------------------------------------------------- | --------------------------------------------------------------------- |
| `--no-first-run`                                           | 停用首次執行體驗                                                      |
| `--disable-client-side-phishing-detection`                 | 停用釣魚偵測                                                          |
| `--disable-sync`                                           | 停用同步以避免帳號提示                                                |
| `--disable-component-extensions-with-background-pages`     | 停用不會被 `--disable-extensions` 影響的內建擴充功能                  |
| `--disable-default-apps`                                   | 停用預設應用程式的安裝                                                |
| `--disable-features=InterestFeedContentSuggestions`        | 在新分頁(NTP)上停用 Discover 資訊流                                   |
| `--disable-features=Translate`                             | 停用 Chrome 翻譯                                                      |
| `--hide-scrollbars`                                        | 在截圖中隱藏捲軸                                                      |
| `--mute-audio`                                             | 將任何音訊靜音                                                        |
| `--no-default-browser-check`                               | 停用預設瀏覽器檢查提示                                                |
| `--ash-no-nudges`                                          | 避免使用者教學提示                                                    |
| `--disable-search-engine-choice-screen`                    | 停用搜尋引擎選擇畫面                                                  |
| `--disable-features=MediaRoute`                            | 停用 Chrome Media Router 的背景網路服務                               |
| `--use-mock-keychain`                                      | 在 Mac 上使用模擬鑰匙圈,避免阻擋對話框                                |
| `--disable-background-networking`                          | 停用背景網路服務                                                      |
| `--disable-breakpad`                                       | 停用當機傾印收集                                                      |
| `--disable-component-update`                               | 停用元件更新                                                          |
| `--disable-domain-reliability`                             | 停用網域可靠性監測                                                    |
| `--no-pings`                                               | 停用超連結稽核 ping                                                   |
| `--enable-features=SidePanelUpdates`                       | 確保側邊面板可見                                                      |
| `--disable-features=DisableLoadExtensionCommandLineSwitch` | 允許在命令列使用 `--load-extension`                                   |
| `--disable-features=ExtensionDisableUnsupportedDeveloper`  | 讓未封裝的擴充功能在 `runtime.reload()` 之後仍存活(Chromium 152 以上) |
| `--enable-unsafe-extension-debugging`                      | 允許透過 Chrome DevTools Protocol (CDP) 管理擴充功能(Chrome 126 以上) |
| `--silent-debugger-extension-api`                          | 隱藏「X is debugging this browser」的資訊列                           |

### 一律生效的工作階段旗標

除了上面的預設表格,每一次 Chromium 啟動都會帶上一組固定的工作階段穩定性旗標。它們能避免背景計時器、被遮蔽的視窗與轉譯程序節流扭曲自動化與 AI 驅動的工作階段。`excludeBrowserFlags` 無法移除它們:

```text
--disable-background-timer-throttling
--disable-renderer-backgrounding
--disable-backgrounding-occluded-windows
--disable-features=TranslateUI
--disable-hang-monitor
--disable-prompt-on-repost
--memory-pressure-off
--max_old_space_size=4096
--disable-dev-shm-usage
```

只有在 `dev` 模式下,Extension.js 才會用三個旗標接上除錯通道:`--remote-debugging-port=<derived port>`、`--remote-debugging-address=127.0.0.1` 與 `--remote-debugging-pipe`。正式模式的啟動(`start`、`preview`)會略過這三個。

### feature 開關的合併

Chromium 只認重複開關中最後出現的那一個。因此 Extension.js 會把每一處 `--enable-features` 與 `--disable-features` 各自合併成一個以逗號連接的開關。預設值、設定與環境變數的值都會一起合併,重複的 feature 名稱會被移除。你可以放心在 `browserFlags` 傳入 `--disable-features=Foo`,而不會抹掉預設值。

### 環境變數開關

- `EXTENSION_BROWSER_FLAGS` 會把以空白分隔的旗標附加到每一次啟動。它在設定旗標之後、也在 `excludeBrowserFlags` 過濾之後才套用,因此 `excludeBrowserFlags` 無法移除來自這個變數的旗標。
- `EXTENSION_HEADLESS=1`(或 `true`)會強制使用 `--headless=new`,除非設定或環境旗標中已經傳入某種 `--headless` 變體。可以把它當成自動化工作階段的搶焦點防護。

<Note>
  在 Docker、持續整合(CI)或容器化環境中,Extension.js 也會自動套用 `--no-sandbox`
  與 `--disable-setuid-sandbox`。在 Linux 上,它會透過下列任一訊號判斷處於容器中:
  `CI=true`、存在 `/.dockerenv` 檔案、存在 `/run/.containerenv` 檔案、
  `REMOTE_CONTAINERS=true`、`CODESPACES=true`,或設定了 `container` 變數。
</Note>

## 支援的目標與參考

| 瀏覽器         | 用法                                     | 更多資訊                                                                               |
| -------------- | ---------------------------------------- | -------------------------------------------------------------------------------------- |
| Chrome         | `extension dev --browser=chrome`         | [Chrome Flags](https://peter.sh/experiments/chromium-command-line-switches/)           |
| Edge           | `extension dev --browser=edge`           | [Edge Flags](https://docs.microsoft.com/en-us/deployedge/microsoft-edge-policies)      |
| Firefox        | `extension dev --browser=firefox`        | [Firefox Flags](https://developer.mozilla.org/en-US/docs/Mozilla/Command_Line_Options) |
| Chromium-based | `extension dev --browser=chromium-based` | [Chromium Flags](https://peter.sh/experiments/chromium-command-line-switches/)         |
| Gecko-based    | `extension dev --browser=gecko-based`    | 以 Firefox 為基礎的瀏覽器共用與 Firefox 相同的旗標。                                   |

在需要時對引擎類型的目標傳入執行檔旗標:

- `--chromium-binary=...`
- `--gecko-binary=...`

## 最佳實務

- **只加入必要的旗標**:減少冗長的旗標清單,降低不穩定或不可攜的設定。
- **優先使用 `excludeBrowserFlags`,而不是整個取代預設**:只移除會與你工作流程衝突的部分。
- **不要手動傳 `--load-extension`**:Extension.js 會在內部管理擴充功能載入相關旗標。
- **針對每個瀏覽器家族驗證**:在 Chromium 上有效的旗標,在 Firefox 上可能無效或被忽略。

## 後續步驟

- 進一步了解 [瀏覽器偏好設定](/zh-Hant/docs/browsers/browser-preferences)。
- 進一步了解 [瀏覽器設定檔](/zh-Hant/docs/browsers/browser-profile)。
- 在 [可用的瀏覽器](/zh-Hant/docs/browsers/browsers-available) 中選擇目標。
- 透過 [`extension.config.*`](/zh-Hant/docs/features/extension-configuration) 串接旗標。
- 第一次接觸擴充功能工具鏈?先從 [瀏覽器擴充功能框架](/zh-Hant/docs/compare) 總覽開始。
