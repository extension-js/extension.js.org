...
`extension dev --browser safari` 會打包你的程式碼、以 Apple 的 `safari-web-extension-converter` 轉換它，再用 `xcodebuild` 建置並簽章一個 app，然後開啟它。你只需要在 Safari 設定中啟用擴充功能一次。之後 `extension logs` 會串流 background 與 content 的記錄列，控制通道會接上，而且每次儲存都會在 Safari 中重新載入擴充功能。這個 dev 迴圈從 4.1.20 起提供。
...
`dist/safari-xcode/.manifest-fingerprint` 判定。從 4.1.20 起寫入的 v4
...
從 4.1.20 起，一個 Safari dev 工作階段對 [`logs`](/zh-Hant/docs/commands/logs) 來說就是一般的工作階段：
...
Safari 沒有實作的 manifest 鍵與權限，會從 Safari 建置中自動移除。從 4.1.20 起，建置會為每個被移除的鍵印出一行，說明是哪個鍵、以及為什麼被移除。沒有任何東西被靜默丟棄，你的來源 `manifest.json` 也不會被更動。
