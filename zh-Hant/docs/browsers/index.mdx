...
你也可以同時執行兩個 dev 工作階段,每個瀏覽器一個終端機。工作階段狀態(ready 契約、控制通道、記錄、設定檔)都以瀏覽器為鍵,所以 Chrome 工作階段與 Firefox 工作階段永遠不會互相衝突。磁碟上的配置請參見 [工作階段產物](/docs/concepts/session-artifacts)。
...
| 在 WSL 下開發           | [WSL 支援](/docs/browsers/wsl)                          |
