...
| `schema`           | `1`            | 表明這個引擎講的是 schema-1 的[結果信封](/zh-Hant/docs/contracts/result-envelope)。      |
...
| `runtime`            | `"attached"` 或 `"detached"` | service worker 連上且可被驅動後為 `attached`。從 4.1.20 起,最後一個已連線的擴充功能情境斷線後為 `detached`,重新連線後又回到 `attached`。 |
| `executorAttachedAt` | ISO string     | service worker 第一次連上的時間。斷線後會作為來源記錄保留。 |
| `executorDetachedAt` | ISO string     | 從 4.1.20 起,最後一個已連線的擴充功能情境斷線的時間。 |
...
- 用[結果信封](/zh-Hant/docs/contracts/result-envelope)讀取指令結果。
- 用[生命週期串流](/zh-Hant/docs/contracts/lifecycle-stream)把同一個工作階段當成影格來串流。
- 用 [Playwright E2E](/zh-Hant/docs/workflows/playwright-e2e) 把整套流程接進測試。
