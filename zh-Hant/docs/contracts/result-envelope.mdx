...
在 `--output json` 之下，每個會結束的指令都會在 stdout 上剛好回一個 schema-1 信封。長時間執行的指令則以 [生命週期影格](/zh-Hant/docs/contracts/lifecycle-stream) 的形式串流出同樣的結構。
...
| 欄位      | 型別   | 意義                                                                             |
| --------- | ------ | -------------------------------------------------------------------------------- |
| `code`    | string | 來自 [錯誤碼表](/zh-Hant/docs/contracts/error-codes) 的穩定 `E_*` 識別碼。請比對這個欄位。 |
...
- 用 [錯誤碼表](/zh-Hant/docs/contracts/error-codes) 針對失敗做分支處理。
- 用 [生命週期串流](/zh-Hant/docs/contracts/lifecycle-stream) 追蹤長時間執行的工作階段。
