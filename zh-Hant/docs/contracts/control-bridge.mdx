...
dev 伺服器把這個通道掛在 [ready.json](/zh-Hant/docs/contracts/ready-json) 所公布的 `controlPort` 上的 `/extjs-control` 路徑。用戶端以信封版本 1 加上一個角色打招呼，角色為 `producer`、`consumer` 或 `controller`。

從 4.1.19 起，伺服器會拒絕來自網頁的交握。`Origin` 標頭為 `http://`、`https://` 或 `null` 時，會在任何訊框之前得到 HTTP 401。擴充功能的 background 送出的是它自己的 scheme，例如 `chrome-extension://` 或 `moz-extension://`。Node.js 用戶端不送出 `Origin`。這兩類用戶端都照常連線。
...
| `surface_reply_failed` | 目標介面已開啟並執行了這個 op，但它的回覆沒有送達。訊息內引用引擎的原文。 |
| `api_unavailable`     | 這個 op 背後的瀏覽器 API 在此不可用。    |

匯出的常數為 `REFUSAL_NEEDS_HEADED_WINDOW`、`REFUSAL_NEEDS_USER_GESTURE`、`REFUSAL_SURFACE_NOT_OPEN`、`REFUSAL_SURFACE_REPLY_FAILED` 與 `REFUSAL_API_UNAVAILABLE`。
...
- 只要 CLI 指令足夠就優先使用它們，從 [ready.json](/zh-Hant/docs/contracts/ready-json) 開始。
- 用 [錯誤碼](/zh-Hant/docs/contracts/error-codes) 把橋接失敗對應到信封中的錯誤碼。
