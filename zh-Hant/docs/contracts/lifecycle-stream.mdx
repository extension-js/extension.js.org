...
一個終止信封無法描述整個工作階段，因此 `dev`、`start` 與 `preview` 會為每一次生命週期轉換串流輸出一個 schema-1 影格。每個影格本身就是獨立成行的一個完整[結果信封](/zh-Hant/docs/contracts/result-envelope)。
...
<Frame>
  <iframe
    ...
    src="https://www.youtube-nocookie.com/embed/YvQWuq387ak?rel=0"
...
- 在 [ready.json](/zh-Hant/docs/contracts/ready-json) 中閱讀這些影格背後的契約。
- 在 [用 agent 驅動 CLI](/zh-Hant/docs/workflows/driving-the-cli) 中了解如何從 agent 驅動整個迴圈。
