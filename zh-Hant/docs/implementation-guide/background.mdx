...
從 4.1.20 起，Safari 建置產出的是非持續性的 background 頁面。Extension.js 會把 `background.service_worker` 條目翻譯成 `background: {scripts: [...]}`，和它為 Firefox 所做的翻譯一樣。
...
從 4.1.20 起的 Safari 建置也會針對 Safari 缺少的擴充功能 API 發出警告，這是在你開始除錯之前找出問題最省事的方法。請見 [建置 Safari 擴充功能](/zh-Hant/docs/browsers/safari)。
