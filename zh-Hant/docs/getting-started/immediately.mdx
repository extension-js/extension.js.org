...
<Frame>
  <iframe
    ...
    src="https://www.youtube-nocookie.com/embed/ApD0gRolb9U?rel=0"
...
<Frame>
  <iframe
    className="w-full aspect-video rounded-xl"
    src="https://www.youtube-nocookie.com/embed/O8cFJ-fA4K4?rel=0"
...
<Frame>
  <iframe
    className="w-full aspect-video rounded-xl"
    src="https://www.youtube-nocookie.com/embed/oXH5Z3N6uec?rel=0"
...
此例使用來自 MDN WebExtensions Examples 的 [Beastify](https://github.com/mdn/webextensions-examples/tree/main/beastify)。它是 Manifest V3 範例，會呼叫 Firefox 風格的 `browser.*` API，而這正是 polyfill 負責轉換的部分。Manifest V2 範例（例如 Apply CSS）無法在 Edge 中執行，因為現代 Chromium 拒絕載入 MV2：請改用 `--browser=firefox` 執行這類範例。
...
- 若你需要處理 manifest 中未宣告的資產，可進一步瞭解 [Special folders](/zh-Hant/docs/features/special-folders)。
...
- 繼續閱讀[建立你的第一個擴充功能](/zh-Hant/docs/getting-started/create-your-first-extension)。
- 為你偏好的技術棧瀏覽起始[範本](/zh-Hant/docs/getting-started/templates)。
- 透過 [dev 指令選項](/zh-Hant/docs/commands/dev)鎖定特定瀏覽器與執行檔。
