...
- [驅動 CLI](/docs/workflows/driving-the-cli):給執行 `extension` 指令的 agent 的實戰手冊。內容涵蓋 `capabilities` 交握、機器輸出、執行階段 attached 閘門,以及各個 act 動詞。
- [機器契約](/docs/contracts/index):這些 agent 會解析的穩定結構。包含 `ready.json`、schema-1 結果信封、錯誤碼表、生命週期影格串流,以及控制橋接。
