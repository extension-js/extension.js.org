...
這之所以可行，是因為 Extension.js 會在建置時擷取你的 `chrome.action.onClicked` 與 `chrome.commands.onCommand` 監聽器（開發期執行階段會在你的 background 程式碼之前載入），並在需要時重播它們。重播執行在 background 情境中，也就是你的監聽器所在的那個 service worker 或 background 頁面。沒有視窗、沒有滑鼠、沒有 flaky 測試。
...
<Note>
  需要以 `--allow-control` 啟動的開發 session —
  參見[除錯總覽](/zh-Hant/docs/debugging)。
</Note>
...
可以考慮[與 Extension.js 並行使用](/zh-Hant/docs/integrations/chrome-devtools-mcp)：讓它負責真正帶手勢的瀏覽器驅動，Extension.js 則負責你日常會用到的、感知建置、跨瀏覽器、無頭的重播。
...
第一行會在背景啟動 session。第二行是就緒閘門：`--wait`
會等待該 session 的[就緒合約](/zh-Hant/docs/contracts/ready-json)後結束，它自己從不啟動任何東西。
把兩者放在同一個 `run:` 步驟裡，讓背景執行的伺服器與閘門共用一個 shell。

由於觸發是可重現且無頭的，它是個可靠的檢核點 — Extension.js 自家的 `smoke:open-action` 檢查正是走相同路徑，它會觸發動作 _以及_ 一個指令，再讀回 `chrome.storage` 來證明每個處理函式確實有執行。

完整 pipeline 請見 [CI 範本](/zh-Hant/docs/workflows/ci-templates#不需-ui-的事件處理測試)。
...
- [除錯總覽](/zh-Hant/docs/debugging) — 完整的控制介面。
- [CI 範本](/zh-Hant/docs/workflows/ci-templates) — 將此整合進 PR 檢核流程。
- [Chrome DevTools MCP](/zh-Hant/docs/integrations/chrome-devtools-mcp)：並列比較還原度與可攜性。
