...
`start` 沒有 Safari 路徑。傳入 `--browser safari`(或 `webkit-based`)會以 `E_COMMAND_UNSUPPORTED_FOR_TARGET` 結束。Safari 目標請改用 [`dev`](/zh-Hant/docs/commands/dev) 或 [`build`](/zh-Hant/docs/commands/build)。
