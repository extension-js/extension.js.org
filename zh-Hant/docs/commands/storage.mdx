...
成功時退出碼為 `0`，任何失敗都是 `1`。機器端的使用者應該讀取 `--output json` 輸出的信封（參見 [結果信封](/zh-Hant/docs/contracts/result-envelope)）。

## 後續步驟

- 用 [`eval`](/zh-Hant/docs/commands/eval) 在同一個工作階段執行任意運算式。
- 準備好狀態之後，用 [`reload`](/zh-Hant/docs/commands/reload) 重新啟動 background worker。
- 用 [`doctor`](/zh-Hant/docs/commands/doctor) 診斷拒絕該呼叫的工作階段。
- 在 [除錯](/zh-Hant/docs/debugging) 了解更完整的除錯工作流程。
