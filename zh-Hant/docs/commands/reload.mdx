...
[`dev`](/zh-Hant/docs/commands/dev) 工作階段在檔案變更時本來就會自動重新載入。`reload` 針對的是自動重新載入看不到的情況：你手動改動的狀態、卡住的 service worker，或是需要在兩次執行之間取得乾淨情境的測試。
...
當作用中分頁不是你要的那一個時，用 [`inspect --list-tabs`](/zh-Hant/docs/commands/inspect#the-discovery-loop) 找出數字形式的分頁 id。
...
成功時結束碼是 `0`，任何失敗都是 `1`。機器消費端應該讀取 `--output json` 輸出的信封（參見 [結果信封](/zh-Hant/docs/contracts/result-envelope)）。

## 後續步驟

- 在 [重新載入與 HMR](/zh-Hant/docs/features/reload-and-hmr) 了解自動重新載入已經涵蓋了哪些情況。
- 用 [`logs`](/zh-Hant/docs/commands/logs) 或 [`inspect`](/zh-Hant/docs/commands/inspect) 確認重新載入後的情境是乾淨的。
- 用 [`doctor`](/zh-Hant/docs/commands/doctor) 診斷拒絕這次呼叫的工作階段。
- 在 [除錯](/zh-Hant/docs/debugging) 中閱讀更完整的除錯流程。
