...
| `--install [boolean]`          | -                  | 當缺少時安裝專案相依套件。基於安全預設停用生命週期指令稿;設 `EXTENSION_ALLOW_INSTALL_SCRIPTS=true` 可執行它們。    | 依指令行為而定          |
| `--parent-pid <pid>`           | -                  | 指定的處理程序 ID 一旦死亡就立即結束 dev 伺服器。供衍生 `dev` 的 harness / agent 使用,宿主當掉不會外洩伺服器。     | 未設                    |
