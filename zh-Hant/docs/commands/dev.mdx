...
如果你的擴充功能放在 monorepo/submodule 中,請了解 `extension.config.*` 如何載入環境檔案(包含 workspace 根目錄的後備):[環境變數](/zh-Hant/docs/features/environment-variables#how-it-works)。
...
| `--chromium-binary <path>`     | -                  | [自訂 Chromium 系列執行檔路徑](/zh-Hant/docs/browsers/running-other-browsers)。                                            | 系統預設                |
| `--gecko-binary <path>`        | `--firefox-binary` | [自訂 Gecko 系列執行檔路徑](/zh-Hant/docs/browsers/running-other-browsers)。                                               | 系統預設                |
...
| `--no-open`                    | -                  | 啟動瀏覽器,但不為你的擴充功能開啟分頁。瀏覽器仍會啟動並載入擴充功能。見[阻止瀏覽器啟動](#阻止瀏覽器啟動)。 | 啟動時會開啟分頁 |
| `--no-browser`                 | -                  | 不啟動瀏覽器。開發伺服器仍會啟動並持續重新建置。見[阻止瀏覽器啟動](#阻止瀏覽器啟動)。 | 預設啟動瀏覽器 |
...
Safari 工作階段就是一個完整的 dev 工作階段。你在 Safari 設定中啟用擴充功能之後,`extension logs` 可以讀取它,控制通道會接上,而且每次儲存都會在 Safari 中重新載入擴充功能。請見 [建置 Safari 擴充功能](/zh-Hant/docs/browsers/safari)。
...
完整的 schema、錯誤狀態與兩階段就緒規則,請見 [ready.json 契約](/zh-Hant/docs/contracts/ready-json)。

`events.ndjson` 只屬於目前這次執行:新一次執行開始時檔案會重設,且每筆記錄都蓋上本次執行的 `runId`(與 `ready.json` 一致),消費端不會看到上一個工作階段的事件與目前的交錯。

工作階段行為異常時,執行 [`extension doctor`](/zh-Hant/docs/commands/doctor)——它依序檢查契約、控制通道、權杖、執行器與瀏覽器,並指出第一個失敗的環節與修復方式。
...
| 你想要                           | 旗標           | 會發生什麼                                                       |
| -------------------------------- | -------------- | ---------------------------------------------------------------- |
| 完全不要瀏覽器                   | `--no-browser` | 不會啟動任何瀏覽器程序。開發伺服器照常執行,並在存檔時重新建置。 |
| 要瀏覽器,但不為擴充功能開啟分頁 | `--no-open`    | 瀏覽器會啟動並載入你的擴充功能。不會為它開啟分頁。               |
...
`--no-browser` 是無頭、CI 與遠端工作的執行模式,也是 [Playwright E2E](/zh-Hant/docs/workflows/playwright-e2e) 工作流程所依賴的旗標。它還有設定寫法 `commands.dev.noBrowser: true`,以及環境變數寫法 `EXTENSION_CLI_NO_BROWSER=1`。

`--no-open` 只是啟動時的一個細節。當你希望瀏覽器停在它原本顯示的頁面上,而不讓 Extension.js 在上面為你的擴充功能開啟分頁時,就用它。`dev`、`start` 與 `preview` 都接受這兩個旗標。
...
從 4.1.21 開始，當 `--wait` 在產生者還在寫入 `ready.json` 時讀到它，會把這個寫到一半的檔案當作又一種暫態，繼續輪詢。到 4.1.20 為止，一次撕裂讀取會讓等待以 JSON 解析錯誤失敗。始終無法解析的檔案仍會以 `E_READY_TIMEOUT` 結束，逾時訊息會點明原因：`The last read of the file failed to parse as JSON`，後面接著解析器自己的錯誤。
...
dev 建置會輸出 `cheap-module-source-map` 類型的 source map，描述的是你的原始碼：原始的 TypeScript 與精確的行號，涵蓋 content script、傳統多檔案群組、background 以及兩個 manifest 版本下的頁面。不會使用任何 `eval` 變體，因此 bundle 在你自己的 CSP 下執行。
...
- 用 [`build`](/zh-Hant/docs/commands/build) 產生正式版產物。
- 用 [`start`](/zh-Hant/docs/commands/start) 驗證正式版啟動流程。
- 透過 [瀏覽器專屬 manifest 欄位](/zh-Hant/docs/features/browser-specific-fields) 了解瀏覽器目標。
