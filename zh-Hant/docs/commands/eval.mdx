...
擴充功能頁面（`popup`、`options`、`sidebar`、`devtools`、`newtab`、`history`、`bookmarks`）是透過它們自己的頁內轉送來回應的，因此該介面必須已在瀏覽器中開啟。請先用 `open`（記載於 [觸發 action 與 command](/zh-Hant/docs/debugging/trigger-actions-and-commands)）把它開起來。

pretty 輸出印出的是值本身：字串原樣輸出，其餘一律以縮排 JSON 輸出。信封結構只會在 `--output json` 之下出現（參見 [結果信封](/zh-Hant/docs/contracts/result-envelope)）。
...
- 目標介面未開啟，或 `--tab` id 已失效：`E_TARGET_NOT_FOUND`。請用 [`inspect --list-tabs`](/zh-Hant/docs/commands/inspect#the-discovery-loop) 找出仍然存活的 id。
- 呼叫超過了 `--timeout`：`E_TIMEOUT`。

運算式成功時結束碼為 `0`，否則為 `1`，因此腳本可以直接據此做判斷。當每個動詞都失敗、而你想知道是哪一環斷掉時，請執行 [`doctor`](/zh-Hant/docs/commands/doctor)。

## 下一步

- 用 [`inspect`](/zh-Hant/docs/commands/inspect) 找出要鎖定的分頁 id 與 DOM 狀態。
- 用 [`storage`](/zh-Hant/docs/commands/storage) 讀寫 `chrome.storage`，不必自己寫運算式。
- 在 [除錯](/zh-Hant/docs/debugging) 中了解更完整的除錯工作流程。
- 在 [結果信封](/zh-Hant/docs/contracts/result-envelope) 中查看機器可讀的輸出格式。
