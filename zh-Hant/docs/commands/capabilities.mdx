...
JSON 輸出是一個 schema-1 信封（參見 [結果信封](/zh-Hant/docs/contracts/result-envelope)），它的 `value` 帶有五個欄位：
...
3. 用動作類指令來驅動它：[`inspect`](/zh-Hant/docs/commands/inspect)、[`eval`](/zh-Hant/docs/commands/eval)、[`storage`](/zh-Hant/docs/commands/storage)、[`reload`](/zh-Hant/docs/commands/reload)。

## 後續步驟

- 在 [結果信封](/zh-Hant/docs/contracts/result-envelope) 中了解每個支援 json 的指令都會輸出的那個信封。
- 用 [`dev`](/zh-Hant/docs/commands/dev) 開啟這次交握所準備的工作階段。
