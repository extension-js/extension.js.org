...
- `publish` 從不編譯。想讓連結指向新鮮的輸出時,請先執行 [`build`](/zh-Hant/docs/commands/build)。
...
- 用 [`build`](/zh-Hant/docs/commands/build) 產出要分享的產物。
- 先用 [`preview`](/zh-Hant/docs/commands/preview) 在本地驗證這些產物。
