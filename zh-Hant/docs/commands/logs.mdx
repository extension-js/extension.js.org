...
`logs` 會讀取 [`dev`](/zh-Hant/docs/commands/dev) 工作階段從你的擴充功能收集到的記錄：background、content script、popup、options 以及其餘情境。一個指令就能依序合併顯示全部內容，不必開啟任何一個 DevTools 視窗。

<Frame>
  <iframe
    className="w-full aspect-video rounded-xl"
    src="https://www.youtube-nocookie.com/embed/R5RZ1vxLBTA?rel=0"
...
| `--browser <browser>`             | 讀取哪一個工作階段（`chrome`、`chromium`、`edge`、`firefox`、`safari`）。                         | `chromium`                      |
...
Safari 工作階段的讀取方式完全相同。`extension logs --browser safari` 會印出 `dev --browser safari` 工作階段中 background 與 content 的記錄列，加上 `--follow` 就是即時串流。請見 [建置 Safari 擴充功能](/zh-Hant/docs/browsers/safari)。

{/* TODO(version): name the release that ships Safari sessions for `logs` and the control bridge. Merged as 0508e40cd, unreleased as of 4.1.19. */}

## 一次性模式

不加 `--follow` 時，指令會直接讀取 `dist/extension-js/<browser>/logs.ndjson`，不需要任何即時連線。開發工作階段會把每一筆記錄附加到那個檔案，所以即使你已經關掉瀏覽器，一次性讀取仍然有效。

如果該檔案不存在，`logs` 會提示你先啟動 `extension dev`，並以結束碼 `1` 結束。機器可讀格式還會輸出一個錯誤碼為 `E_LOGS_NOT_FOUND` 的失敗信封（參見 [結果信封](/zh-Hant/docs/contracts/result-envelope)）。
...
- 用 [`doctor`](/zh-Hant/docs/commands/doctor) 診斷 `logs --follow` 連不上的工作階段。
- 用 [`inspect`](/zh-Hant/docs/commands/inspect) 一次呼叫就同時拿到 DOM 快照與最近的主控台輸出。
- 在 [除錯](/zh-Hant/docs/debugging) 中了解更完整的除錯工作流程。
- 在 [結果信封](/zh-Hant/docs/contracts/result-envelope) 中查看機器可讀的失敗格式。
