...
從 4.1.20 起，Safari 工作階段的讀取方式完全相同。`extension logs --browser safari` 會印出 `dev --browser safari` 工作階段中 background 與 content 的記錄列，加上 `--follow` 就是即時串流。請見 [建置 Safari 擴充功能](/zh-Hant/docs/browsers/safari)。
