...
`preview` 沒有 Safari 路徑。傳入 `--browser safari`(或 `webkit-based`)會以 `E_COMMAND_UNSUPPORTED_FOR_TARGET` 結束。Safari 是受支援的瀏覽器,但這個指令無法啟動它。Safari 目標請改用 [`dev`](/zh-Hant/docs/commands/dev) 或 [`build`](/zh-Hant/docs/commands/build)。
