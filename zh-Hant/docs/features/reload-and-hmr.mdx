...
上面的層級是 Chromium 與 Gecko 的重新載入路徑。從 4.1.20 起，Safari 的 dev 工作階段走另一條路：每次儲存會重建 `dist/safari`，以一次增量 `xcodebuild` 同步 app 的資源，並在 Safari 中重新載入擴充功能。在 Safari 26.5.2 上量測，從儲存到擴充功能完成重新載入大約 4 秒，沒有 HMR 這一層。請見 [建置 Safari 擴充功能](/zh-Hant/docs/browsers/safari)。
