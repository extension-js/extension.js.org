...
這兩個階段都以專案根目錄為錨點，也就是包含 `package.json` 的目錄。這就是為什麼 `pages/`、`scripts/` 與 `public/` 輸入只會從那個目錄解析，絕不從 `src/` 解析，即使是 `src/manifest.json` 定義了擴充功能也一樣。參見 [特殊資料夾](/zh-Hant/docs/features/special-folders#where-special-folders-live) 中的根目錄規則。
