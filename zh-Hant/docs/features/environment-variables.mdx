...
在 4.1.19 之前，打包工具會把 `import.meta.url` 替換成原始檔的 `file://` 路徑。這個路徑會把建置機器上的資料夾名稱帶進發佈的 bundle。從 4.1.19 起，Extension.js 改為在執行階段解析裸的 `import.meta.url`：

| 程式碼執行的位置                                   | `import.meta.url` 的值                     |
| -------------------------------------------------- | ------------------------------------------ |
| 擴充功能頁面與 worker（service worker、popup、options） | 該頁面或 worker 的 URL                     |
