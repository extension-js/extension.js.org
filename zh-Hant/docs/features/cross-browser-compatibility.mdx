...
Extension.js 透過帶前綴的 manifest 欄位，讓單一份 `manifest.json` 在各瀏覽器上都能運作。在編譯時，Extension.js 會把帶前綴的鍵（`chromium:`、`chrome:`、`edge:`、`firefox:`、`gecko:`）過濾到當前目標。`chromium:` 套用到所有 Chromium 家族瀏覽器，`chrome:` 與 `edge:` 只套用到對應的那個瀏覽器，未加前綴的鍵則套用到所有目標。完整前綴清單請見 [瀏覽器專屬的 manifest 欄位](/zh-Hant/docs/features/browser-specific-fields)。
...
對 Chromium 家族目標，Extension.js 以 `chromium:` 涵蓋整個家族，以 `chrome:` 或 `edge:` 指定單一瀏覽器。  
對 Firefox 家族目標，Extension.js 會使用 `firefox:` 與 `gecko:`。

<Note>
  完整的帶前綴 manifest 欄位細節，請參閱 [瀏覽器專屬的 manifest
  欄位](/zh-Hant/docs/features/browser-specific-fields)。
</Note>
