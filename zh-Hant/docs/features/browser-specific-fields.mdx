...
Safari 沒有實作的鍵與權限，會從 Safari 建置中自動移除，而且建置會為每個被移除的鍵印出一行，說明是哪個鍵、以及為什麼被移除。`content_scripts[].world` 會保留，因為 Safari 從 Safari 18 起就支援它。請見 [建置 Safari 擴充功能](/zh-Hant/docs/browsers/safari)。
...
不要寫 `edge:key`。Edge Add-ons 會拒絕任何在 manifest 中包含 `key` 的套件，因此該欄位在 Edge 建置裡沒有用處。擴充功能 id 由 Partner Center 指派。正式模式的 Edge 建置會丟棄 `key` 並印出一行說明原因，開發模式的建置則會保留它，因為此時穩定的 id 有用，而且不涉及任何商店。
