...
這樣設定後，從彈出視窗和選項頁分別執行 `await import("../shared/big")` 都會解析到同一個輸出檔 `shared.js`。請把 `chunks` 保持為 `"async"`。同步拆分會把程式碼移出 entry 檔案，但輸出的 HTML 和 manifest 只引用該 entry 檔案，頁面永遠不會載入被拆出的 chunk。service worker 不能使用動態 `import()`，會保留自己的一份模組副本。content script 也可以共享這個 chunk，但必須透過 `web_accessible_resources` 公開該檔案，並透過 `chrome.runtime.getURL` 匯入。這兩條規則請見[延遲載入](/zh-Hant/docs/implementation-guide/lazy-loading)。
...
- 進一步了解 [extension 設定（`extension.config.js`）](/zh-Hant/docs/features/extension-configuration)。
