...
Extension.js 只拆分 HTML 頁面。預設的 `optimization.splitChunks` 會給頁面兩個穩定的同級檔案：`shared/framework.js` 存放 UI 框架的執行階段，`shared/commons.js` 存放被兩個或更多頁面匯入的任何模組。框架群組涵蓋 React、Preact、Vue、Svelte 與 Lit。這兩個名稱都不帶雜湊，所以 manifest 項目或公開資源可以一直指向它們。
