...
<CardGroup cols={2}>
  <Card title="選擇 Extension.js,如果你…">
    ...
    - 想要 Rspack 的 Rust 等級建置速度,以及助理可讀的文件:文件 MCP 端點和 `llms.txt`([詳情](/zh-Hant/docs/ai-access))。
  </Card>
  <Card title="選擇 WXT,如果你…">
    - 偏好**檔案系統慣例**(`entrypoints/`)勝於明確的 manifest。
    - 需要把 **Manifest V2** 作為主要目標。
    - 希望每個檔案都自動 import `browser`,它會解析為原生的 `browser` 物件或 `chrome`。
  </Card>
</CardGroup>
...
| 維度                 | Extension.js                                                          | WXT                                                             |
| -------------------- | --------------------------------------------------------------------- | --------------------------------------------------------------- |
| 打包工具             | [Rspack](/zh-Hant/docs/features/rspack-configuration)(Rust 為基礎)            | Vite(正在遷移到 Rolldown)                                       |
...
| 瀏覽器目標           | Chrome、Edge、Firefox、Safari(macOS,透過 Xcode)、Chromium、Gecko、自訂執行檔 | Chrome、Edge、Firefox、Safari(只建置,不會啟動瀏覽器)、自訂執行檔 |
| Manifest V3          | 預設                                                                  | 預設                                                            |
...
| `browser.*` polyfill | `--polyfill`,`dev` 預設開啟,`build` 預設關閉                          | `browser` 別名到 `chrome`,真正的 polyfill 需自行啟用 `@wxt-dev/webextension-polyfill` |
| 範本                 | React、Preact、Vue、Svelte、TypeScript、JavaScript、init              | React、Vue、Svelte、Solid、vanilla                              |
| TypeScript           | 一流支援                                                              | 一流支援                                                        |
| AI 介面              | 文件 MCP 端點 + `llms.txt`([詳情](/zh-Hant/docs/ai-access))                 | 提供 `llms.txt`                                                 |
...
引數也可以是遠端 GitHub URL 或 ZIP 壓縮檔,適合在不 clone 的情況下啟動範例。詳見[立即開始](/zh-Hant/docs/getting-started/immediately)。
...
- **Extension.js** 在一份 `manifest.json` 內使用[瀏覽器前綴 manifest 欄位](/zh-Hant/docs/features/browser-specific-fields)(`chrome:`、`firefox:`、`gecko:` 等)。未加前綴的鍵套用到所有目標,加了前綴的鍵只會落到對應的建置。
- **WXT** 從 `wxt.config.ts` 與每個目標的覆寫運算 manifest。瀏覽器差異存在於 TypeScript 設定中,而不是 manifest 本身。

兩個專案取得 `browser` 命名空間的方式也不同。當你傳入 `--polyfill` 時,Extension.js 會打包 [webextension-polyfill](https://github.com/mozilla/webextension-polyfill),`extension dev` 預設會這麼做,`extension build` 則不會。WXT 會從 `wxt/browser` 自動 import 一個 `browser` 物件。在 wxt 0.21.4 中,那個物件只是原生別名:瀏覽器提供 `globalThis.browser` 時就用它,否則用 `globalThis.chrome`,所以它並不是 polyfill。完整的 polyfill 是另一個需要自行啟用的模組 `@wxt-dev/webextension-polyfill`。
...
2. 用 [`extension.config.js`](/zh-Hant/docs/features/extension-configuration) 取代 `wxt.config.ts` 來設定建置預設值。
3. 用 `extension dev`/`extension build` 取代 `wxt`/`wxt build` 腳本。

大多數 React、Preact、Vue 與 Svelte 原始檔可以直接複製過來,毋需修改。完整的逐步指南請見 [從 WXT 遷移](/zh-Hant/docs/migrate/from-wxt)。

## 何時選擇 Extension.js

- 你想把 manifest 當作唯一來源,而不是由設定產生。
- 你想要一個 CLI 引數就能開發遠端範例(`extension dev <github-url>`)。
- 你希望有文件 MCP 端點和 `llms.txt` 供助理使用。
...
- 你希望各處都自動 import `browser`,並在需要時自行啟用完整的 polyfill 模組。
