...
當缺少 `vue-loader` 或 `@vue/compiler-sfc` 這類選用的 Vue 整合套件時，建置錯誤會指出與內建工具鏈相符的確切版本。錯誤中的安裝指令會依你專案使用的套件管理器來呈現。
...
從 4.1.19 開始，Vue 專案打包的是僅含執行階段的建置 `vue.runtime.esm-bundler.js`，也就是 `vue` 套件在 `module` 進入點指定的檔案。Extension.js 會把 `vue$`、`@vue/runtime-dom`、`@vue/runtime-core` 與 `@vue/shared` 都別名到那個建置，這樣你的應用程式與任何被轉譯的 workspace 相依套件共用同一個 Vue 實例。完整建置帶有範本編譯器，擴充功能的 CSP 無法執行它，而且它的 `innerHTML` 與 `Function` 呼叫會通不過 addons.mozilla.org 的商店檢查。
...
那個建置用 `new Function` 編譯範本，這需要擴充功能 CSP 裡有 `unsafe-eval`。Manifest V3 的擴充功能頁面禁止它，所以這個別名只對 Manifest V2 專案有幫助。
...
<Frame>
  <iframe
    ...
    src="https://www.youtube-nocookie.com/embed/Jdf4wHl0s1I?rel=0"
...
<Frame>
  <iframe
    className="w-full aspect-video rounded-xl"
    src="https://www.youtube-nocookie.com/embed/LIfZxR-B2Nk?rel=0"
