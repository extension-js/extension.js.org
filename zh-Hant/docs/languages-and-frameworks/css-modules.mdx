...
Content script 與擴充功能頁面的處理方式相同。Rspack 會產生具區域作用域的 class 名稱，將它們匯出到你的 JavaScript，Extension.js 則把產生的樣式表註冊到 manifest 的 `content_scripts[].css` 項目，並一併註冊到 `web_accessible_resources`，讓它也可以被注入到 shadow root 中。匯入你需要的 class 名稱並直接套用：
...
可參考上方 `content-css-modules` 範本中的可執行範例。
