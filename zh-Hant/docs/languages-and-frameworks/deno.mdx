...
| Deno 版本      | 狀態                                                                     |
| -------------- | ------------------------------------------------------------------------ |
| 2.5 及更新版本 | 支援，2.8.0 除外                                                         |
| 2.8.0          | 拒絕。該版本無法在打包器內部解析 `node:querystring`，Deno 在 2.8.1 修正了 |
| 早於 2.5       | 拒絕，因為下限是 2.5。作為背景，這些版本模擬的 Node 版本也低於 22.12     |
...
TypeScript 的偵測規則與其他地方完全相同。參見 [TypeScript](/zh-Hant/docs/languages-and-frameworks/typescript)：由 SWC 編譯原始碼，`typescript` 套件只有在做 `tsc --noEmit` 時才需要。

## 受管理的瀏覽器需要 Node.js

`extension install <browser>` 透過 `npx`（或啟動 CLI 的那個套件管理器）下載受管理的瀏覽器。`deno` 沒有這樣的執行器，所以在 Deno 上這一條指令需要 `PATH` 裡有 Node.js。沒有它，指令會在下載步驟失敗。`dev`、`build`、`preview` 與 `start` 只靠 Deno 就能執行，系統裡已經安裝的瀏覽器也不需要下載。參見 [`install`](/zh-Hant/docs/commands/install)。
