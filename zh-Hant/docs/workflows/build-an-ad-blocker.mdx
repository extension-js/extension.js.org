...
用 `declarativeNetRequest` API 打造一個 adblocker（也寫作 ad blocker 或 content blocker）。你宣告比對規則，瀏覽器就會在請求離開網路堆疊前原生封鎖它們。
...
在編輯 `manifest.json` 之前，先知道 manifest 變更需要重新啟動開發伺服器。參考 [開發更新行為](/zh-Hant/docs/workflows/dev-update-behavior)。
...
- 當兩個目標出現分歧時，使用 [瀏覽器專屬 manifest 欄位](/zh-Hant/docs/features/browser-specific-fields)。
...
- 在 [攔截網路請求](/zh-Hant/docs/workflows/intercept-network-requests) 觀察流量而不封鎖它。
- 用 [安全檢查清單](/zh-Hant/docs/workflows/security-checklist) 稽核你的權限面。
- 在 [Manifest V3 概念](/zh-Hant/docs/concepts/manifest-v3) 檢視 background 模型。
