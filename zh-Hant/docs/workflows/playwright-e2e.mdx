...
`ready.json`（schema v2）包含為腳本／代理人設計的穩定欄位：
...
- `port`
- `pid`
- `browserPid`（啟動的瀏覽器程序，用它來收尾關閉）
- `runId`
- `startedAt`
- `compiledAt`
- `errors`
- `runtime`：service worker 連上之後為 `"attached"`
- `executorAttachedAt`

把這份合約當作就緒狀態與失敗的真相來源。完整的欄位參考，包含 `extensionId`、`profilePath`、`cdpPort` 以及各種錯誤狀態，請見 [ready.json](/zh-Hant/docs/contracts/ready-json)。

<Frame>
  <iframe
    className="w-full aspect-video rounded-xl"
    src="https://www.youtube-nocookie.com/embed/zE2SOnM8gDc?rel=0"
...
第 2 步有一個要注意的地方：`status: "ready"` 只代表編譯完成。如果你的測試會透過 act 指令（`eval`、`storage`、`reload`、`open`）驅動擴充功能，還要等到合約帶有 `runtime: "attached"`。由 Playwright 啟動的瀏覽器會自己載入 `distPath`，所以純 UI 測試只需要 `ready`。
...
2. 程序 B：`extension dev --wait --browser=<browser> --output json`
3. 等待步驟成功結束後再啟動 Playwright

正式導向的變體：

1. 程序 A：`extension start --no-browser`
2. 程序 B：`extension start --wait --browser=<browser> --output json`
3. 等待步驟成功結束後再啟動 Playwright

`--output json` 會在 stdout 印出單一信封物件，給人閱讀的文案則移到 stderr。較舊的 `--wait-format` 別名仍可使用，但會在 stderr 發出警告。
...
`headless: false` 是必要的，不是偏好設定。Playwright 預設的 headless 模式會執行 Chromium
的 `headless_shell` 執行檔，它完全不會載入任何擴充功能。為了 CI 把它改成 `true` 的測試
套件仍然會通過，因為它悄悄測試的是一個沒有你的擴充功能的瀏覽器。若要在沒有可見視窗的
情況下執行，請保持 `headless: false` 並在 `args` 中加入 `--headless=new`，它使用的是確實
支援擴充功能的完整 Chromium headless 模式。
...
`playwright` 範本會建立一套可用的設定，你可以直接拿來參考：

- `playwright.config.ts` 與一個 `e2e/` 資料夾，見 [extension-js/examples/playwright](https://github.com/extension-js/examples/tree/main/examples/playwright)

## 下一步

- 設定 [CI 範本](/zh-Hant/docs/workflows/ci-templates)。
- 讓指令工作流程與 [dev](/zh-Hant/docs/commands/dev) 和 [build](/zh-Hant/docs/commands/build) 對齊。
...
<Frame>
  <iframe
    className="w-full aspect-video rounded-xl"
    src="https://www.youtube-nocookie.com/embed/y1twvcH9SSU?rel=0"
