...
popup 失去焦點就會關閉，錄製也隨之中止。改把錄製器放在專屬頁面上。把 `recorder.html` 與 `recorder.js` 放進 `pages/` [特殊資料夾](/zh-Hant/docs/features/special-folders)，Extension.js 會把它們編譯為 entrypoint。
...
如果你要的是靜態圖片而不是錄影，`chrome.tabs.captureVisibleTab` 會把使用中分頁的可見部分作為 data URL 回傳。它需要 `activeTab` 權限或該頁面的主機權限。

```js
const dataUrl = await chrome.tabs.captureVisibleTab(undefined, { format: "png" })
```

整頁截圖不是一次呼叫就能完成。從 content script 分步捲動頁面，逐步擷取，再在 canvas 上拼接圖片。整頁擷取工具就是這樣運作的。
...
- 使用 [瀏覽器專屬 manifest 欄位](/zh-Hant/docs/features/browser-specific-fields)，讓 `tabCapture` 權限不進入 Firefox 建置。
...
- 在 [管理下載](/zh-Hant/docs/workflows/manage-downloads) 管理儲存的檔案。
- 檢視 `pages/` 項目的 [特殊資料夾](/zh-Hant/docs/features/special-folders) 合約。
- 用 [安全檢查清單](/zh-Hant/docs/workflows/security-checklist) 稽核擷取權限。
