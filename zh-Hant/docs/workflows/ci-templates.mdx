...
- 給腳本或 CI 自動化使用時，優先採用 `--output json`。較舊的 `--wait-format` 別名仍可運作，但會在 stderr 發出警告。
- 在機器輸出模式下，給人看的文字會移到 stderr，因此 stdout 只會帶有一份可解析的 JSON 文件。
- 收尾時，從 `ready.json` 讀取 `browserPid` 並向該程序送出訊號。那是回收瀏覽器程序時受支援的把手。
...
第一行啟動 session，第二行以它的就緒合約作為閘門。`--wait` 只負責等待，
它不會啟動 session，所以兩者要放在同一個 `run:` 步驟裡。

同樣的方式也能重播鍵盤指令（`extension open command --name <cmd>`）。回傳的 payload 與唯一注意事項（重播不會帶有使用者操作手勢，所以 `activeTab` 不會授予）請見 [觸發 actions 與指令](/zh-Hant/docs/debugging/trigger-actions-and-commands)。
...
對於緩慢或資源受限的容器，可以透過環境變數增加瀏覽器傳輸逾時（見 [瀏覽器傳輸調校變數](/zh-Hant/docs/features/environment-variables#瀏覽器傳輸調校變數)）。
...
`create` 不會替你產生 CI workflow。想要一份可以直接照抄的完整範例，可以看看 Extension.js 用在自己身上的那一份：
...
- 加入或精修 [Playwright E2E 測試](/zh-Hant/docs/workflows/playwright-e2e)。
- 檢視 [指令參考](/zh-Hant/docs/commands/dev) 以對齊腳本。
