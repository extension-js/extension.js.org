...
- 如果在持續整合（CI）容器中瀏覽器連線緩慢或不穩定，請透過 [瀏覽器傳輸調校變數](/docs/features/environment-variables#瀏覽器傳輸調校變數) 增加連線逾時。
...
把檔案移到專案根目錄下的其他資料夾（例如 `bin/`、`tools/` 或 `ci-scripts/`）。詳情請見 [特殊資料夾](/docs/features/special-folders#scripts-不允許-nodejs-腳本)。
