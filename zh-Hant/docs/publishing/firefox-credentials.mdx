...
附加元件 ID（AMO 稱它為 GUID）在每一次提交中識別你的附加元件。它同時也是要填進
`browser_specific_settings.gecko.id` 的那個值。Extension.js 在這個鍵存在時會讀取它，但從不替你寫入。
...
只有在擴充功能確實不傳輸任何資料時才使用 `["none"]`。這份宣告、[STORE.md](/zh-Hant/docs/workflows/store-metadata)
的隱私章節與程式碼三者必須一致。完整規則請見
[Firefox 商店就緒](/zh-Hant/docs/features/multi-platform-builds#firefox-store-readiness-data-collection-permissions)。
