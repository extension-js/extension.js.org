...
After installing, your package manager may report that build scripts for
`less` were not run (pnpm prints `Ignored build scripts: less` and asks you to
approve them; npm shows a similar notice). This is expected and **safe to
ignore** — Extension.js compiles Less through its own bundler pipeline, so it
does not rely on Less's optional post-install step. Your `.less` and
`.module.less` files build and hot-reload the same either way.

If you prefer to silence the warning, you can explicitly allow the script:

- **pnpm:** run `pnpm approve-builds` and select `less`, or add
  `pnpm.onlyBuiltDependencies: ["less"]` to your `package.json`.
- **npm:** the notice is informational; no action is required.

There is no functional downside to leaving the scripts unrun for typical
Extension.js projects.
