...
The Preact integration also uses optional packages such as `@prefresh/core`, `@prefresh/utils`, and `@rspack/plugin-preact-refresh`. When one is missing, the build error names the exact version that matches the bundled toolchain. The install command in the error is phrased for the package manager that your project uses.
...
{/* VIDEO_SPEC id=hmr-preact grammar=D→E ~12s loop  [family: hmr]  [SHIPPED: video/scenes/hmr-preact.scene.mjs]
  Needs: mock transcript `cmd-dev` (reused — the dev session is staged off
    camera); fixture mock/takes/hmr-preact/src/newtab/NewTabApp.tsx
    (new-preact's file, verbatim), seeded by the scene over
    src/newtab/NewTabApp.tsx, which carries a .pristine twin — React, Preact
    and Tailwind all film that one path, so each take seeds it; page
    mock/site/preact.html (new-preact's own new tab); usesEditor
    src/newtab/NewTabApp.tsx at 15:9 — the JSX heading text, eight spaces of
    indent; stage(): dev running, new tab open, editor on the file
  D editor.replaceToEndOfLine('Hello from Preact.') mark:edited
    editor.save()                                   mark:saved
  E browser.focus(); the UI updates in place (note: Preact fast-refresh is
    intentionally disabled — this shows the reload path, not state-keeping)
                                                    mark:updated → hold read → end
  post: trim start→end, posterAt updated, ramps [start→edited 1.35x], loop
  run: node video/record.mjs hmr-preact --mock */}
