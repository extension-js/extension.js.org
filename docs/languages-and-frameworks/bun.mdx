...
The published packages declare `"engines": {"node": ">=22.12"}`. The CLI checks the runtime at startup: it stops on a Node version below that floor, and it stops on Bun whatever version Bun reports.

The refusal is about the runtime, not about a version number. Bun is detected through `process.versions.bun`, which the runtime sets itself, so every route into it is covered: the `--bun` flag on `bunx` and `bun run`, and a `run.bun` default in `bunfig.toml`. Forcing the Bun runtime trips the guard:
