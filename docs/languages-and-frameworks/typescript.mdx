...
Extension.js runs TypeScript across every extension context — background, content scripts, popup, options, and side panel. It uses the default Rspack + SWC (Speedy Web Compiler) pipeline with no separate `tsc` step, no ts-loader to configure, and no extra webpack/Rspack rule for `.ts`/`.tsx` files.
