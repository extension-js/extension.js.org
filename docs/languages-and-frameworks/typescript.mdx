...
Extension.js runs TypeScript across every extension context — background, content scripts, popup, options, side panel. It uses the Rspack + SWC (Speedy Web Compiler) pipeline by default. You do not need a separate `tsc` step, a ts-loader, or an extra bundler rule for `.ts`/`.tsx` files.
