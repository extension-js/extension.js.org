...
Extension.js runs TypeScript across every extension context — background, content scripts, popup, options, side panel. It uses the default Rspack (bundler) + SWC (Speedy Web Compiler) pipeline. You do not need a separate `tsc` step, a ts-loader, or an extra bundler rule for `.ts`/`.tsx` files.
