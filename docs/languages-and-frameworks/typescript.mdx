...
description: 'Build TypeScript browser extensions with zero bundler config. Extension.js supports TS across background, content scripts, and pages via Rspack and SWC.'
icon: 'code'
keywords:
  [
    'extension.js',
...
Extension.js runs TypeScript across every extension context — background, content scripts, popup, options, side panel. It uses the default Rspack + SWC (Speedy Web Compiler) pipeline. There is no separate `tsc` step, no ts-loader to configure, and no extra webpack/Rspack rule for `.ts`/`.tsx` files.
...
Extension.js compiles TypeScript through the default bundler pipeline but does not run full `tsc` type-checking during builds.
