...
That build compiles templates with `new Function`, which needs `unsafe-eval` in the extension CSP. Manifest V3 extension pages forbid it, so the alias only helps a Manifest V2 project.
