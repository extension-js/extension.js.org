...
Extension.js treats Deno as a first-class runtime, and a CI lane runs `extension build` and `extension dev` on Deno for every change. Scaffold with Deno and the project gets a `deno.jsonc` instead of a `package.json`. Work in a hybrid project and `deno.jsonc` sits beside `package.json` for Deno-native settings. Either way, `extension dev` and `extension build` behave the same.

## Which Deno versions work

Extension.js needs Deno 2.5 or newer. The CLI reads `process.versions.deno` at startup and judges Deno on its own version, never on the Node version that Deno emulates, so the number in the message is the one you can act on.

| Deno version  | Status                                                                                                  |
| ------------- | ------------------------------------------------------------------------------------------------------- |
| 2.5 and newer | Supported, apart from 2.8.0                                                                               |
| 2.8.0         | Refused. That release cannot resolve `node:querystring` inside the bundler, and Deno fixed it in 2.8.1     |
| Older than 2.5 | Refused, because the floor is 2.5. As background, those releases also emulate a Node version below 22.12 |
...
## Managed browsers need Node.js

`extension install <browser>` downloads a managed browser through `npx`, or through the package manager that launched the CLI. `deno` has no such runner, so on Deno that one command needs Node.js on your `PATH`. Without it the command fails at the download step. `dev`, `build`, `preview`, and `start` run on Deno alone, and a browser that is already installed on your system needs no download. See [`install`](/docs/commands/install).
