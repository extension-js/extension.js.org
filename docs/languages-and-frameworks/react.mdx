---
title: "React for browser extensions"
description: "Build browser extension UIs with React using Extension.js. Get auto-configured JSX transforms, aliases, and fast refresh during development."
keywords:
  [
    "extension.js",
    "extensions",
...
The React integration also uses optional packages such as `react-refresh` and `@rspack/plugin-react-refresh`. When one is missing, the build error names the exact version that matches the bundled toolchain. The install command in the error is phrased for the package manager that your project uses.
...
{/* VIDEO_SPEC id=hmr-react grammar=D→E ~12s loop  [family: hmr]  [SHIPPED: video/scenes/hmr-react.scene.mjs]
  Needs: mock transcript `cmd-dev` (reused — the dev session is staged off
    camera, so its output never reaches the take); fixture
    mock/takes/hmr-react/src/newtab/NewTabApp.tsx (new-react's file, verbatim),
    seeded by the scene over src/newtab/NewTabApp.tsx, which carries a
    .pristine twin; page mock/site/react.html (new-react's own new tab);
    usesEditor src/newtab/NewTabApp.tsx at 15:9 — the JSX heading text, eight
    spaces of indent, no quotes to put back; stage(): dev running, new tab
    open, editor on the file
  D editor.replaceToEndOfLine('Hello from React.')  mark:edited
    editor.save()                                   mark:saved
  E browser.focus(); fast refresh swaps the component in place — state kept, no
    extension reload                                mark:updated → hold read → end
  post: trim start→end, posterAt updated, ramps [start→edited 1.35x], loop
  run: node video/record.mjs hmr-react --mock */}
