...
JSX pages compile for the framework the project installs: React, Preact, and Vue (through `vue/jsx-runtime`). Svelte components are not JSX and keep compiling through `svelte-loader`. The parser follows the file extension: `.tsx` and `.jsx` files parse JSX, and a `.ts` file never does.
