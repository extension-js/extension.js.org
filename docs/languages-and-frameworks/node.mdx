...
description: 'Use browser-native APIs first, then selectively polyfill Node core modules when needed. Extension.js targets the browser runtime by default.'
icon: 'server'
keywords:
  [
    'extension.js',
...
Extension.js targets the browser runtime by default and does not automatically polyfill Node core modules. That keeps bundles small, but if a dependency reaches for `buffer`, `stream`, or `crypto`, you will see a resolution error at build time. The rest of this page covers when to add polyfills and when to pick a browser-native alternative instead.
...
- Extension.js disables Node core fallbacks such as `crypto`, `path`, and `fs` by default.

If you import Node APIs without adding explicit fallbacks, the build fails.
...
`polyfill: true` in CLI/config enables `webextension-polyfill` (a compatibility shim that gives Chromium browsers access to the `browser.*` API namespace), not Node core module polyfills.
