...
Extension.js targets the browser runtime by default and does not automatically polyfill Node core modules. That keeps bundles small. If a dependency reaches for `buffer`, `stream`, or `crypto`, you will see a resolution error at build time. The rest of this page covers when to add polyfills and when to pick a browser-native alternative.
...
Setting `polyfill: true` in CLI/config enables `webextension-polyfill`, a compatibility shim that gives Chromium browsers access to the `browser.*` API namespace. It does not enable Node core module polyfills.
