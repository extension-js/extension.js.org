...
description: 'Scope styles by default and avoid naming collisions across extension popups, options UIs, and pages. Zero-config CSS Modules via the Rspack CSS pipeline.'
icon: 'layer-group'
keywords:
  [
    'extension.js',
...
Extension.js supports CSS Modules with zero configuration for module-style filenames (for example `styles.module.css`) and routes them through the Rspack CSS pipeline so class names stay locally scoped to the component that imported them.
...
- you can use module class exports directly in JS/TS/TSX files
