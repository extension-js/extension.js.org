...
Extension.js discovers locale files next to your manifest, validates that each declared locale has a `messages.json`, and emits locale JSON assets into the browser-specific build. Extension.js picks up edits to any locale file in dev without a full restart.
...
Here is an example of a `messages.json` file used for translations:
