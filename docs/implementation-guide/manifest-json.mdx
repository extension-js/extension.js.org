---
title: 'Manifest.json compilation and output'
description: 'Treat manifest.json as the source of truth for entrypoints and assets. Extension.js compiles, rewrites paths, and emits a browser-ready manifest per target.'
icon: 'brackets-curly'
keywords:
  [
    'extension.js',
...
Extension.js compiles your manifest, filters browser-prefixed fields, rewrites runtime paths, and validates referenced files. It emits a ready-to-load manifest per target browser.
...
If a project has a `package.json`, Extension.js does not treat `public/manifest.json` as a valid fallback. This guard prevents a copied static asset from overwriting the generated manifest.
...
The manifest is also where your extension declares what it is allowed to do. Extension.js compiles the manifest, but you still need good permission design.
...
Extension.js rewrites manifest paths to canonical output locations when needed. Two important examples:
...
- Validate icons, JSON resources, and content script assets as part of CI (continuous integration) to catch path regressions early.
