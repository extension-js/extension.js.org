...
- Classic service worker mode uses `importScripts` (the legacy Web Worker script-loading function) for chunk loading.
...
- Use the background context as the central coordinator for messaging, storage, and browser API calls, since it has full access to extension APIs.
