...
Extension.js processes HTML entrypoints from two places: manifest fields (popup, options, side panel, new tab, DevTools) and the special `pages/` folder for routes the manifest does not name directly. Each entry is walked for scripts, styles, and linked assets so you author plain HTML and get the asset graph wired automatically.
