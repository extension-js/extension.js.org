...
Extension.js processes HTML entrypoints from two places: manifest fields (popup, options, side panel, new tab, DevTools) and the special `pages/` directory. Use `pages/` for routes the manifest does not name directly. Extension.js walks each entry for scripts, styles, and linked assets. You author plain HTML and Extension.js wires the full set of linked assets automatically.
...
Most user-facing extension surfaces that render UI are just HTML entrypoints with different manifest fields and runtime expectations. Use the surface that matches your interaction model, then keep the HTML page itself thin.
