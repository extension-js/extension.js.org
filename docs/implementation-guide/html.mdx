...
Extension.js processes HTML entrypoints from two places: manifest fields (popup, options, side panel, new tab, DevTools) and the special `pages/` folder. Use `pages/` for routes the manifest does not name directly. You author plain HTML and Extension.js processes each entry for scripts, styles, and linked assets, wiring the asset graph automatically.
