...
Extension.js processes HTML entrypoints from two places: manifest fields (popup, options, side panel, new tab, DevTools) and the special `pages/` folder for routes the manifest does not name directly. It walks each entry for scripts, styles, and linked assets. You author plain HTML and Extension.js wires everything automatically.
