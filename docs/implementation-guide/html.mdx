...
- A `<base href>` tag is honored for links and removed from the page: `<a href>`, `<area href>`, and `<form action>` values are resolved against it and written into the output, while scripts, styles, and images stay inside the extension.
...
| Page action popup | `page_action.default_popup`                                 | address-bar popup on Firefox, shipped as its own page beside the toolbar popup |
...
<Frame>
  <iframe
    ...
    src="https://www.youtube-nocookie.com/embed/wfAMR_pFi-I?rel=0"
    title="Extension.js: New Tab template"
    loading="lazy"
