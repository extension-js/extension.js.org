...
Extension.js processes HTML entrypoints from two sources: manifest fields and the special `pages/` folder. Manifest fields include popup, options, side panel, new tab, and DevTools. Use `pages/` for routes the manifest does not name directly. You author plain HTML and Extension.js walks each entry for scripts, styles, and linked assets automatically.
