...
Extension.js processes HTML entrypoints from two sources: manifest fields (popup, options, side panel, new tab, DevTools) and the special `pages/` folder. Use `pages/` for routes the manifest does not name directly. Extension.js walks each entry for scripts, styles, and linked assets. You author plain HTML and Extension.js configures everything automatically.
