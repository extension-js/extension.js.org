...
Extension.js processes HTML entrypoints from two sources: manifest fields and the special `pages/` folder. Manifest fields include popup, options, side panel, new tab, and DevTools. Use `pages/` for routes the manifest does not name directly. Extension.js walks each entry for scripts, styles, and linked assets. Author plain HTML and Extension.js configures everything automatically.
...
Most extension surfaces that render UI are just HTML entrypoints with different manifest fields and runtime expectations. Use the surface that matches your interaction model, then keep the HTML page itself thin.
