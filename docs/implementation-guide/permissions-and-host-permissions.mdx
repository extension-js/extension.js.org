...
description: "Design extension permissions with least privilege using permissions, host_permissions, optional_permissions, and optional_host_permissions in manifest.json."
icon: "puzzle"
keywords: ["extension.js", "browser", "manifest", "content script", "popup", "docs", "implementation guide", "permissions and host permissions"]
---
...
| Capability that can be requested later                 | `optional_permissions`      |
...
- Keep one manifest accurate in [manifest.json](/implementation-guide/manifest-json).
- Review page access in [Web accessible resources](/implementation-guide/web-accessible-resources).
- Validate boundary handling in [Messaging](/implementation-guide/messaging).
- Do a release pass with [Security checklist](/workflows/security-checklist).
