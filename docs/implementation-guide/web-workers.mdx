...
<Warning>
  A development session injects a broad `web_accessible_resources` entry that a
  production build does not. Code that reaches a bundled file from the page can
  therefore work under `extension dev` and fail after `extension build`. Test
  the production output before you ship.
</Warning>
