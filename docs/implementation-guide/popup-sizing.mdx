...
Safari renders the popup as a popover with the same content-driven model. Keep the explicit-width pattern and test with `extension dev --browser=safari`, which runs on macOS only and needs the full Xcode app. A save reloads the extension in Safari in roughly 4 seconds (see [Safari](/docs/browsers/safari)). Safari is stricter about drawing before layout settles, so pre-sized containers matter more there.
