...
Safari renders the popup as a popover with the same content-driven model. Keep the explicit-width pattern and test with `extension dev --browser=safari`, which runs on macOS only, needs the full Xcode app, and has no live reload yet, so every size tweak means another build (see [Safari](/docs/browsers/safari)). Safari is stricter about drawing before layout settles, so pre-sized containers matter more there.
