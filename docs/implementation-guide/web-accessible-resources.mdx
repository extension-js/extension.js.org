---
title: 'Web accessible resources and build-time merging'
...
| ------------------------ | -------------------------------------------------------------- |
| Manifest V2/V3 schema handling  | Support both array and object `web_accessible_resources` shapes by manifest version   |
| Build-aware merging      | Combine declared resources with required build/runtime entries |
| Path normalization       | Resolve manifest web accessible resources (WAR) paths to target-safe output references    |
...
Automatic inclusion is convenient, but you should still review the final exposure surface.
