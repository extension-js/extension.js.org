...
- Extension.js uses browser-first module resolution, checking `browser`, `module`, and `main` fields in each dependency's `package.json`.
- Extension.js emits entries predictably and does not split code into separate runtime chunks by default.
- Path resolution rewrites hardcoded extension file paths (such as arguments to `chrome.runtime.getURL()`) to their correct output locations.
