---
title: 'JavaScript and TypeScript in extensions'
...
Extension.js collects script entrypoints from `manifest.json`, HTML pages, and special folders, then compiles them with the default SWC (Speedy Web Compiler)-based setup.
...
Here is an example of defining JavaScript files in `manifest.json`:
...
- **Content scripts:** Extension.js injects HMR/remount flow for fast updates.
...
- Extension.js uses browser-first resolution (`browser`, `module`, `main`).
- Extension.js emits entries predictably and does not split code into separate runtime chunks by default.
