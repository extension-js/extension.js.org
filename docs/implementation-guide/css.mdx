...
Extension.js supports plain CSS plus Sass and Less preprocessors without additional configuration. It routes styles differently for page contexts (popup, options, side panel) and content-script contexts. Page styles ship as linked assets. Extension.js injects content-script styles into the host document so they apply to the active tab.
