...
Extension.js supports plain CSS plus Sass and Less preprocessors out of the box. Extension.js routes styles differently for page contexts (popup, options, side panel) and content-script contexts. Page styles ship as linked assets, while Extension.js injects content-script styles into the host document so they apply in the active tab.
