...
- **`world: "MAIN"` is Chromium-only.** Firefox does not support the `world` field and ignores it — your script still runs in the isolated world. For cross-browser MAIN world behavior, use the `chromium:` manifest prefix to target Chromium only. Then provide an isolated-world fallback for Firefox.
