...
Why this matters: Extension.js remounts content scripts during development. Without a cleanup function, you can easily duplicate UI, event listeners, observers, and timers.
...
- In dev mode, Extension.js adds HMR accept/dispose behavior and remount flow.
...
- **`world: "MAIN"` is Chromium-only.** Firefox does not support the `world` field and ignores it. Your script still runs in the isolated world on Firefox. For cross-browser MAIN world behavior, use the `chromium:` manifest prefix to declare it only for Chromium targets. Then provide an isolated-world fallback for Firefox.
