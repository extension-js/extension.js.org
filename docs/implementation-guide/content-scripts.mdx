...
For every content-script-like entry, Extension.js expects a mount-style default export (a function that sets up behavior and optionally returns a cleanup callback):
...
Why this matters: Extension.js remounts content scripts during development. Without a cleanup contract, you can easily duplicate UI, event listeners, observers, and timers.
...
- CSS updates trigger internal remount events in development.
...
In **development mode**, content script JS filenames include a short hash suffix (for example, `content-0.abcd1234.js`). This forces the browser to load a fresh `chrome-extension://` URL after each rebuild. Chrome aggressively caches extension resources. Stable filenames let the browser serve cached scripts even after a hard refresh. Production builds use clean `content-0.js` names.
...
- **`world: "MAIN"` is Chromium-only.** Firefox does not support the `world` field and ignores it — your script still runs in the isolated world. For cross-browser MAIN world behavior, use the `chromium:` manifest prefix to declare it only for Chromium targets. Provide an isolated-world fallback for Firefox.
...
The browser still controls where a content script runs. Extension.js bundles the file, but the manifest entry still defines the matching contract.
