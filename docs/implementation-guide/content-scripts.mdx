...
In **development mode**, content script JS filenames include a short hash suffix (for example, `content-0.abcd1234.js`). This forces the browser to load a fresh `chrome-extension://` URL after each rebuild, because Chrome aggressively caches extension resources. Production builds use clean `content-0.js` names.
