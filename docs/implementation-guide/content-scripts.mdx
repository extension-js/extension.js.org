---
title: "Content scripts with HMR and manifest entries"
description: "Build page-integrated features with content scripts. Extension.js compiles manifest entries, wraps them for HMR, and emits predictable outputs."
keywords:
  [
...
Extension.js compiles content script entries from `manifest.json` and wraps them for runtime mounting and hot module replacement (HMR) behavior. It emits predictable `content_scripts/*` outputs.
...
Why this matters: Extension.js remounts content scripts during development. Without a cleanup function, you can duplicate UI, event listeners, observers, and timers.
...
That means `scripts/` is not a generic folder for loose JavaScript files:
...
{/* VIDEO_SPEC id=content-script-hmr grammar=D→E ~12s loop  [family: hmr]  [SHIPPED: video/scenes/content-script-hmr.scene.mjs]
  Needs: mock transcript `cmd-dev` (reused — the dev session is staged off
    camera); fixture mock/takes/content-script-hmr/src/content/scripts.js (the
    `content` template's file, verbatim), seeded by the scene over
    src/content/scripts.js, which carries a .pristine twin; page
    mock/site/content-host.html — a replica of example.com, the host page this
    template's own e2e tests inject into, running the template's `initial()`
    body itself because mock mode has no extension to inject it; usesEditor
    src/content/scripts.js at 36:23 — the opening quote of the widget title;
    stage(): dev running, host page open, editor on the file
  D editor.replaceToEndOfLine("'Hello from your content script'") mark:edited
    editor.save()                                   mark:saved
  E browser.focus(); the widget updates on the page without a tab refresh — the
    wrapper re-injected the fresh build             mark:updated → hold read → end
  honest limit: the address bar reads localhost, because mock pages are served
    over http and the browser actor has no navigate surface. The widget, the
    host page and the update are right; the URL is not the one a viewer would
    have. A take against the real example.com is real-mode-only (A1/A3)
  post: trim start→end, posterAt updated, ramps [start→edited 1.35x], loop
  run: node video/record.mjs content-script-hmr --mock */}
...
In **development mode**, content script JS filenames include a short hash suffix (for example, `content-0.abcd1234.js`). This forces the browser to load a fresh `chrome-extension://` URL after each rebuild.
