...
Extension.js does not replace or wrap browser storage APIs — it only compiles the code that calls them. The choice of storage area still matters though, because it shapes service-worker reliability, cross-browser parity, and how painful future schema migrations become.
