...
The schema path must resolve to a file inside the extension directory. Before the browser launches, Extension.js flags a `storage.managed_schema` path outside it as a load blocker. Chrome refuses the whole extension over a missing managed schema.
