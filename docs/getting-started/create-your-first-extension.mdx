...
`src/manifest.json` and `src/background.js`. The next two steps replace those two files.
...
Create `src/service_worker.js`, and delete the `src/background.js` the scaffold shipped so nothing else claims the background entry:
