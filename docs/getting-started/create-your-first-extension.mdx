...
You will build an Omnibox (address bar) shortcut: type `gh` in the browser address bar, enter a query, and land on GitHub search results. Along the way you will wire a `manifest.json` and handle input in a background service worker. You will also use the dev loop (`create` → `dev` → `build`) — the same loop you will reach for on every project after this one.
...
Every extension starts with a manifest file. It defines metadata, permissions, and the files the browser loads at runtime. Based on the [plan above](#the-plan), set the `gh` shortcut and add a service worker for user events.
