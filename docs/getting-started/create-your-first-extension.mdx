...
{/* VIDEO_SPEC id=first-extension grammar=A→B→C ~12s loop  [SHIPPED: video/scenes/first-extension.scene.mjs]
  Needs: mock transcript `first-extension` (the step-4 `npm run dev` boot);
    mock page `github-search.html`, a replica of the Extension.js welcome page
    a first dev run opens — the finished github-search owns no page of its own,
    so that welcome page is the whole of its payoff; mock `npm` shim so the
    spelling this page documents resolves.
    not-shootable: the `gh` omnibox flow. No mode has a browser keyboard actor,
    so real mode does not cover it either — the cut ends on the loaded extension.
    A single take carries one typed command and one transcript, so the `create`
    step cannot share this cut with the dev boot.
  A terminal.type('npm run dev')                    mark:command-submitted
  B dev boot streams ~5s (ramp 2.5x in post)        mark:build-streaming-done
  C browser.open() on the welcome page naming GitHub Search
                                                    mark:browser-visible → hold read → end
  post: trim start→end, posterAt browser-visible, loop
  run: node video/record.mjs first-extension --mock */}
