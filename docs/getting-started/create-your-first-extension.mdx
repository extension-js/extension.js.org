...
> The interface you are building in this guide.
...
Every extension starts with a manifest file. It defines metadata, permissions, and runtime files. Based on the [plan above](#the-plan), you set the `gh` shortcut and add a service worker for user events.
...
- `omnibox.keyword`: The keyword `gh` triggers an omnibox event when the user types it.
- `background.service_worker`: Listens to the omnibox event and handles it.
