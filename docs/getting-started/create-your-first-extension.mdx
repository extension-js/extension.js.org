...
You will build an Omnibox (address bar) shortcut: type `gh` in the browser address bar, enter a query, and land on GitHub search results. Along the way you will wire a `manifest.json` and handle input in a background service worker.

You will also practice the dev loop (`create` → `dev` → `build`) that you will reach for on every project after this one.
