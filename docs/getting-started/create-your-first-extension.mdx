...
You will build an omnibox (address bar) shortcut: type `gh` in the browser address bar, enter a query, and land on GitHub search results. Along the way you will wire a `manifest.json`, handle input in a background service worker, and use the dev loop (`create` → `dev` → `build`). This is the same loop you will reach for on every project after this one.
