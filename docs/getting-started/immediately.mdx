...
There are two common entry points: point `extension dev` at a local folder you cloned, or point it at a GitHub URL and let it fetch the sample for you. The sections below cover both paths, what Extension.js treats as a project root, and how it handles ambiguous inputs like a bare sample name.
...
- **Local path:** treated as a path relative to the current working folder (for example `./my-extension` or `../samples/page-redder` after you clone a repo).
