...
You can run MDN WebExtensions examples in Edge by passing the GitHub tree URL and enabling the polyfill. The polyfill maps Firefox-style `browser.*` API calls to Chrome-style `chrome.*` calls. This lets Firefox-oriented extensions work in Chromium-based browsers.
