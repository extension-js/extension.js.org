...
You have two common entry points: point `extension dev` at a local folder you cloned, or point it at a GitHub URL and let it fetch the sample for you. The sections below cover both paths. You will learn what Extension.js treats as a project root and how it handles ambiguous inputs like a bare sample name.
...
You can run examples from MDN WebExtensions in Edge by passing the GitHub tree URL and enabling the polyfill. The polyfill is a compatibility layer that maps Firefox-style `browser.*` API calls to `chrome.*`, so Firefox-oriented extensions work in Chromium-based browsers.
