...
Extension.js supports Microsoft Edge natively.
...
You can run MDN WebExtensions examples in Edge by passing the GitHub tree URL and enabling the polyfill. The polyfill is a compatibility layer that maps Firefox-style `browser.*` API calls to Chrome-style `chrome.*` calls, so Firefox-oriented extensions work in Chromium-based browsers.
