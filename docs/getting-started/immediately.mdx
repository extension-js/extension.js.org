...
{/* VIDEO_PLACEHOLDER_TODO: Show Apply CSS extension fetched and running with `--browser=edge --polyfill=true`. Intentionally hidden until a real demo video is available so it does not render in docs or appear in llms artifacts. */}
