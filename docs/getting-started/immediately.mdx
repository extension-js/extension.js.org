...
You can run MDN WebExtensions examples in Edge. Pass the GitHub tree URL and enable the polyfill (a compatibility layer). The polyfill maps Firefox-style `browser.*` API calls to Chrome-style `chrome.*` calls, letting your Firefox-oriented extension work in Chromium-based browsers.
