...
You can run MDN WebExtensions examples in Edge by passing the GitHub tree URL and enabling the polyfill. The polyfill maps Firefox-style `browser.*` API calls to Chrome-style `chrome.*` calls, letting your Firefox-oriented extension work in Chromium-based browsers.
