...
{/* VIDEO_SPEC id=templates-gallery grammar=A→B→C ~14s loop  [SHIPPED: video/scenes/templates-gallery.scene.mjs]
  Needs: two mock transcripts, resolved per subcommand from the typed argv —
    `templates-gallery.create` (create --template=new-react) and
    `templates-gallery.dev` (dev boot); the mock page is already the react
    template's newtab, so no new fixture
  A terminal.type('npx extension@latest create my-extension --template=new-react')
                                                    mark:command-submitted
  B scaffold streams, then terminal.type('npx extension@latest dev') and the
    boot streams ~9s (ramps 2.4x typing / 2.2x work in post)
                                                    mark:build-streaming-done
  C browser.open() the template's UI running — pick a slug, get a working
    extension, hold read                            mark:browser-visible → end
  post: trim start→end, posterAt browser-visible, loop
  run: node video/record.mjs templates-gallery --mock */}
