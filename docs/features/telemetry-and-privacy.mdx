...
Extension.js collects a tiny amount of anonymous telemetry to understand which commands are used and which fail. Extension.js never collects source code, file paths, URLs, or project content.
...
- **Sampling** — `command_executed` is sampled at 20 % by default. Override with `EXTENSION_TELEMETRY_SAMPLE_RATE` (0.0–1.0). Failures are never sampled.
- **Per-run cap** — at most **3 events** per CLI process. Override with `EXTENSION_TELEMETRY_MAX_EVENTS`.
- **Debounce** — duplicate `(event, command, success)` tuples within 60 s are dropped. Override with `EXTENSION_TELEMETRY_DEBOUNCE_MS`.
...
CI environments follow the same rules — set `EXTENSION_TELEMETRY=0` in your CI environment to turn telemetry off across the board.
