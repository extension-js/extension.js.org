...
| `command_failed`   | 100 % (the CLI always sends failures) | `command`, `success: false`, `version`  |
...
- **Debounce** — the CLI drops duplicate `(event, command, success)` tuples within 60 s. Override with `EXTENSION_TELEMETRY_DEBOUNCE_MS`.
...
- IP addresses (the CLI sets `$ip` to `null` on every payload)
