...
- **Debounce** — Extension.js drops duplicate `(event, command, success)` tuples within 60 s. Override with `EXTENSION_TELEMETRY_DEBOUNCE_MS`.
...
Telemetry is **opt-out**. On the first run where none of the overrides above apply, Extension.js prints a one-line notice explaining how to disable telemetry. It also records an `enabled` consent marker so the notice does not repeat.
...
Extension.js appends every event it considers sending (whether or not it ships) to `events.jsonl` next to the consent file. Inspect it any time. Delete it freely.
