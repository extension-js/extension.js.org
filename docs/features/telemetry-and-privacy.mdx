...
Telemetry is **opt-out**. On the first run where no overrides apply, the CLI prints a one-line notice explaining how to disable telemetry. It also records an `enabled` consent marker so the notice does not repeat.
