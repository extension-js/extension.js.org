...
The `create` command may add two more properties: `template` (the sanitized template name) and `source` (where the template came from). The `create` command is also exempt from sampling, so template usage numbers stay accurate.
