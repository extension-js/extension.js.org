...
Extension.js collects minimal, anonymous telemetry from CLI and build workflows so the framework can improve without turning your projects into tracking surfaces.
...
- coarse project-shape signals (package manager, framework family, TypeScript usage, monorepo usage, and broad ecosystem booleans such as `has_next_dependency`)
...
- Are you only experimenting locally, or repeatedly producing release artifacts?
- How often do you use multi-browser release flows?
- Are you adopting CI-style and machine-readable workflows?

## Workflow cohorts for dashboards

Extension.js also emits a normalized `workflow_profile` event to make aggregate reporting easier in tools such as PostHog (an analytics platform).

Extension.js assigns each run to one cohort:
...
That contract is intentionally narrower than product-side operational analytics. If a field could identify a specific repository, user, or company, it does not belong in Extension.js CLI telemetry.
...
- Review [Global flags](/docs/workflows/global-flags) for `--no-telemetry`.
