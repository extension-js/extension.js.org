...
Extension.js splits HTML pages only. The default `optimization.splitChunks` gives pages two stable siblings, `shared/framework.js` for the UI framework runtime (React, Preact, Vue, Svelte, Solid or Lit) and `shared/commons.js` for anything two or more pages import. The names carry no hash, so a manifest entry or a public asset can keep pointing at them. Every other surface keeps exactly one file, because the manifest names a single script for a background worker, a content script or an injected script, and those surfaces never load a sibling.
