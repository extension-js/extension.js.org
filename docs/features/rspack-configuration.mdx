...
Solve custom bundler needs — loaders (file-type transforms), plugins (build-time extensions), and aliases (import shortcuts) — without forking the default setup. Extension.js builds with [Rspack](https://rspack.dev), a Rust-based JavaScript bundler (`@rspack/core`), and lets you extend the generated configuration directly.
