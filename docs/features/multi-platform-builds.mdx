...
{/* VIDEO_SPEC id=build-matrix grammar=A→B ~14s loop  [SHIPPED: video/scenes/build-matrix.scene.mjs]
  Needs: mock transcript `build-matrix` (three sequential build summaries, then a
    dist/ tree showing chrome, edge, and firefox side by side)
  A terminal.type('extension build --browser=chrome,edge,firefox')
                                                    mark:command-submitted
  B three build streams ~6s (ramp 2.5x in post); payoff stays in the terminal:
    the three dist/<browser> trees                  mark:matrix-listed → hold read → end
  post: trim start→end, posterAt matrix-listed, loop
  run: node video/record.mjs build-matrix --mock */}
