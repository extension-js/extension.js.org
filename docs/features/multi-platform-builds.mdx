---
title: 'Multi-platform builds for Chrome, Edge, and Firefox'
description: 'Build and package the same extension for Chrome, Edge, and Firefox without separate build scripts. Output artifacts to dist and generate zip packages.'
icon: 'box'
keywords:
  [
    'extension.js',
...
| `--polyfill`            | Polyfills `browser.*` API calls for Chromium-family targets. |
...
- **Build per target in CI (continuous integration)**: Treat each browser output as an independent artifact.
