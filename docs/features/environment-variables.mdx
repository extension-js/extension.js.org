...
Browser-scoped files such as `.env.chrome` or `.env.chrome.development` are **not** used for this preload step. Use plain `process.env` set by your shell or CI pipeline, or rely on bundle-time env (above) for `EXTENSION_PUBLIC_*` injection into extension code.
