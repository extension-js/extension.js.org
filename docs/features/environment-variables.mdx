...
If no matching file exists next to the project and no `.env.defaults` is found there, Extension.js repeats the same search from the **nearest workspace root**. The workspace root is the directory that contains `pnpm-workspace.yaml` when walking up from the project. This lets monorepo extensions share root env files for **bundle** injection.

**Monorepo constraint:** Workspace fallback only runs when that `pnpm-workspace.yaml` marker exists on an ancestor. Pure npm or Yarn workspaces that rely only on `package.json` `"workspaces"` without `pnpm-workspace.yaml` will **not** trigger this automatic root lookup. In that case, keep env files beside the extension package, add a `pnpm-workspace.yaml` at the repo root if appropriate, or use another sharing strategy.
...
When you build, Extension.js replaces `$EXTENSION_PUBLIC_API_KEY` in the output HTML.
