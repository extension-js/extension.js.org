...
Browser extensions ship code that runs on your users' machines. That makes the line between configuration and secrets sharper than in a typical web app. Anything you bundle into JavaScript, HTML, or the compiled `manifest.json` can be inspected.
...
<Note>
  ...
  not a full cascade through every file.
</Note>
...
Browser extensions run on your users' machines. Any value you bundle into JavaScript, HTML, or the compiled `manifest.json` can be inspected by anyone who installs your extension. Treat the bundle as public.
