...
When building your extension, the compiler picks **one** env file from your extension package directory by first match in this order:
...
If no matching file exists next to the project and no `.env.defaults` is found there, Extension.js repeats the same search from the **nearest workspace root**. The workspace root is the directory containing `pnpm-workspace.yaml`, found by walking up from the project. This keeps monorepo extensions able to share root env files for **bundle** injection.
...
Extension.js does **not** use browser-scoped files such as `.env.chrome` or `.env.chrome.development` for this preload step. Instead, use plain `process.env` set by your shell or CI (continuous integration) pipeline. You can also rely on bundle-time env (above) for `EXTENSION_PUBLIC_*` injection into extension code.
