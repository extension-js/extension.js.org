...
Before 4.1.19, the bundler replaced `import.meta.url` with the source file's `file://` path. That path put folder names from the build machine into the shipped bundle. From 4.1.19, Extension.js resolves a bare `import.meta.url` at runtime instead:

| Where the code runs                                         | Value of `import.meta.url`                          |
| ----------------------------------------------------------- | --------------------------------------------------- |
| Extension pages and workers (service worker, popup, options) | The URL of that page or worker                      |
