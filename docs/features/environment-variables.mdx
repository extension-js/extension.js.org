...
**Monorepo constraint:** Workspace fallback only runs when a `pnpm-workspace.yaml` marker exists on an ancestor. Pure npm or Yarn workspaces that rely only on `package.json` `"workspaces"` without `pnpm-workspace.yaml` do **not** get this automatic root lookup. Keep env files beside the extension package, or add a `pnpm-workspace.yaml` at the repo root.
