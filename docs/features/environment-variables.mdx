...
If no matching file exists next to the project and no `.env.defaults` is found there, Extension.js repeats the same search from the **nearest workspace root**. The workspace root is the folder containing `pnpm-workspace.yaml`, found by walking up from the project. This lets your monorepo extensions share root env files for **bundle** injection.

**Monorepo constraint:** Workspace fallback only runs when a `pnpm-workspace.yaml` marker exists on an ancestor. Pure npm or Yarn workspaces that rely only on `package.json` `"workspaces"` do **not** get this automatic root lookup. To enable it, keep env files beside your extension package or add a `pnpm-workspace.yaml` at the repo root.
...
Extension.js does **not** use browser-scoped files such as `.env.chrome` or `.env.chrome.development` for this preload step. Instead, use plain `process.env` set by your shell or CI (continuous integration) pipeline. You can also rely on bundle-time env (above) to inject `EXTENSION_PUBLIC_*` values into extension code.
