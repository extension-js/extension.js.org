...
When building your extension, Extension.js picks **one** env file from your extension package directory by first match in this order:
...
If no matching file exists next to the project, Extension.js repeats the same search from the **nearest workspace root**. The workspace root is the directory containing `pnpm-workspace.yaml`, found by walking up from the project. This lets monorepo extensions share root-level env files for **bundle** injection.
...
Extension.js does **not** use browser-scoped files such as `.env.chrome` or `.env.chrome.development` for this preload step. Instead, set values through plain `process.env` in your shell or CI (continuous integration) pipeline. You can also rely on compile-time environment loading (described above) for injecting `EXTENSION_PUBLIC_*` values into extension code.
...
This split exists because the config loads before Extension.js knows the browser target, while the bundler runs with the active browser and mode.
...
| `EXTENSION_TELEMETRY_DEBOUNCE_MS`  | Dedup window for identical event combinations (ms)   | `60000`   |
