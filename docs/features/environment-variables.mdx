...
{/* VIDEO_SPEC id=env-reload grammar=D→E ~14s loop  [SHIPPED: video/scenes/env-reload.scene.mjs]
  Needs: mock transcript `cmd-dev` (reused); fixture .env + .env.pristine holding
    EXTENSION_PUBLIC_HEADING, whose value is the sentence the mock page's #heading
    already shows — the page renders no URL, so an API-URL value has nothing to
    prove itself against; usesEditor .env line 2 column 1 (whole KEY=value retyped,
    unquoted so no auto-closing pair lands mid-take); stage(): dev running,
    browser open, editor on .env — clip opens mid-workflow
  D editor.replaceToEndOfLine('EXTENSION_PUBLIC_HEADING=Welcome to your staging build.')
                                                    mark:edited
    editor.save()                                   mark:saved
  E browser.focus(); rebuild picks the new value and the page shows it in place
    (mock: hmr({heading}) drives the swap)          mark:updated → hold read → end
  post: trim start→end, posterAt updated, ramps [start→edited 1.35x], loop
  run: node video/record.mjs env-reload --mock */}
