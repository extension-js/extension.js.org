...
`extension.config.js` / `.mjs` / `.cjs` runs in Node. Before evaluating the file, Extension.js preloads a **small** set of files into `process.env` so the config can read them:
