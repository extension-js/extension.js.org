...
Extension.js does **not** use browser-scoped files such as `.env.chrome` or `.env.chrome.development` for this preload step. Instead, set values through plain `process.env` in your shell or CI (continuous integration) pipeline. You can also rely on bundle-time env (described above) for injecting `EXTENSION_PUBLIC_*` values into extension code.
