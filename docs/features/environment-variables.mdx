...
If no matching file exists next to the project and no `.env.defaults` is found there, Extension.js repeats the same search from the **nearest workspace root**. The workspace root is the directory that contains `pnpm-workspace.yaml` when walking up from the project. That keeps monorepo extensions able to share root env files for **bundle** injection.
...
During the build, Extension.js replaces `$EXTENSION_PUBLIC_API_KEY` in the output HTML.
