...
Browser extensions ship code that runs on the user's machine. That makes the line between configuration and secrets sharper than in a typical web app: anything bundled into JavaScript, HTML, or the compiled `manifest.json` can be inspected.
...
Browser extensions run on the user's machine. Any value bundled into JavaScript, HTML, or the compiled `manifest.json` can be inspected by anyone who installs your extension. Treat the bundle as public.
