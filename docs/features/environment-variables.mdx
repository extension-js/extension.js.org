...
If no matching file exists next to the project and no `.env.defaults` is found there, Extension.js repeats the same search from the **nearest workspace root**. The workspace root is the closest ancestor directory containing `pnpm-workspace.yaml`. This lets monorepo extensions share root env files for **bundle** injection.

**Monorepo constraint:** Workspace fallback only runs when a `pnpm-workspace.yaml` marker exists on an ancestor. Pure npm or Yarn workspaces without that file will **not** get this automatic root lookup. In that case, keep env files beside the extension package, add a `pnpm-workspace.yaml` at the repo root, or use another sharing strategy.
