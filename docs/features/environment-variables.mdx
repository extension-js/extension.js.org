...
Manage API hosts, keys, and browser-specific values cleanly across environments. Extension.js uses **two different** environment loading paths: one for **compiled extension bundles** (browser/mode aware) and a **narrower** one while loading **`extension.config.*`** in Node. Both paths matter depending on where you read variables.
