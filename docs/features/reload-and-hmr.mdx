...
description: "Understand which file changes trigger HMR, hard extension reloads, or full restarts during development so you can iterate faster with Extension.js."
icon: "gauge-high"
keywords: ["extension.js", "extensions", "chrome", "firefox", "browser", "manifest", "hmr", "content script"]
---
...
Extension.js uses HMR when code can update without restarting extension runtime wiring.
...
Extension.js applies browser-specific hard reload mechanisms internally (Chromium and Firefox flows differ under the hood), but unifies behavior at the CLI/user level.

> **Why do content script filenames include hashes in dev?** Chrome aggressively caches `chrome-extension://` resources. A stable filename like `content-0.js` can serve stale code even after a full extension reload. Extension.js appends a short build hash (e.g., `content-0.abcd1234.js`) in development so each rebuild produces a fresh URL that bypasses the cache.
...
- Review off-manifest asset flows in [special folders](/features/special-folders).
- Understand structural update outcomes in [dev update behavior](/workflows/dev-update-behavior).
