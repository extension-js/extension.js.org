...
<Note>
  Scripts you inject programmatically with `chrome.scripting.executeScript`
  from `/scripts/*` are **replayed** on edit. When you change a file under
  `/scripts/`, Extension.js re-runs the same injection on the same tab and
  tears down the previous mount — so dynamically injected scripts update live,
  the way declarative `content_scripts` do, instead of leaving stale DOM until
  you manually re-trigger the call. This is a dev-only convenience.
</Note>
