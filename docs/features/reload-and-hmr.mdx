...
>
> Chrome aggressively caches `chrome-extension://` resources. A stable filename like `content-0.js` can serve stale code even after a full extension reload. Extension.js appends a short build hash (for example, `content-0.abcd1234.js`) in development. Each rebuild produces a fresh URL that bypasses the cache. Production builds use clean names.
