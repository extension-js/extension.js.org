...
{/* VIDEO_SPEC id=reload-fanout grammar=A→B ~12s loop
  Needs: mock transcript `reload-fanout` (dev session up on a project where
    background and a content script both import shared/api.ts)
  A editor edits shared/api.ts and saves           mark:saved
  B one classified instruction fans out: the CLI prints
    "service_worker + content_script (shared/api.ts)", the SW restarts AND the
    content script re-injects, from a single save  mark:fanned-out → hold read → end
  post: trim start→end, posterAt fanned-out, loop
