...
Extension.js applies browser-specific hard reload mechanisms internally (Chromium and Firefox flows differ under the hood), but Extension.js unifies behavior at the CLI/user level.

> **Why do content script filenames include hashes in dev?** Chrome aggressively caches `chrome-extension://` resources, so a stable filename like `content-0.js` can serve stale code even after a full extension reload. Extension.js appends a short build hash (for example, `content-0.abcd1234.js`) in development so each rebuild produces a fresh URL that bypasses the cache. Production builds use clean names.
