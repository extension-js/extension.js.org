...
Each dev reload is announced with a single context label built by the dev server (naming what changed and which surface it affects). The same label appears in CLI stdout, in the page devtools console, and in the devtools pill, so you can correlate a reload across all three without guessing.
