...
<Frame>
  <iframe
    ...
    src="https://www.youtube-nocookie.com/embed/qjXVWBrLq74?rel=0"
...
<Frame>
  <iframe
    className="w-full aspect-video rounded-xl"
    src="https://www.youtube-nocookie.com/embed/q7BRUgMB7O4?rel=0"
...
The tiers above are the Chromium and Gecko reload paths. A Safari dev session updates by a different route. Each save rebuilds `dist/safari`, resyncs the app's resources with an incremental `xcodebuild`, and reloads the extension in Safari. Measured on Safari 26.5.2, that is roughly 4 seconds from save to reloaded extension, with no HMR tier. See [Building Safari extensions](/docs/browsers/safari).

{/* TODO(version): name the release that ships the Safari reload loop. Merged as 0508e40cd, unreleased as of 4.1.19. */}
