...
Not every manifest field rewrites to a folder with the same source name. Extension.js bases the emitted path on the browser-facing extension contract, not on the original authoring path.
...
The resolver de-duplicates warnings per file transform pass to reduce noise.
...
- Extension.js preserves absolute OS file paths instead of treating them like extension output paths.
