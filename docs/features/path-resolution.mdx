...
Keep runtime paths working when files move, entries are renamed, or browser targets change. Extension.js rewrites supported paths across manifest fields and extension API usage so compiled assets resolve correctly inside `dist/<browser>`.
...
- **JS/TS path rewriting**: rewrites static path literals passed to supported extension APIs (for example `runtime.getURL`, `tabs.update`, `scripting.*`, `action.*`, `sidePanel.*`).
...
Not every manifest field rewrites to a directory with the same source name. Extension.js bases output paths on what the browser expects at runtime, not on your original source file paths.
...
Extension.js groups duplicate warnings to reduce noise.
