...
Both passes anchor on the project root, the directory that contains your `package.json`. That is why `pages/`, `scripts/`, and `public/` inputs resolve only from that directory, never from `src/`, even when `src/manifest.json` defines the extension. See the root rule in [Special folders](/docs/features/special-folders#where-special-folders-live).
...
| `chrome_url_overrides.newtab`  | `/src/newtab.html`         | `chrome_url_overrides/newtab.html` |
