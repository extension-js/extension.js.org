...
All Chromium-based and Gecko-based targets work on Windows, macOS, and Linux. Safari is the exception: running it needs macOS with full Xcode. `extension build --browser=safari` still produces the complete `dist/safari` bundle on Windows and Linux, skipping the Xcode packaging step with a warning.
...
| Apple Safari                                     |   ⚠️    |  ✅   |  ⚠️   | macOS: `build`/`dev` only, needs Xcode, no reload. Windows/Linux: `build` produces `dist/safari`, Xcode packaging skipped |
