...
All Chromium-based and Gecko-based targets work on Windows, macOS, and Linux. Safari is the exception: running it needs macOS with full Xcode. A Safari dev session there streams logs and reloads on save, like the other targets. `extension build --browser=safari` still produces the complete `dist/safari` bundle on Windows and Linux, skipping the Xcode packaging step with a warning.
...
| Apple Safari                                     |   ⚠️    |  ✅   |  ⚠️   | macOS: `build` and `dev`, needs Xcode, with logs and reload. No `preview` or `start`. Windows/Linux: `build` produces `dist/safari`, Xcode packaging skipped |
