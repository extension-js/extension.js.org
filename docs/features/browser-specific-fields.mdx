...
Do not write `edge:key`. Edge Add-ons rejects any package whose manifest contains `key` at all, so the field has no use in an Edge build. Partner Center assigns the extension id instead. A production Edge build drops `key` and prints one line saying why, and a development build keeps it, where a stable id is useful and no store is involved.
