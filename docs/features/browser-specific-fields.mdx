...
Keys and permissions that Safari does not implement are dropped from the Safari build automatically, and the build prints one line per dropped key, naming the key and why it went. `content_scripts[].world` is kept, because Safari has supported it since Safari 18. See [Building Safari extensions](/docs/browsers/safari).
