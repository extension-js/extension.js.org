...
When a prefixed key matches the active target, Extension.js strips the prefix and writes the key to the output manifest.
...
- **Build per target in CI (continuous integration)**: Generate and verify each browser output (`dist/<browser>`) to catch compatibility regressions early.
