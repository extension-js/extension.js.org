...
{/* VIDEO_SPEC id=public-folder grammar=A→B ~12s loop  [SHIPPED: video/scenes/public-folder.scene.mjs]
  Needs: mock transcript `public-folder` (one Chromium build card whose dist tree
    carries public/logo.png as a bare `logo.png` at the output root and
    public/fonts/inter.woff2 as `fonts/inter.woff2`). There is no copied-assets
    section: `build` prints no source→output pairing anywhere, so the tree is the
    only surface the copy shows on. Fixture project needs those two public files.
  A terminal.type('extension build')                mark:command-submitted
  B build streams ~5s (ramp 2.0x in post); payoff is the tree — the public files
    sit at the output root under their own names and keep their subfolder, while
    every compiled artifact is under a folder the build named
                                                    mark:paths-shown → hold read → end
  post: trim start→end, posterAt paths-shown, loop
  run: node video/record.mjs public-folder --mock */}
