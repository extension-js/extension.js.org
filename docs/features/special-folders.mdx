...
Extension.js wraps every file inside `scripts/` with a browser content-script mount runtime. If you place a Node.js-only file there (for example, a CLI launcher or build helper), the wrapper breaks the file because the shebang is no longer on line 1 or Node-only APIs are unavailable in the browser context.
