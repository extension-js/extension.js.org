...
The manifest does not directly declare many extension files — iframe pages, scripts you inject dynamically with `chrome.scripting.executeScript`, and static vendor assets. You may also need companion extensions during development. Special folders make these first-class in the build pipeline.
...
Extension.js wraps every file inside `scripts/` with a browser content-script mount runtime. If you place a Node.js-only file there (for example, a CLI launcher or build helper), the wrapper breaks it. The shebang is no longer on line 1, and Node-only APIs are unavailable in the browser context.
