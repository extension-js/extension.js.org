...
- The `start` command runs `build` then `preview` internally. Extension.js applies settings in `commands.start`, but you can also put build-specific settings in `commands.build` and browser-launch settings in `commands.preview`.
