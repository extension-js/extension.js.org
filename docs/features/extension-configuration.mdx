...
Share browser defaults, command options, and build settings across your team. Stop repeating CLI flags. Extension.js reads `extension.config.js` (or `.mjs` / `.cjs`) from your project root. It applies settings to every command and the bundler configuration.
