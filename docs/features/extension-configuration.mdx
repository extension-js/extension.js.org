...
- The `start` command runs `build` then `preview` internally. Extension.js applies settings from `commands.start`. You can also put build-specific settings in `commands.build`. For finer control over browser-launch settings, use `commands.preview`.
