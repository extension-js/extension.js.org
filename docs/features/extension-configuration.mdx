...
Share browser defaults, command options, and build settings across your team without repeating CLI flags. Extension.js reads `extension.config.js` (or `.mjs` / `.cjs`) from your project root and applies your settings to `dev`, `start`, `preview`, `build`, and bundler config.
...
- The `start` command runs `build` then `preview` internally. Extension.js applies settings from `commands.start`, but you can also put build-specific settings in `commands.build`. For finer control over browser-launch settings, use `commands.preview`.
