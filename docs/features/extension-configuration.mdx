...
Share browser defaults, command options, and build settings across your team. Stop repeating CLI flags. Extension.js reads `extension.config.js` (or `.mjs` / `.cjs`) from your project root and applies settings to every command and the bundler.
...
- CLI flags override config values. The full merge order is: CLI flags > per-command config (`commands.dev`, etc.) > per-browser config (`browser.chrome`, etc.) > top-level defaults.
