...
Share browser defaults, command options, and build settings across your team without repeating CLI flags. Extension.js reads `extension.config.js` (or `.mjs` / `.cjs`) from your project root. It applies settings to `dev`, `start`, `preview`, `build`, and bundler config.
...
- In monorepos, if Extension.js finds no project-local `.env*` file, it falls back to the nearest workspace root (the folder containing `pnpm-workspace.yaml`) and applies the same order there.
...
- The `start` command runs `build` then `preview` internally. Extension.js applies settings from `commands.start`, but you can also put build-specific settings in `commands.build`. Use `commands.preview` for browser-launch settings when you need finer control.
