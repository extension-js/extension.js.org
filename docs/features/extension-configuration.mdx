...
- The `start` command runs `build` then `preview` internally. Settings in `commands.start` are applied, but you can also put build-specific settings in `commands.build`. Use `commands.preview` for browser-launch settings when you need finer control.
