---
title: "Extension configuration reference"
description: "Configure browser defaults, per-command options, companion extensions, package transpile lists, and Rspack hooks in a single extension.config.js file."
icon: "puzzle"
keywords: ["extension.js", "extensions", "chrome", "firefox", "edge", "browser", "rspack", "devtools"]
---
...
- The `start` command runs `build` then `preview` internally. Settings in `commands.start` are applied, but you can also put build-specific settings in `commands.build` and browser-launch settings in `commands.preview` for finer control.
...
- Learn more about [Browsers Available](/browsers/browsers-available).
- Learn more about [Rspack Configuration](/features/rspack-configuration).
