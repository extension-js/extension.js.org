...
<Steps>
  ...
  <Step title="Enter the credentials">
    ...
    the guided setup dialog. On the direct track, set the environment variables
    in your own CI, or keep them in a `.env.submit` file you write yourself.
  </Step>
  ...
  <Step title="Submit">
    ...
    against the
    [platform API](https://docs.extension.dev/automate/api?utm_source=extension-js-org&utm_medium=sponsor&utm_campaign=docs-seam),
...
On the direct track, you hold the credentials yourself. A `.env.submit` file is
the usual per-project credential file for that track. Keep it out of git.
...
| Submission command          | The release flow on extension.dev, or its API                       | Each store's own API, called by you          |
| Review tracking             | Dashboard status, polled automatically                              | You poll each store yourself                 |
