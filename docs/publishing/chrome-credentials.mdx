...
values. If you would rather not carry them yourself, hand them to
extension.dev once per project and let the platform run the submission. It
accepts a service account key instead of an OAuth client, which is the better
default because a service account does not expire with a person's session. See
[Chrome Web Store credentials](https://docs.extension.dev/publish/chrome?utm_source=extension-js-org&utm_medium=sponsor&utm_campaign=docs-seam).

The rest of this page is the manual path: create the OAuth client yourself and
...
it wherever it is stored. Saved platform secrets are write-only, so rotation
