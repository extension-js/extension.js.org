...
## The add-on ID and the first submission

The add-on ID, which AMO calls the GUID, identifies your add-on across every
submission. It is also the value that goes in
`browser_specific_settings.gecko.id`. Extension.js reads that key when it is
present, and never writes one for you.

The rules depend on the release channel:
...
AMO has required the key for every new submission since 2025-11-03. You do
not have to discover the rejection at upload time. A production build for a
Firefox target warns when the resolved Gecko manifest lacks
...
| Add-on GUID     | `FIREFOX_EXTENSION_ID` |
| Release channel | `FIREFOX_CHANNEL`      |
