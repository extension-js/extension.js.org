...
<Warning>
  ...
  `chrome.scripting.executeScript` on the active tab or `captureVisibleTab`) will
  behave differently than a real click. The result includes `gesture: false`,
  and a `warning` when your manifest declares `activeTab`. When you need true
  click fidelity, use chrome-devtools-mcp's `trigger_extension_action` (below).
</Warning>
