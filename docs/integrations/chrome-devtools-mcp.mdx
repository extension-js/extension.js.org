...
description: "Run Google's chrome-devtools-mcp with the Extension.js MCP server so AI agents get Chrome debugging plus cross-browser extension control from one config."
keywords:
  [
    "chrome-devtools-mcp",
    "extension.js",
...
{/* VIDEO_SPEC id=mcp-agent grammar=A→B ~13s loop  [SHIPPED: video/scenes/mcp-agent.scene.mjs]
  Needs: mock transcript `mcp-agent` (authored, every byte derived from Claude
    Code 2.1.220's own list renderer) and a `mock/bin/claude` shim forwarding
    argv to mock/player.mjs — same shape as mock/bin/npm, refusing anything but
    `mcp list`. MOCK-ONLY, and the scene's stage() enforces it: a real
    `claude mcp list` prints the maintainer's own servers and spawns every one
    of them to health-check it.
  NOT the agent session the first draft asked for. That draft was wrong twice:
    the player replays any transcribed text, tool calls included, so replay was
    never the blocker; and the payoff cannot be filmed at all, because an
    agent-driven reload looks pixel-for-pixel like a hand-driven one, leaving
    beat C unable to show "no human touched the browser". A model's output is
    also captured once and never re-shootable, which is what the rest of this
    backlog is built to avoid.
  A terminal.type('claude mcp list')               mark:command-submitted
  B health check, then both servers ✔ Connected — one config, no collision
                                                   mark:servers-connected → hold read → end
  post: trim start→end, posterAt servers-connected, ramp the health check, loop
  run: node video/record.mjs mcp-agent --mock */}
...
| -------------------------- | --------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `install_extension`        | `extension_dev`, `extension_start`      | Loading is tied to a real build + hot reload, not a static folder                                                                                                                     |
...
| n/a                        | `extension_open` (`surface: "command"`) | Replays a `chrome.commands.onCommand` keyboard-shortcut handler, no chrome-devtools-mcp equivalent                                                                                    |
...
Since this page already recommends running both servers, let chrome-devtools-mcp own genuine-gesture browser driving and let Extension.js own the build-aware, cross-browser replay, with no need to duplicate it.
