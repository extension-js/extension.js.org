...
shadcn/ui works inside an extension the same way it works in a web app, provided the project is already on a Tailwind-enabled React setup. Components live in your source tree (not `node_modules`), so you keep full control over styling and behavior per surface.
...
- Extension.js emits content-script styles through its CSS pipeline, but those styles can still conflict with the host page.

If you need stronger isolation for content scripts, mount your UI inside a Shadow DOM root and scope your class names intentionally.
