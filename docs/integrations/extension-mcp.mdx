...
{/* VIDEO_SPEC id=mcp-change-verify grammar=A→B ~14s loop
  Needs: an MCP client session against a running extension_dev session
  A the client asks for a change, the dev session rebuilds on save
  B the client verifies the result in the browser through extension_logs
    and extension_eval, no human touches the browser → hold read → end
