...
{/* VIDEO_SPEC id=mcp-change-verify grammar=A→B ~14s loop
  Needs: mock transcript `mcp-change-verify` (claude -p print-mode report) and
    the extended claude shim; the extension_dev session runs off camera
  A terminal runs `claude -p "Rename the new tab heading ..."`  mark:command-submitted
  B the agent edits, the session rebuilds on save, and its printed report shows
    the change verified through extension_reload and extension_inspect, no human
    touching the browser              mark:change-verified → hold read → end
  run: node video/record.mjs mcp-change-verify --mock */}
