...
For the JSON resource details, see [JSON](./json.mdx).
...
- Declare schema-backed enterprise storage in [manifest.json](./manifest-json.mdx).
- Review resource handling in [JSON](./json.mdx).
- Move state between contexts with [Messaging](./messaging.mdx).
