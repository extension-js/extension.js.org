...
{/* VIDEO_PLACEHOLDER: Show switching between managed ephemeral, persistent, and explicit profile modes in development. */}
![Video placeholder: profile mode selection](https://placehold.co/960x540?text=Video+placeholder)
...
{/* VIDEO_PLACEHOLDER: Show profile directories created under `dist/extension-js/profiles` and reused when `persistProfile` is enabled. */}
![Video placeholder: managed profile lifecycle](https://placehold.co/960x540?text=Video+placeholder)
...
{/* VIDEO_PLACEHOLDER: Show CLI `--profile` override applied in `dev`, then reused in `start`/`preview`. */}
![Video placeholder: cli profile override](https://placehold.co/960x540?text=Video+placeholder)
