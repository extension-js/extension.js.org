...
{/* VIDEO_DEMO_SOON: Show env file resolution order for one build target and mode, then confirm selected values in output. */}
![Video demo soon: env file priority resolution](https://placehold.co/960x540?text=Video+demo+soon)
...
{/* VIDEO_DEMO_SOON: Show Chrome and Firefox runs picking different env files and producing different resolved values. */}
![Video demo soon: browser-specific env variants](https://placehold.co/960x540?text=Video+demo+soon)
...
{/* VIDEO_DEMO_SOON: Show one value consumed in `manifest.json`, HTML, and JS (`process.env` / `import.meta.env`) during a build. */}
![Video demo soon: using env values across extension files](https://placehold.co/960x540?text=Video+demo+soon)
