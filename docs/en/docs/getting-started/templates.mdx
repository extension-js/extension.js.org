import {PackageManagerTabs} from '@theme'
import {AvatarImage} from '../../../../theme/components/avatar-image'
import {CarouselTemplates} from '../../../../theme/components/carousel-templates'
import {ALL_TEMPLATES} from '@components/carousel-templates/data'

<AvatarImage icon="extension" />

# Templates

Start faster by generating extensions from official templates instead of building folder structure and config from scratch.

## Template capabilities

| Capability              | What it gives you                                         |
| ----------------------- | --------------------------------------------------------- |
| Ready project structure | Start with a working manifest, scripts, and source layout |
| Stack-specific setup    | Choose templates by framework/runtime needs               |
| Faster onboarding       | Move from idea to runnable extension in minutes           |
| Consistent defaults     | Reduce setup drift across projects and team members       |

## Quick usage

<PackageManagerTabs
  command={{
    npm: 'npx extension@latest create my-extension --template=<slug>',
    pnpm: 'pnpx extension@latest create my-extension --template=<slug>',
    yarn: 'yarn dlx extension@latest create my-extension --template=<slug>'
  }}
/>

Replace `<slug>` with the template you want to start from.

## Choose your template fast

- Start with `new` or `new-typescript` for a minimal JavaScript or TypeScript baseline.
- Pick `new-react`, `new-preact`, `new-vue`, or `new-svelte` when your UI stack is framework-first.
- Match the template to your team stack to reduce setup drift.

{/* VIDEO_PLACEHOLDER: Show browsing templates, selecting one slug, creating a project, and running the first `dev` command. */}
![Video placeholder: template selection and scaffold flow](https://placehold.co/960x540?text=Video+placeholder)

## Available templates

Highlighted starter options include TypeScript, JavaScript, Svelte, Vue, React, and Preact.

{
(() => {
const preferredTemplateSlugs = [
'new-typescript',
'new',
'new-svelte',
'new-vue',
'new-react',
'new-preact'
]

    const preferredTemplates = preferredTemplateSlugs.flatMap((slug) =>
      ALL_TEMPLATES.filter((template) => template.name === slug)
    )

    return <CarouselTemplates templates={preferredTemplates} />

})()
}

## How it works

When you run `create --template=<slug>`, Extension.js fetches the selected template, generates the project files, and can install dependencies automatically.

## More templates

Find official template sources in the examples repository:

- [extension-js/examples](https://github.com/extension-js/examples/tree/main/examples)

Template folder names map directly to `--template=<slug>`.

## Best practices

- Use templates to bootstrap your project quickly and efficiently.
- Start using templates with technology stacks you are familiar with.
- Choose the browser extension context that best fits your project requirements.

## Next steps

- Learn how [Browser runners](../browsers/browsers-available) help you validate extension behavior.
- Learn how Extension.js handles [Environment variables](../features/environment-variables).

---
