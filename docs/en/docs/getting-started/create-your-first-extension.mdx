...
{/* VIDEO_PLACEHOLDER: Show the final UX first: type `gh` in the browser omnibox, enter a query, and open GitHub results in a new tab. */}
![Video placeholder: final omnibox GitHub search flow](https://placehold.co/960x540?text=Video+placeholder)
...
{/* VIDEO_PLACEHOLDER: Show the interface and interaction model for the omnibox keyword (`gh`) before implementation details. */}
![Video placeholder: omnibox interaction model](https://placehold.co/960x540?text=Video+placeholder)
...
{/* VIDEO_PLACEHOLDER: Existing Loom embed for manifest setup. Keep this embed and replace only if explicitly requested. */}
...
{/* VIDEO_PLACEHOLDER: Existing Loom embed for first `dev` run and loading the extension in the browser. Keep this embed and replace only if explicitly requested. */}
...
{/* VIDEO_PLACEHOLDER: Show omnibox live suggestions returned from GitHub API while typing, then selecting one result. */}
![Video placeholder: omnibox suggestions enhancement](https://placehold.co/960x540?text=Video+placeholder)
