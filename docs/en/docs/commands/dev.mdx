...
{/* VIDEO_PLACEHOLDER_TODO: Show `--source` flow for remote extension inspection and output updates while watching source changes. Intentionally hidden until a real demo video is available so it does not render in docs or appear in llms artifacts. */}
