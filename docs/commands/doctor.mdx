...
{/* VIDEO_SPEC id=cmd-doctor grammar=A→B ~12s loop  [SHIPPED: video/scenes/cmd-doctor.scene.mjs]
  Needs: mock transcript `cmd-doctor` (the 7 check lines: passes, one fail with its
    remediation line, exit 1)
  A terminal.type('extension doctor')               mark:command-submitted
  B checks stream in dependency order ~3s (no ramp — each line is the content);
    clip ends on the first failing leg and its one-line fix
                                                    mark:diagnosis-shown → hold read → end
  post: trim start→end, posterAt diagnosis-shown, loop
  run: node video/record.mjs cmd-doctor --mock */}
...
{/* VIDEO_SPEC id=doctor-sessions grammar=A→B ~12s loop
  Needs: mock transcript `doctor-sessions` (two live sessions: chromium + firefox
    contracts under dist/extension-js/, then the doctor run)
  A terminal.type('extension doctor')               mark:command-submitted
  B the session-resolution warn names both sessions and the chosen one
    mark:warn-shown, then the numbered legs pass     mark:checks-shown → hold read → end
  run: node video/record.mjs doctor-sessions --mock */}
