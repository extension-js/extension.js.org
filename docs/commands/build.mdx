...
After the asset summary, a successful build prints the result line and then one pointer to the documentation for what comes after the artifact:

```plaintext
⏵⏵⏵ Build succeeded with no warnings. Your extension is ready for deployment.
⏵⏵⏵ Send this build to someone for review: https://docs.extension.dev/share/unpublished-build-for-review?utm_source=extension-js-org&utm_medium=sponsor&utm_campaign=docs-seam
```

Builds that succeed with warnings print `Build succeeded with N warning(s).` followed by the same pointer.
...
- Send the build to a reviewer behind a link by following [Share an unpublished build for review](https://docs.extension.dev/share/unpublished-build-for-review?utm_source=extension-js-org&utm_medium=sponsor&utm_campaign=docs-seam), the link a successful build prints.
