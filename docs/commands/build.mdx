...
{/* VIDEO_SPEC id=cmd-build grammar=A→B ~14s loop  [SHIPPED: video/scenes/cmd-build.scene.mjs]
  Needs: mock transcript `cmd-build` (edge + chrome build summaries, dist tree listing).
    The zip flags were dropped from the typed command: the zip artifact lines print on
    the debug channel only, so a default run shows nothing for them on screen.
  A terminal.type('extension build --browser=edge,chrome')
                                                    mark:command-submitted
  B two build streams ~8s (ramp 2.5x in post); payoff stays in the terminal:
    dist/edge and dist/chrome trees and the deployment-ready close
                                                    mark:artifacts-listed → hold read → end
  post: trim start→end, posterAt artifacts-listed, loop
  run: node video/record.mjs cmd-build --mock */}
...
After the asset summary, a successful build prints two lines and stops:
...
Builds that succeed with warnings print `Build succeeded with N warning(s).` in place of the first line, then the warning details.
...
- A successful run prints a `status: "built"` frame. Its `value` carries the project path, built browsers, the resolved mode, and one summary per browser. Each summary records the output path, asset totals, warning text, and the Safari app identity when relevant.
