...
For monorepo/submodule projects, config-time env resolution (project root first, then workspace-root fallback) is documented in [Environment variables](/docs/features/environment-variables#how-it-works).
...
After running `build`, Extension.js generates optimized files for the selected browser targets. Extension.js writes output to `dist/` with one subfolder per target. Each folder contains bundled JavaScript, CSS, HTML, and required runtime assets.
