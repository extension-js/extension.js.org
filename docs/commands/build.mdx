---
title: "Build command"
description: "Create production extension artifacts for one or more browser targets with the build command, writing optimized output to dist with optional zip packaging."
icon: "puzzle"
keywords: ["extension.js", "extensions", "chrome", "firefox", "edge", "browser", "manifest", "javascript"]
---
...
For monorepo/submodule projects, config-time env resolution (project root first, then workspace-root fallback) is documented in [Environment variables](/features/environment-variables#extensionconfig-env-resolution-in-monorepos).
...
- Run existing build output with [`preview`](/commands/preview).
- Build and launch in one command with [`start`](/commands/start).
...
- Review supported targets in [browsers available](/browsers/browsers-available).
