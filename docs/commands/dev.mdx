...
{/* VIDEO_SPEC id=dev-context-reload grammar=D→E ~14s loop  [SHIPPED: video/scenes/dev-context-reload.scene.mjs]
  Needs: mock transcript `cmd-dev` (reused); fixture manifest.json + manifest.json.pristine;
    usesEditor manifest.json line 3 column 11 (the "name" value's opening quote, so
    shift+end takes `"my-extension",` and the typed string puts all of it back);
    stage(): dev running, browser open, editor on the file — clip opens mid-workflow
    real-mode-only (proof): the new name is legible only on the browser's extensions
    page, so a real take opens on liveUrl chrome://extensions/
  D editor.replaceToEndOfLine('"My Extension (renamed)",')  mark:edited
    editor.save()                                           mark:saved
  E browser.focus(); manifest change = full extension restart, new name visible
    (mock serves one static page and has no navigate(), so it stands the restart
     up as hmr({reload}) — the strategy, not the name)      mark:updated → hold read → end
  post: trim start→end, posterAt updated, ramps [start→edited 1.35x], loop
  run: node video/record.mjs dev-context-reload --mock */}
...
| `--install [boolean]`          | -                  | Install project dependencies when missing. Lifecycle scripts are disabled for safety; set `EXTENSION_ALLOW_INSTALL_SCRIPTS=true` to run them.    | command behavior default                        |
| `--parent-pid <pid>`           | -                  | Exit the dev server as soon as the given process ID dies. For harnesses/agents that spawn `dev`, so a crashed owner cannot leak a server.        | unset                                           |
...
{/* VIDEO_SPEC id=cmd-dev grammar=A→B→C ~14s loop  [SHIPPED: video/scenes/cmd-dev.scene.mjs]
  Needs: mock transcript `cmd-dev` (real 4.0.19 dev shape: browser-detect line, header
    card, ready + compiled lines — NOT the hero.json shape, which predates 4.0.19 and
    needs a re-cut of its own)
  A terminal.type('extension dev')                  mark:command-submitted
  B build streams ~4s (ramp 2.0x in post)           mark:build-streaming-done
  C browser.open() extension visibly running, hold read
                                                    mark:browser-visible → end
  post: trim start→end, posterAt browser-visible, loop
  run: node video/record.mjs cmd-dev --mock  (scenes/hero.scene.mjs is the reference cut) */}
