...
Use source options only with `dev`. Source inspection opens a target URL in the browser and prints the full, live HTML of the page **after** your content scripts run. This is useful for verifying what your extension actually changes on a page.
...
- **Automation reliability:** Treat `dev` as the watch-mode companion (`--no-browser` + `dev --wait`) and `start` as the production companion (`--no-browser` + `start --wait`). Use `--wait-format=json` for machine consumers in Playwright/CI flows.
