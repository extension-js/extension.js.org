...
A Safari session is a full dev session. Once you enable the extension in Safari Settings, `extension logs` reads it, the control bridge attaches, and every save reloads the extension in Safari. See [Building Safari extensions](/docs/browsers/safari).
