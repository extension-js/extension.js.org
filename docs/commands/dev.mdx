---
title: 'Dev command'
description: 'Run the Extension.js dev command for browser extension development with watch mode, HMR, browser launch, and context-aware reload behavior.'
icon: 'puzzle'
keywords:
  [
    'extension.js',
...
`dev` runs the development pipeline, watches project files, and applies update strategies based on what changed (Hot Module Replacement, hard reload, or restart-required diagnostics).
...
If you omit the path, Extension.js uses `process.cwd()`. You can also pass a **GitHub tree URL** (for example, `https://github.com/user/repo/tree/main/path`) to develop a remote extension locally. Extension.js clones it and runs dev mode on the local copy.
...
Use source options only with `dev`. Source inspection opens a target URL in the browser and prints the full, live HTML of the page **after** the browser injects your content scripts. This is useful for verifying what your extension actually changes on a page.
...
- **Browser compatibility:** Test your extension in different browsers to ensure compatibility with your target audience.
...
- **Automation reliability:** Treat `dev` as the watch-mode companion (`--no-browser` + `dev --wait`) and `start` as the production companion (`--no-browser` + `start --wait`). Use `--wait-format=json` for scripts and CI automation.
