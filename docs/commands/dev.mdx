---
title: 'Dev command'
description: 'Run the Extension.js dev command for browser extension development with watch mode, hot module replacement (HMR), browser launch, and context-aware reload behavior.'
icon: 'puzzle'
keywords:
  [
    'extension.js',
...
| ---------------------- | ----------------------------------------------------- |
| Watch mode iteration   | Tight edit -> rebuild -> validate loop while you code |
| Browser-target control | Explicit cross-browser validation per command         |
| Profile-aware runs     | Reliable fresh or persisted profiles for your workflow|
...
If you omit the path, Extension.js uses `process.cwd()`. You can also pass a **GitHub tree URL** (for example, `https://github.com/user/repo/tree/main/path`) to develop a remote extension locally. Extension.js clones the repository and runs dev mode on your local copy.
...
- **Browser compatibility:** Test your extension in different browsers to confirm it works with your target browsers.
- **Polyfilling:** If Firefox or a Gecko-based browser is also a target, use the `--polyfill` flag to enable `browser.*` API compatibility in Chromium-based browsers.
