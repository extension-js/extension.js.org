...
`--no-browser` disables browser launch but keeps the full dev loop. The dev server still watches your files, and on each rebuild it broadcasts a reload over the control bridge to the extension's service worker, which reloads itself — so content scripts, the service worker, and pages pick up your changes without a launched browser driving them. A content-script-only change reloads just the matching tabs (granular reload); a service-worker or manifest change restarts the extension.

This makes `--no-browser` behave like a normal `dev` session for headless, continuous integration (CI), and remote/dev-container workflows: load the built `dist/<browser>` into any browser you control, and it keeps reloading on save. (Use `--no-reload` if you want a static dev bundle that never reloads — see below.)
