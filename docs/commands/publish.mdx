...
Without either, the command exits with code `1` before any network call happens, and prints this to stderr:

```plaintext
No token. Set EXTENSION_DEV_TOKEN (create one in the extension.dev dashboard, or via the project access-tokens API) or pass --token.
```
...
A project is either public or private. `publish` only reads that setting; it never changes it. The platform decides what kind of link you get:
