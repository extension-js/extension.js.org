...
To use the default JavaScript starter, omit `--template` entirely. Add `--template=<slug>` only for another stack from the [official examples](https://github.com/extension-js/examples/tree/main/examples). The slug `init` is an alias for the same default starter. Only use a literal slug named `default` if that folder exists in the examples repository.
