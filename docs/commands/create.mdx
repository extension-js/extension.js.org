...
{/* VIDEO_SPEC id=cmd-create grammar=A→B ~12s loop  [SHIPPED: video/scenes/cmd-create.scene.mjs]
  Needs: mock transcript `cmd-create` (create output: template files, install summary,
    "cd my-extension && extension dev" next-step lines)
  A terminal.type('npx extension@latest create my-extension --template=new-react')
                                                    mark:command-submitted
  B scaffold streams ~5s (ramp 2.0x in post); payoff stays in the terminal:
    the generated tree and the next-step hint       mark:scaffold-done → hold read → end
  post: trim start→end, posterAt scaffold-done, loop
  run: node video/record.mjs cmd-create --mock */}
