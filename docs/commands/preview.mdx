...
> `preview` is run-only. It prefers `dist/<browser>` when that output exists, but it can also run from another unpacked extension folder if that folder already contains a `manifest.json`.
...
If you do not provide a path, Extension.js uses `process.cwd()`.
