...
> `preview` is run-only. It prefers `dist/<browser>` when that output exists. It can also run from another unpacked extension directory that already contains a `manifest.json`.
...
If you omit the path, Extension.js uses the current working directory.
