...
{/* VIDEO_SPEC id=cmd-preview grammar=A→C ~10s loop  [SHIPPED: video/scenes/cmd-preview.scene.mjs]
  Needs: mock transcript `cmd-preview` (launch lines only — preview skips recompiling,
    so the B beat is deliberately absent)
  A terminal.type('extension preview')                 mark:command-submitted
  C browser.open() almost immediately — the takeaway IS the missing build wait,
    hold read                                           mark:browser-visible → end
  post: trim start→end, posterAt browser-visible, loop
  run: node video/record.mjs cmd-preview --mock */}
