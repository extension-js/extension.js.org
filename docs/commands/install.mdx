...
{/* VIDEO_SPEC id=cmd-install grammar=A→B ~12s loop  [SHIPPED: video/scenes/cmd-install.scene.mjs]
  Needs: mock transcript `cmd-install` (download progress, unpack lines, resolved
    cache path printed last)
  A terminal.type('extension install chrome')       mark:command-submitted
  B download + unpack streams ~5s (ramp 2.5x in post); payoff is the cache path
    line (~/Library/Caches/extension.js/browsers)   mark:installed → hold read → end
  post: trim start→end, posterAt installed, loop
  run: node video/record.mjs cmd-install --mock */}
