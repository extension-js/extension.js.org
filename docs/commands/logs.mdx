...
{/* VIDEO_SPEC id=logs-follow grammar=A→B ~12s loop
  Needs: mock transcript `logs-follow` (running dev session, a file edit that logs
    from background and content in the same second)
  A terminal.type('extension logs --follow')        mark:command-submitted
  B a source file is saved in a second pane, entries stream in seq order from
    background and content contexts                 mark:stream-flowing → hold read → end
