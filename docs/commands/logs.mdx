...
A Safari session is read the same way. `extension logs --browser safari` prints the background and content rows of a `dev --browser safari` session, and `--follow` streams them. See [Building Safari extensions](/docs/browsers/safari).
