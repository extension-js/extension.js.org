...
{/* VIDEO_SPEC id=inspect-loop grammar=A→B ~14s loop
  Needs: mock transcripts `inspect-loop.inspect` + `.inspect.2` (tabs.query rows
    with numeric ids, then the DOM summary for the chosen tab)
  A terminal.type('extension inspect --list-tabs')  mark:tabs-listed
  B terminal.type('extension inspect --tab 412 --with-console'), the DOM
    summary returns with the console tail printed
    below it in the shared logs line format         mark:inspect-returned → hold read → end
