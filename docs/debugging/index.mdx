...
- **`eval` needs `--allow-eval` _and_ a per-session token.** The token is written to a `0600` file outside `dist/` so it never ships in a build — a random local process can't quietly drive your service worker.
