...
{/* VIDEO_SPEC id=debug-drive grammar=A→B ~10s loop  [SHIPPED: video/scenes/debug-drive.scene.mjs]
  Needs: mock transcript `debug-drive` (eval round-trip; pretty output is
    JSON.stringify(value, null, 2), NOT an envelope — the schema-1 envelope
    prints only under --output json); one command only, since a second verb
    would need a second take; a real take needs a dev session already running
  A terminal.type('extension eval "({title: document.title, url: location.href})" --context page')
                                                    mark:command-submitted
  B the object comes back from the live tab — no DevTools window, no context
    dropdown, just a value at a prompt      mark:eval-returned → hold read → end
  post: trim start→end at rate 2.0 through command-submitted (77 chars of
    typing), return leg unramped, posterAt eval-returned, loop
  run: node video/record.mjs debug-drive --mock */}
...
The log stream also carries structured `dx.signal` entries, machine-readable diagnostics the runtime emits about its own health. Filter to just those with `extension logs --signals-only`.
