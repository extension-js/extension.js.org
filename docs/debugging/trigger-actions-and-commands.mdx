...
This works because Extension.js captures your `chrome.action.onClicked` and `chrome.commands.onCommand` listeners at build time (the dev runtime loads before your background code) and replays them on demand. The replay runs in the background context, the same service worker or background page where your listeners live. No window, no mouse, no flake.

{/* VIDEO_SPEC id=trigger-action grammar=A→B ~9s loop  [SHIPPED: video/scenes/trigger-action.scene.mjs]
  Needs: mock transcript `trigger-action` (the one-line --output json envelope —
    printResult JSON.stringify's it with no indent; the pretty blocks below are
    the same envelope formatted for the page); the onClicked example, i.e. an
    action with NO default_popup; a real take needs a dev session already
    running with --allow-control
  A terminal.type('extension open action --output json')
                                                    mark:command-submitted
  B one control-channel round trip and the envelope prints — the handler ran
    with no window, no mouse, no click
                                          mark:action-replayed → hold read → end
  No beat C, and no popup shot. On an action that DOES declare a default_popup
    the verb calls chrome.action.openPopup(), which refuses with "Could not find
    an active browser window" (needs_headed_window → E_HEADED_WINDOW_REQUIRED)
    unless a browser window is frontmost — so typing in the terminal and then
    cutting to the browser can never fire it, in any order. Shooting it needs
    type(…, {submit: false}), the cut, then terminal.submit(), which runs the
    line from off camera; that hides the moment the command runs, and a popup on
    screen looks exactly like someone clicked, which is the opposite of the
    claim. The replay is the honest shot.
  post: trim start→end at rate 1.6 through command-submitted (34 chars of
    typing), round trip unramped, posterAt action-replayed, loop
  run: node video/record.mjs trigger-action --mock */}
...
```bash
pnpm extension open action --output json
```
...
```bash
pnpm extension open command --name toggle-theme --output json
```
...
Boot a session, fire the action, and assert on the side effect, with no browser UI and no Playwright. This is the whole loop in a GitHub Actions job:

```yaml
- run: pnpm extension dev --browser=chromium --allow-control --wait --output json
