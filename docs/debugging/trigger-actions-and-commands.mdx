...
<Frame>
  <iframe
    ...
    src="https://www.youtube-nocookie.com/embed/pE5lPLcyYsY?rel=0"
    title="Extension.js: extension open action"
    loading="lazy"
...
The first line boots the session in the background. The second is the readiness gate: `--wait`
waits for that session's [ready contract](/docs/contracts/ready-json) and exits, it never
launches anything itself. Keep both in one `run:` step so the backgrounded server and the gate
share a shell.
...
- [Chrome DevTools MCP](/docs/integrations/chrome-devtools-mcp): fidelity vs. portability, side by side.
