...
description: "Fire chrome.action.onClicked and chrome.commands.onCommand from the terminal or CI — headless, deterministic, on Chrome and Firefox. Plus a real-gesture click on Chromium 149+ when you need activeTab."
keywords:
  [
...
<Note>
Requires a dev session started with `--allow-control` — see [Debugging overview](/docs/debugging).
</Note>
...
```json
{ "ok": true, "value": { "triggered": "onClicked", "listeners": 1, "gesture": false } }
```
...
```json
{ "ok": true, "value": { "triggered": "command", "command": "toggle-theme", "listeners": 1, "gesture": false } }
```
...
We recommend [running it alongside Extension.js](/docs/integrations/chrome-devtools-mcp) anyway: let it own genuine-gesture browser driving, while Extension.js owns the build-aware, cross-browser, headless replay you use for everything else.
