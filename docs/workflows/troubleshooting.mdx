...
description: "Unblock local development quickly by checking high-signal failures first, from manifest errors to HMR issues and browser launch problems."
icon: "puzzle"
keywords: ["browser", "manifest", "react", "vue", "preact", "hmr", "docs", "workflows"]
---
...
| Changes are not updating | Update type (HMR vs restart-required) | Review [Dev update behavior](/workflows/dev-update-behavior) and restart when required |
...
See [dev update behavior](/workflows/dev-update-behavior) for the full matrix.
...
When running inside a container, the dev server binds to `127.0.0.1` by default, which is unreachable from the host.

- Pass `--host 0.0.0.0` to bind to all interfaces.
- Use `--port 0` to let the OS assign a free port if `8080` is taken.
- If browser connections are slow or flaky in CI/containers, increase the transport timeouts via [browser transport tuning variables](/features/environment-variables#browser-transport-tuning-variables).
...
- Review [security checklist](/workflows/security-checklist).
- Review [performance playbook](/workflows/performance-playbook).
