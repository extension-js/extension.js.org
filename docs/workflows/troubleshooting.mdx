...
If the build fails with `scripts/ is a reserved folder in Extension.js`, you have a Node.js file inside the `scripts/` special folder. Extension.js wraps every file in `scripts/` with a browser content-script runtime, so Node.js-only files break.
