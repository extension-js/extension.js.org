...
description: 'Fix common Extension.js issues fast with a practical triage path. Covers restart diagnostics, dependency setup, browser launch, and manifest path resolution.'
icon: 'puzzle'
keywords:
...
| Changes are not updating | Update type (HMR vs restart-required) | Review [Dev update behavior](/docs/workflows/dev-update-behavior) and restart when required |
...
When running inside a container, the dev server binds to `127.0.0.1` by default, so you cannot reach it from the host machine.

- Pass `--host 0.0.0.0` to bind to all interfaces.
- Use `--port 0` to let the OS assign a free port if `8080` is taken.
- If browser connections are slow or flaky in CI/containers, increase connection timeouts. See [browser transport tuning variables](/docs/features/environment-variables#browser-transport-tuning-variables).
