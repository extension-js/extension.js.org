...
When you run inside a container, the dev server binds to `127.0.0.1` by default, so the host machine cannot reach it.

- Pass `--host 0.0.0.0` to bind to all interfaces.
- Use `--port 0` to let the OS assign a free port if `8080` is taken.
- If browser connections are slow or flaky in CI/containers, increase the connection timeouts using [browser transport tuning variables](/docs/features/environment-variables#browser-transport-tuning-variables).
