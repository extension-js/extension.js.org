...
For a still image instead of a recording, `chrome.tabs.captureVisibleTab` returns the visible part of the active tab as a data URL. It needs the `activeTab` permission or a host permission for the page.

```js
const dataUrl = await chrome.tabs.captureVisibleTab(undefined, { format: "png" })
```

A full-page screenshot is not one call. Scroll the page in steps from a content script, capture each step, and stitch the images on a canvas. Full-page capture tools work this way.
