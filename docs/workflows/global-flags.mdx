...
Extension.js telemetry is intentionally anonymous and privacy-safe. It collects less data than analytics typically used in web framework tooling.
...
- Telemetry behavior and privacy details are documented in repository `TELEMETRY.md` and in [Telemetry and privacy](/docs/features/telemetry-and-privacy).
- Command pages document command-specific flags separately.
- Global flags can be combined with command-specific options.
- For machine consumers (AI/CI), check command readiness from `dist/extension-js/<browser>/ready.json` instead of parsing terminal logs.
