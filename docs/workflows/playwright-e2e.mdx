---
title: "Playwright E2E testing for extensions"
description: "Write repeatable end-to-end tests for browser extensions with Playwright. Cover extension flows, UI rendering, and integrations in CI and locally."
keywords:
  [
    "extension.js",
    "extensions",
...
{/* VIDEO_SPEC id=e2e-run grammar=A→B ~10s loop  [SHIPPED: video/scenes/e2e-run.scene.mjs]
  Needs: mock transcript `e2e-run` (playwright 1.56 list reporter: 3 tests, one
    worker, green summary, html report hint); terminal-only for now — Playwright
    drives its own Chromium outside the actor's stage control, so no browser beat
  A terminal.type('npx playwright test --project=chromium')
    (--project is load-bearing: the baseline config above declares two projects,
    so a bare run is two worker jobs with interleaved rows and an even test
    count)                                          mark:command-submitted
  B tests stream ~5.7s (ramp 2.0x in post); payoff is "3 passed" with the
    ready.json gate having done the waiting         mark:tests-green → hold read → end
  post: trim start→end, ramp typing 1.7x and the stream 2.0x, posterAt
    tests-green, loop
  run: node video/record.mjs e2e-run --mock */}
