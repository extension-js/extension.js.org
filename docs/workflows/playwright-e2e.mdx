...
In other words, `--no-browser` produces build output and the wait step confirms readiness before tests proceed. If your environment cannot run a second CLI process, poll `ready.json` directly instead.
...
This repository includes a Playwright configuration at:
