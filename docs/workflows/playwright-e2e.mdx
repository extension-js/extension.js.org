...
description: 'Write repeatable end-to-end tests for browser extensions with Playwright. Test extension flows, UI rendering, and integrations in CI and local environments.'
icon: 'puzzle'
keywords:
  [
    'extension.js',
...
- A readiness gate (`extension dev --wait --browser=<browser>`) is the **synchronization step**. It tells Playwright when the extension is ready.

In other words, `--no-browser` produces build output. The wait step then confirms readiness before tests proceed. If your environment cannot run a second CLI process, poll `ready.json` directly instead.
...
You can find a Playwright config in this repository at:
