...
When CI runs inside Docker or a devcontainer, pass `--host 0.0.0.0` so the host or other containers can reach the dev server. If the default port is taken, use `--port 0` for OS-assigned allocation.
