...
When CI runs inside Docker or a dev container, pass `--host 0.0.0.0` so external processes can reach the dev server. If another process already occupies the default port, use `--port 0` for automatic assignment.
