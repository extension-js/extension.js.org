---
title: "CI templates for browser extensions"
description: "Automate lint, build, and Playwright E2E checks for your browser extension project using GitHub Actions CI templates with Extension.js."
icon: "puzzle"
keywords: ["firefox", "browser", "playwright", "docs", "workflows", "ci templates", "templates", "capabilities"]
---
...
When CI runs inside Docker or a devcontainer, pass `--host 0.0.0.0` so the dev server is reachable from the host or other containers. If the default port is taken, use `--port 0` for OS-assigned allocation.

```bash
pnpm extension dev --host 0.0.0.0 --port 0 --no-browser
```

For slow or resource-constrained containers, increase browser transport timeouts via environment variables (see [browser transport tuning variables](/features/environment-variables#browser-transport-tuning-variables)).
...
- Add or refine [Playwright E2E tests](/workflows/playwright-e2e).
- Review [commands reference](/commands/dev) for script alignment.
