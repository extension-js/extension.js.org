---
title: "CI templates for extension builds"
...
Automate lint, build, and end-to-end (E2E) checks so extension changes stay releasable.

Use continuous integration (CI) to run the same quality gates on every pull request and release branch.
...
- For scripts or CI automation, prefer `--output json`. The older `--wait-format` alias still works but warns on stderr.
- Under machine output, human copy moves to stderr, so stdout carries exactly one parseable JSON document.
- For teardown, read `browserPid` from `ready.json` and signal that process. It is the supported handle for reaping the browser.

### Example readiness gate

```bash
pnpm extension start --wait --browser=chromium --wait-timeout=60000 --output json
```
...
```yaml
- run: |
    pnpm extension dev --browser=chromium --allow-control &
    pnpm extension dev --browser=chromium --wait --output json
    pnpm extension open action --browser=chromium --output json
...
The first line boots the session, the second gates on its ready contract. `--wait` only waits,
it does not launch a session, so both belong in the same `run:` step.

The same approach replays keyboard commands (`extension open command --name <cmd>`). See [Trigger actions and commands](/docs/debugging/trigger-actions-and-commands) for the result payloads and the one caveat (replay carries no user gesture, so `activeTab` is not granted).
...
`create` does not scaffold a CI workflow. For a complete example to copy, read the one Extension.js runs on itself:

- [`.github/workflows/e2e.yml`](https://github.com/extension-js/extension.js/blob/main/.github/workflows/e2e.yml)
