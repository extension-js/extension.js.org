...
When your CI runs inside Docker or a devcontainer, pass `--host 0.0.0.0` so external processes can reach the dev server. If the default port is taken, use `--port 0` for automatic port assignment.
