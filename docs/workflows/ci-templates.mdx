...
- Prefer the built-in wait commands. Use `extension dev --wait --browser=<browser>` for watch/dev or `extension start --wait --browser=<browser>` for production.
...
When CI runs inside Docker or a devcontainer, pass `--host 0.0.0.0` so external processes can reach the dev server. If the default port is taken, use `--port 0` for automatic port assignment.
...
You can find an E2E workflow example in this repository at:
