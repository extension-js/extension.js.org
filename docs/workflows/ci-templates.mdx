...
When CI runs inside Docker or a devcontainer, pass `--host 0.0.0.0` so external processes can reach the dev server. If another process is using the default port, use `--port 0` for automatic port assignment.

```bash
pnpm extension dev --host 0.0.0.0 --port 0 --no-browser
```

For slow or resource-constrained containers, increase browser transport timeouts with environment variables. See [browser transport tuning variables](/docs/features/environment-variables#browser-transport-tuning-variables).
