...
Extensions tend to fail store audits and internal reviews for the same handful of reasons: permissions that have drifted wider than the feature needs, content scripts that trust page context, web accessible resources (WAR) that expose internals to arbitrary origins, and message handlers that act on unverified senders. Run this checklist before every release cut.
