...
Extension performance regressions tend to come from a short list of places. The most common culprits are service worker cold-starts (initial loads when no cached state exists), heavy work at content-script injection time, oversized UI bundles, and media-heavy web-accessible resources. The checklists below map each regression to a concrete fix you can apply without restructuring your project.
