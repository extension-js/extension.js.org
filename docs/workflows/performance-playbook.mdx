...
Extension performance regressions come from a short list of places: service worker cold-starts (initial startup delays), heavy content-script injection, oversized UI bundles, and media-heavy web-accessible resources. The checklists below map each regression type to a concrete fix.
...
- Limit web-accessible resource (WAR) exposure to required assets.
