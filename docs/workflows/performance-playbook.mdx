...
Extension performance regressions tend to come from a short list of places. The most common are cold-starts (initial loads when no cached state exists) in the service worker, heavy work at content-script injection time, oversized UI bundles, and media-heavy web accessible resources. The checklists below map each regression to a concrete fix. You can apply them without restructuring the project.
