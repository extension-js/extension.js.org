...
<Frame>
  <iframe
    ...
    src="https://www.youtube-nocookie.com/embed/xSVgKCiJBIg?rel=0"
...
extension navigate https://example.com/ --browser=chromium --output json
extension logs --context content --output ndjson
```

`navigate` points a tab at a url through the extension itself, as a static
`tabs` call rather than an eval, so it works on every engine including a
Safari MV3 background that refuses eval. It reuses the active tab, takes
`--tab <id>` for a specific one, and `--new-tab` (with `--background` to
leave it unfocused) to open another. Firefox refuses privileged pages such
as `about:newtab` and `about:config` from an extension's tabs API, so
`navigate` answers `E_ARGS` for them there with a hint naming the rule, while
`chrome://newtab/` opens on Chromium. Chromium refuses `javascript:` urls the
same way, and the hint points at `eval` instead. Web urls and `about:blank`
work on every engine. A `--tab` id no open tab carries answers
`E_TARGET_NOT_FOUND` on every verb, with `inspect --list-tabs` as the way to
find one.
