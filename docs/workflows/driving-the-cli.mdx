...
{/* VIDEO_SPEC id=agent-drives-dev grammar=A→B ~12s loop
  Needs: mock transcripts agent-drives-dev.capabilities + .storage; terminal-only,
    the agent IS the terminal. The --allow-control dev session runs off camera in
    its own process: a held dev prompt cannot take another command.
  A terminal runs `extension capabilities`, reads the envelope  mark:handshake-shown
  B `extension storage get --key todos --output json` answers with a schema-1
    envelope the agent parses            mark:envelope-shown → hold read → end
