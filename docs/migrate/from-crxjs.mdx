...
{/* VIDEO_SPEC id=migrate-crxjs grammar=A→B→C ~14s loop  [family: migrate-wxt]  [SHIPPED: video/scenes/migrate-crxjs.scene.mjs]
  Needs: mock transcript `migrate-crxjs` — its own file, not a cmd-dev reuse: the
    prompt reads ~/crxjs-app, so output naming my-extension would contradict it.
    real-mode-only: a genuinely migrated CRXJS project (manifest.config.ts folded into
    plain manifest.json). See the migrate-wxt spec for what the family shape claims.
  ...
  C browser.open() the migrated project running on Extension.js — the page's own
    step 7, no UI rewrite                           mark:browser-visible → hold read → end
...
- **Hashed asset paths:** production builds under [`dist/<browser>`](/docs/features/path-resolution) use clean names. `extension dev` appends a build hash to content scripts, for example `content_scripts/content-0.813e908a.js`, to defeat the extension resource cache. See [Reload and HMR](/docs/features/reload-and-hmr). If your code hardcoded Vite-style hashed filenames, replace them with manifest-relative paths.
