...
{/* VIDEO_SPEC id=migrate-wxt grammar=A→B→C ~14s loop  [SHIPPED: video/scenes/migrate-wxt.scene.mjs]
  Needs: mock transcript `migrate-wxt` — its own file, not a cmd-dev reuse: the prompt
    reads ~/wxt-app, so output naming my-extension would contradict it. There is no
    `extension migrate` command; steps 1-5 are hand edits to files, so step 6 is the
    only beat a clip can carry. real-mode-only: a genuinely migrated WXT project
    (manifest.json in place of entrypoints/, wrappers unwrapped). In mock the migrated
    project is carried by name in the boot output; the payoff page is the shared mock
    newtab, so the clip claims "it boots here", not "your UI is byte-identical" —
    that claim needs a before/after the shot grammar has no beat for.
  A terminal.type('extension dev')  (typed inside the migrated project)
  ...
  run: node video/record.mjs migrate-wxt --mock */}

## What changes, what stays

**Stays the same:** your React/Vue/Svelte components, your styles, your tests, your
`browser.*`/`chrome.*` API calls (`extension dev` applies the `browser` polyfill by
default, and `extension build` takes `--polyfill`), and `@wxt-dev/storage` if you use it standalone (it wraps the extension
...
- `browser.*` calls: keep them. `extension dev` applies the polyfill by default, and
  `extension build` needs `--polyfill`. Switching to `chrome.*` also works.
...
Firefox with `--browser=firefox`. `extension dev` applies the polyfill by default, so
`browser.*` code runs on Chromium as written. Pass `--polyfill` to `extension build`, where
the default is off.

## Common gotchas

- **Manifest V2:** WXT supports MV2 as a build target, and Extension.js targets Manifest V3.
