...
{/* VIDEO_SPEC id=migrate-plasmo grammar=A→B→C ~14s loop  [family: migrate-wxt]  [SHIPPED: video/scenes/migrate-plasmo.scene.mjs]
  Needs: mock transcript `migrate-plasmo` — its own file, not a cmd-dev reuse: the
    prompt reads ~/plasmo-app, so output naming my-extension would contradict it.
    real-mode-only: a genuinely migrated Plasmo project (popup.tsx / contents/*
    conventions mapped into manifest.json entries). See the migrate-wxt spec for what
    the family shape claims.
  A terminal.type('extension dev')                  mark:command-submitted
  B browser-detect line, header card naming the migrated project, ready + compiled
    (~3.6s, ramp 2.0x in post)                      mark:build-streaming-done
  C browser.open() the migrated project running on Extension.js — the page's own
    step 6, no UI rewrite                           mark:browser-visible → hold read → end
  post: trim start→end, posterAt browser-visible, loop
  run: node video/record.mjs migrate-plasmo --mock */}

## Is Plasmo still maintained

Check the signals yourself before you decide. These values come from npm and GitHub on 14 September 2026.

| Signal                  | Value                       |
| ----------------------- | --------------------------- |
| Last npm release        | 0.90.5, published 17 May 2025 |
| Last commit on `main`   | 17 May 2025                 |
| Open issues             | 371                         |
| Downloads in the last month | about 2.3 million       |

Plasmo still installs and still builds. Downloads stay high because existing projects keep it pinned. There has been no release in sixteen months. If your extension works today, you do not need to move. If you start a new extension, or you need Node.js 22, current Manifest V3 changes, or a bundler that still receives updates, this guide takes about ten minutes.
...
Check popup, options, content scripts, and background behavior, then run the same against Firefox with `--browser=firefox`. Code that calls `chrome.*` runs on Firefox as written, because Firefox supports the `chrome.*` namespace natively. The `--polyfill` flag is for the other direction: it supplies `browser.*` on Chromium targets. `extension dev` applies it by default, and `extension build` leaves it off unless you pass the flag.

## Common gotchas

- **Icons in `assets/`:** Plasmo auto-generates icon sizes from `assets/icon.png`. Extension.js uses whatever `"icons"` declares in the manifest, so export the sizes you need. See [Icons](/docs/implementation-guide/icons).
...
- **Storage hooks:** `useStorage` from `@plasmohq/storage/hook` works unchanged, because it only depends on React and `chrome.storage`.

## See also

- [Migrate from WXT](/docs/migrate/from-wxt)
