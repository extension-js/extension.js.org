...
{/* VIDEO_SPEC id=browser-switch grammar=A→B→C ~14s loop  [SHIPPED: video/scenes/browser-switch.scene.mjs]
  Needs: mock transcript `browser-switch` (one dev block per browser, in the
    order typed); scene browserTargets ['chromium','edge','firefox'] — the
    actor drives Edge, Firefox and Brave alongside Chromium now, so this is NOT
    real-mode-only; recorder wiring still owed: scene targets passed into
    createBrowser, the stage guard's app list taken from browser.processNames(),
    and stage.register() accumulating a pid PER browser (it keeps one today, so
    a switch orphans the earlier browsers and they fail the take as strangers)
  NOTE the typed command says chromium, not chrome: Chrome 150 ignores
    --load-extension silently, so a Chrome window here would carry no extension
    and no theme. chromium is the CLI default and the table's first row.
  NOTE Firefox's striped "under remote control" bar stays visible: dev always
    passes -start-debugger-server, so every real Firefox session shows it.
  A terminal.type('extension dev --browser=chromium,edge,firefox')
                                                    mark:command-submitted
  B the first browser's block streams (detect, card, ready, compiled)
                                                    mark:build-streaming-done
  C browser.open() then two switchTo(), each window raised into the same stage
    rect, each held before the next          mark:chromium-visible → chromium-held
                                             → edge-visible → edge-held
                                             → firefox-visible → end
  post: trim start→end, posterAt edge-visible, ramp the typing and each ~2s
    launch/switch gap (the holds stay at 1x), loop
  run: node video/record.mjs browser-switch --mock */}
