...
**Fully supported CLI targets** include the Chromium family (`chromium`, `chrome`, `edge`, `chromium-based` with a custom binary) and Firefox (`firefox`, `gecko-based` / `firefox-based`). Extension.js documents and tests those `--browser` values end-to-end.
