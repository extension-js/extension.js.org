...
If you don't specify a browser, the CLI defaults to `chromium`.
...
**Fully supported CLI targets** are the Chromium family (including `chromium`, `chrome`, `edge`, and `chromium-based` with a custom binary) and Firefox (`firefox`, `gecko-based` / `firefox-based`). Extension.js documents and tests those `--browser` values end-to-end.
