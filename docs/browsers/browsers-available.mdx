...
**Fully supported CLI targets** are the Chromium family and Firefox (Gecko engine). Chromium targets include `chromium`, `chrome`, `edge`, and `chromium-based` with a custom binary. Firefox targets include `firefox`, `gecko-based`, and `firefox-based`. Extension.js documents and tests these `--browser` values end-to-end.

**Safari** is not a supported `--browser` value in the CLI today. You can still author extensions with shared web code and add Safari-specific manifest or build steps outside this workflow. Do not assume that npm package keywords imply a managed Safari target.
