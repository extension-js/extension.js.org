...
Settings merge in this order: browser defaults, then command defaults, then the CLI-selected command context.
...
- **Chromium family (`chrome`, `edge`, `chromium`, `chromium-based`)**
  ...
  - Extension.js manages `--load-extension=...` and removes it from user-provided flags.
