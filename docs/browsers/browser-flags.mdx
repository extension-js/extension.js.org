...
{/* VIDEO_SPEC id=browser-flags grammar=A→C ~14s loop  [SHIPPED: video/scenes/browser-flags.scene.mjs]
  Needs: mock transcript `cmd-dev` (reused); extension.config.js fixture with
    browserFlags: ["--auto-open-devtools-for-tabs"]; in mock the browser actor
    launches Chrome with the same flag so the payoff is authentic
  A terminal.type('extension dev')                  mark:command-submitted
  C browser.open() — DevTools is already open on launch: the config flag did
    that, no manual step                            mark:browser-visible → hold read → end
  post: trim start→end, posterAt browser-visible, loop
  run: node video/record.mjs browser-flags --mock */}
