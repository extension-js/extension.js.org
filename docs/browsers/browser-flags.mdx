...
A headless session, a container, or a virtual machine often has no GPU. WebGL contexts then fail to create, and canvas code that needs one throws. Pass the Chromium switches through `browserFlags`:
...
`--ignore-gpu-blocklist` lets Chromium use a GPU that its blocklist would refuse. `--use-gl=angle` with `--use-angle=swiftshader` renders WebGL in software, which is the right choice in continuous integration. `--enable-unsafe-webgpu` turns on WebGPU where the browser still gates it. Firefox has no equivalent switches. Set `webgl.force-enabled` through browser preferences instead.
...
<Frame>
  <iframe
    ...
    src="https://www.youtube-nocookie.com/embed/GHyvLeSUaa4?rel=0"
