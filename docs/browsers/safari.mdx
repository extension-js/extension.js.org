...
- **Manual one-time enable.** Toggling the extension on is a Safari security control and cannot be automated. On ad-hoc builds, allowing unsigned extensions is a second control with the same rule.
