...
A paid Apple Developer account is **not** required. Without `--development-team` the app is signed ad-hoc, and Safari accepts an ad-hoc signed app: it lists the extension like any other, with the Develop menu setting **Allow unsigned extensions** turned off. That setting belongs to a different path, described in [Temporary extensions](#temporary-extensions-no-xcode).
...
- **Always Allow on Every Website…** for a development extension that you are
...
Developer ▸ **Add Temporary Extension…**, which loads an unpacked folder such as
...
- **On every save**: an incremental `xcodebuild` resync updates the app's resources from the freshly rebuilt `dist/safari`, and the extension reloads in Safari.
...
converter, while a background page ran immediately. WebKit deliberately prefers
...
still ships, and it is your call whether to guard it, branch on the target, or
...
- **Packaging is macOS only.** The Xcode step needs macOS with the full Xcode app. (`build` on other platforms still produces `dist/safari`, and `dev` requires macOS.)
- **`preview` and `start` have no Safari path.** See [Command support](#command-support) for what was measured.
- **The enable gesture is manual.** Turning the extension on and granting it website access are Safari security controls and cannot be automated. Both survive restarts, so you pay for them once.
