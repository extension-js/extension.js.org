...
{/* VIDEO_SPEC id=safari-build grammar=A→B ~11s loop  [SHIPPED: video/scenes/safari-build.scene.mjs]
  Needs: mock transcript `safari-build` (transcribed from a real macOS build on
    Xcode 26.3, converter and xcodebuild timings included). NOT real-mode-only:
    the take is terminal-only, so nothing has to be mocked except the CLI. The
    spec's original beat C cannot be shot in any mode, not just in mock — a
    plain `build` prints the `open` command rather than launching the app, and
    Safari's "Allow Unsigned Extensions" plus the Settings toggle are security
    controls no recorder can drive. Safari is therefore not in this clip.
  A terminal.type('extension build --browser=safari') mark:command-submitted
  B convert 1.25s → xcodebuild 7.60s (ramp 3.0x in post)
                                                    mark:xcodebuild-done
    bundle id note and the Safari enable hint play at 1x, they are prose
                                                    mark:enable-steps-shown → hold read → end
  post: trim start→end, posterAt enable-steps-shown, loop
  run: node video/record.mjs safari-build --mock */}
...
{/* VIDEO_SPEC id=safari-run grammar=A→B ~14s loop
  Needs: mock transcript `safari-run` (dev boot on --browser=safari). Terminal
    side ONLY: the existing safari-build spec abandoned its Safari-side half
    (the enable toggles are security controls no recorder can drive), and this
    spec covers the terminal side of the loop, nothing Safari-window-shaped.
  A terminal.type('extension dev --browser=safari')  mark:command-submitted
  B convert streams, then xcodebuild, then the open line, then the pluginkit
    "registered" confirmation lands             mark:registered → hold read → end
  post: trim start→end, ramp convert+xcodebuild 3.0x, posterAt registered, loop
  run: node video/record.mjs safari-run --mock */}
