...
{/* VIDEO_SPEC id=profile-persist grammar=A→B ~12s loop  [SHIPPED: video/scenes/profile-persist.scene.mjs]
  Needs: mock transcript `profile-persist` (dev boot with persistProfile: true in
    extension.config.js). No "Persistent profile … (reused from previous
    session)" line exists — the header card's Profile row is the only surface,
    and it ends in the stable `dev` leaf where an ephemeral run would show a
    generated three-word name. Reuse across sessions is not filmable: it needs a
    second run, and stopping the first needs ctrl-c (not in the actor
    vocabulary). The clip shows the mode, not the survival.
  A terminal.type('extension dev --browser=chrome') mark:command-submitted
  B boot streams ~3s (ramp 2.0x in post); payoff is the card's Profile row
    resolving to …/profiles/chrome-profile/dev      mark:profile-shown → hold read → end
  post: trim start→end, posterAt profile-shown, loop
  run: node video/record.mjs profile-persist --mock */}
