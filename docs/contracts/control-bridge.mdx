...
From 4.1.19, the server refuses a handshake from a web page. An `Origin` header of `http://`, `https://`, or `null` gets HTTP 401 before any frame. The extension background sends its own scheme, such as `chrome-extension://` or `moz-extension://`. Node.js clients send no `Origin`. Both connect as before.
...
A `dx.signal` event is a structured diagnostic that the runtime raises about the dev loop itself. Branch on its `code` and `status`, and surface `remediation` to the user. The shape is reserved ahead of its producers: no emitter ships yet, so today every event in the stream carries `log`.
...
| `tabs.navigate` | Point a tab at a url: `args.url`, plus `target.tabId` for a specific tab or `args.newTab` (and `args.active`) for a new one; otherwise the active tab. A static `tabs` call, so it works where an MV3 background refuses eval. |
...
| `surface_reply_failed` | The surface was open and ran the op, but its reply never arrived. The message quotes the engine. |
| `api_unavailable`     | The browser API behind the op is unavailable here.         |

The exported constants are `REFUSAL_NEEDS_HEADED_WINDOW`, `REFUSAL_NEEDS_USER_GESTURE`, `REFUSAL_SURFACE_NOT_OPEN`, `REFUSAL_SURFACE_REPLY_FAILED`, and `REFUSAL_API_UNAVAILABLE`.
