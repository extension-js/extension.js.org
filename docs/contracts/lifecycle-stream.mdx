...
{/* VIDEO_SPEC id=lifecycle-frames grammar=A→B ~10s loop
  Needs: mock transcript `lifecycle-frames` (EXTENSION_OUTPUT=ndjson dev session
    already streaming; terminal-only, frames are the whole story)
  A terminal shows starting -> compiled -> ready frames, then an edit lands a
    syntax error and a compile-failed frame arrives with ok:false and
    value.output                                     mark:compile-failed
  B a second edit fixes it and a recompiled frame closes the loop
                                                     mark:recovered → hold read → end
  run: node video/record.mjs lifecycle-frames --mock */}
