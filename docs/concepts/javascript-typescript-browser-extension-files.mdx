...
You do not need to write a `tsconfig.json` from scratch. Extension.js ships sensible defaults. Types for `chrome.*`, `browser.*`, `import.meta.env`, and the public env keys come from Extension.js's own ambient types. From Extension.js 4.1.19, the `@types/chrome` and `@types/webextension-polyfill` packages that those types need install with `extension`. See [Types for the chrome and browser APIs](/docs/languages-and-frameworks/extension-api-types) and [TypeScript](/docs/languages-and-frameworks/typescript).
