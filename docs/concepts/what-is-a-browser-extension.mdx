...
A browser extension is software that adds features to a browser. Extensions can change how pages render, add new UI inside the browser chrome (toolbars, popups, side panels, new-tab pages), respond to network and tab events, and call privileged browser APIs that ordinary web pages cannot.
...
Extensions cannot do anything the user has not granted permission for, and the browser sandboxes extensions away from raw OS access.
...
- **Permissions:** A few `permissions` and `host_permissions` entries are evaluated differently per browser.
...
- Pick a [browser target](/docs/browsers/browsers-available) for development.
