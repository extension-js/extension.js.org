...
{/* VIDEO_SPEC id=multi-browser-dev grammar=A→B ~12s loop
  Needs: mock transcripts for two parallel dev boots (chrome + firefox) on one
    project; split-pane terminal layout, no browser window needed
  A two terminals side by side type `extension dev --browser chrome` and
    `extension dev --browser firefox` on the same project   mark:both-submitted
  B both cards print: each session shows its own profile, port, and control
    channel, no collision                                   mark:cards-visible → hold read → end
  run: node video/record.mjs multi-browser-dev --mock */}
