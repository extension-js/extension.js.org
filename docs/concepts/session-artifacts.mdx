...
{/* VIDEO_SPEC id=multi-browser-dev grammar=A→B ~12s loop
  Needs: mock transcript `multi-browser-dev` (one dev boot: --browser takes a
    comma list and starts one session per vendor); terminal only
  A terminal.type('extension dev --browser=chrome,firefox')  mark:command-submitted
  B both sessions stream into one terminal: two compiles, chrome's card with its
    own profile, firefox's card with its own id, two ready lines, no collision
                                            mark:cards-visible → hold read → end
