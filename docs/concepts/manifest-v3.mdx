---
title: "Manifest V3 service workers, content scripts, and host_permissions explained"
sidebarTitle: "Manifest V3"
description: "How Manifest V3 changes service workers, content scripts, web_accessible_resources, and host_permissions in Chrome and Firefox, with the exact manifest.json fix for each common MV3 error."
keywords:
  [
    "manifest v3",
...
- Bundles the background entry into one file, so `import` resolves at build time, and keeps `type: "module"` when you declare it.
...
- See how [Extension.js compares to WXT](/docs/compare/extension-js-vs-wxt) and other frameworks on MV3 handling.
- Migrating an existing MV3 project? Start from [WXT](/docs/migrate/from-wxt), [Plasmo](/docs/migrate/from-plasmo), or [CRXJS](/docs/migrate/from-crxjs).
- Starting fresh? Scaffold an MV3-ready project from a [template](/docs/getting-started/templates): [React](/docs/getting-started/templates/react), [TypeScript](/docs/getting-started/templates/typescript), [Svelte](/docs/getting-started/templates/svelte), and more.
